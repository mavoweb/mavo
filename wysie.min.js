...
i.client.authDriver(new Dropbox.AuthDriver.Popup({receiverUrl:new URL(location)+""})),i.client.authenticate({interactive:!t},function(t,r){t&&n(Error(t)),i.client.isAuthenticated()?(i.permissions.on(["logout","edit"]),e()):(i.permissions.off(["logout","edit","add","delete"]),n())})})}).then(function(){i.client.getAccountInfo(function(t,n){t||i.wysie.wrapper._.fire("wysie:login",e.extend({backend:i},n))})})["catch"](function(){})},logout:function(){var e=this;return this.client.isAuthenticated()?new Promise(function(t,i){e.client.signOut(null,function(){e.permissions.off(["edit","add","delete"]).on("login"),e.wysie.wrapper._.fire("wysie:logout",{backend:e}),t()})}):Promise.resolve()},"static":{test:function(e){return/dropbox.com/.test(e.host)||"dropbox:"===e.protocol}}}),!0)}}(Bliss),function(e){self.Wysie&&Wysie.Storage.Backend.add("Github",_=e.Class({"extends":Wysie.Storage.Backend,constructor:function(){this.storage.permissions=this.permissions,this.permissions.on("login"),this.key=this.storage.param("key")||"7e08e016048000bc594e",
e.extend(this,_.parseURL(this.url)),this.repo=this.repo||"wysie-data",this.path=this.path||this.wysie.id+".json",this.url=new URL("https://raw.githubusercontent.com/"+this.username+"/"+this.repo+"/"+this.branch+"/"+this.path+"?ts="+Date.now()),this.permissions.on("read"),this.login(!0)},get:Wysie.Storage.Backend.Remote.prototype.get,put:function(e){var t=this;e.data=Wysie.toJSON(e.data),e.path=e.path||"";var i="repos/"+this.username+"/"+this.repo+"/contents/"+e.path;return this.req(i,{data:JSON.stringify({ref:this.branch})}).then(function(n){return t.req(i,{method:"PUT",data:JSON.stringify({message:"Updated "+e.name,content:btoa(e.data),branch:t.branch,sha:n.sha})})}).then(function(e){console.log("success")})},get authenticated(){return!!this.accessToken},req:function(t){var i=arguments.length<=1||void 0===arguments[1]?{}:arguments[1];return e.fetch("https://api.github.com/"+t,e.extend(i,{responseType:"json",headers:{Authorization:"token "+this.accessToken}})).then(function(e){return Promise.resolve(e.response);
})},login:function(e){var t=this;return this.ready.then(function(){return t.authenticated?Promise.resolve():new Promise(function(i,n){e?(t.accessToken=localStorage["wysie:githubtoken"],t.accessToken&&i(t.accessToken)):(t.authPopup=open("https://github.com/login/oauth/authorize?client_id="+t.key+"&scope=repo,gist&state="+location.href,"popup","width=900,height=500"),addEventListener("message",function(e){e.source===t.authPopup&&(t.accessToken=localStorage["wysie:githubtoken"]=e.data,t.accessToken||n(Error("Authentication error")),i(t.accessToken))}))}).then(function(){return t.getUser()}).then(function(e){return t.permissions.on("logout"),t.req("repos/"+t.username+"/"+t.repo)}).then(function(e){e.permissions.push&&t.permissions.on("edit")})["catch"](function(e){return console.error(e)})})},logout:function(){return this.authenticated&&(localStorage.removeItem("wysie:githubtoken"),delete this.accessToken,this.permissions.off(["edit","add","delete"]).on("login"),this.wysie.wrapper._.fire("wysie:logout",{
backend:this})),Promise.resolve()},getUser:function(){var e=this;return this.req("user").then(function(t){e.wysie.wrapper._.fire("wysie:login",{backend:e,name:'<a href="https://github.com/'+t.login+'" target="_blank">\n							<img class="avatar" src="'+t.avatar_url+'" /> '+t.name+"\n						</a>"})})},"static":{test:function(e){return/\bgithub.(com|io)|raw.githubusercontent.com/.test(e)},parseURL:function(e){var t={};e=new URL(e,location);var i=e.pathname.slice(1).split("/");return/github.io$/.test(e.host)?(t.username=e.host.match(/([\w-]+)\.github\.io$/)[1],t.branch="gh-pages"):t.username=i.shift(),t.repo=i.shift(),/raw.githubusercontent.com$/.test(e.host)?t.branch=i.shift():/github.com$/.test(e.host)&&"blob"==i[0]&&(i.shift(),t.branch=i.shift()),t.filename=i[i.length-1],t.path=i.join("/"),t}}}),!0)}(Bliss);