(function ($, $$) {
  let _ = (Mavo.ImplicitCollection = class ImplicitCollection extends (
    Mavo.Node
  ) {
    ...
    getData(o = {}) {
      ...
      if (this.data) {
        ...
        let rendered = Mavo.toArray(Mavo.subset(this.data, this.inPath));
    ...
    dataRender(data, o = {}) {
      if (data !== undefined) {
        data =
          data === null ? [] : Mavo.toArray(data).filter((i) => i !== null);
        let changed = data.length !== this.liveData.length;

        this.children.forEach(
          (item, i) => (changed = item.render(data?.[i], o) ?? changed)
        );
      }
