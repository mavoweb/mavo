  ...
  let _ = (Mavo.UI.Message = $.Class({
    constructor: function (mavo, message, o = {}) {
      ...
      this.element = $.create({
        ...
        [$.type(this.message) == "string" ? "innerHTML" : "contents"]: this
          .message,
        events: {
      ...
      if (o.dismiss.timeout) {
        let timeout =
          typeof o.dismiss.timeout === "number" ? o.dismiss.timeout : 5000;

        $.bind(this.element, {
          mouseenter: (e) => clearTimeout(this.closeTimeout),
          mouseleave: Mavo.rr(
            (e) => (this.closeTimeout = setTimeout(() => this.close(), timeout))
          ),
        });
      }
    ...
    async close(resolve) {
      ...
      let duration = this.element.style.transition
        ? 1000 *
          parseFloat(
