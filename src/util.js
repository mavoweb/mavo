(function ($, $$) {
  let _ = $.extend(Mavo, {
    ...
    primitivify: (object, primitive) => {
      if (object) {
        ...
        object.valueOf = object.toJSON = object[Symbol.toPrimitive] = () =>
          primitive;
      }
    ...
    objectify: (value, properties) => {
      let primitive = Mavo.value(value);
    ...
    delete: (arr, element, all) => {
      do {
        let index = arr && arr.indexOf(element);
    ...
    elementPath: function (ancestor, element) {
      if (Array.isArray(element)) {
        ...
        for (
          ...
          let index = 0;
          let countNonElementSiblings =
            parent === element && element.nodeType !== 1;
          ...
          while (
            (sibling =
              sibling[
                `previous${countNonElementSiblings ? "" : "Element"}Sibling`
              ])
          ) {
    ...
    revocably: {
      add: function (element, insert) {
        let comment = _.revocably.isRemoved(element);
      ...
      remove: function (element, commentText) {
        ...
        if (!comment) {
          commentText =
            commentText || element.id || element.className || element.nodeName;
          comment = _.data(
            element,
            "commentstub",
      ...
      setAttribute: function (element, attribute, value) {
        let previousValue = _.data(element, "attribute-" + attribute);
    ...
    inView: {
      is: (element) => {
        ...
        return (
          ((0 <= r.bottom && r.bottom <= innerHeight) ||
            (0 <= r.top && r.top <= innerHeight)) && // vertical
      ...
      when: (
        element,
        rootMargin = `${innerHeight / 2}px ${innerWidth / 2}px`
      ) => {
        let observer = (_.inView.observer =
          _.inView.observer ||
    ...
    in: function (property, obj) {
      if (obj) {
        return (
          (typeof obj === "object" && property in obj) ||
    ...
    getCanonicalProperty: function (obj, property) {
      if (obj && (property || property === 0)) {
        ...
        if (property.toLowerCase) {
          ...
          let i = properties.map((p) => p.toLowerCase()).indexOf(propertyL);
    ...
    subset: function (obj, path, value) {
      if (arguments.length == 3) {
        ...
        if (path.length) {
          let last = path[path.length - 1];
          let parent = $.value(obj, ...path.slice(0, -1));
        ...
        return path.reduce((obj, property, i) => {
          ...
          path[i] = Array.isArray(meta.property)
            ? meta.property[0]
            : meta.property;
      ...
      let ro =
        callbackOrObserver instanceof ResizeObserver
          ...
          : new ResizeObserver((entries) => {
              let contentRect = entries[entries.length - 1].contentRect;
    ...
    Observer: class Observer {
      ...
      update(element, attribute, options) {
        ...
        if (
          options === undefined ||
          (!options.attributes && !options.childList && !options.characterData)
        ) {
          if (this.attribute) {
    ...
    wrap: (index, length) =>
      index < 0 ? length - 1 : index >= length ? 0 : index,
   ...
   */
  _.Observers = class Observers extends (
    Map
  ) {
    constructor({ observer, callback } = {}) {
      ...
      this.observer =
        observer ||
        (self.observer = self.observer || new MutationObserver(this.callback));
    }
    ...
    static matchesRecord(o, r) {
      ...
      if (o.attribute) {
        ...
        if (
          ...
          !o.attribute.includes?.(r.attributeName)
        ) {
