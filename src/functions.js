...
(function ($, val) {
  let _ = (Mavo.Functions = {
    ...
    get: function (obj, property, meta = {}) {
      ...
      if (Array.isArray(obj) && property && isNaN(property)) {
        ...
        if (eqIndex > -1) {
          ...
          ret = obj.filter((e, i) => {
            let passes = _.get(e, meta.query.property) == meta.query.value;
    ...
    first: (n, arr) => {
      ...
      if (n < 0) {
        ...
        for (let i = 0; i < arr.length && ret.length < numReturn; i++) {
    ...
    last: (n, arr) => {
      ...
      if (n < 0) {
        ...
        for (let i = arr.length - 1; i >= 0 && ret.length < numReturn; i--) {
          if (Mavo.value(arr[i]) !== null) {
            ret.push(arr[i]);
          }
        }
    ...
    intersects: function (arr1, arr2) {
      if (arr1 && arr2) {
        let set2 = new Set(Mavo.toArray(arr2).map(val));
    ...
    replace: $.extend(
      (haystack, needle, replacement = "", iterations = 1) => {
        ...
        let needleRegex = RegExp(Mavo.escapeRegExp(needle), "g");
        let ret = haystack,
          prev;
        let counter = 0;
    ...
    starts: $.extend(
      (haystack, needle) => _.search(str(haystack), str(needle)) === 0,
      {
        multiValued: true,
      }
    ),
    ...
    ends: $.extend(
      (haystack, needle) => {
        ...
        let i = _.search(haystack, needle);
    ...
    fromlast: $.extend(
      (haystack, needle) => _.between(haystack, needle, "", true),
      {
        multiValued: true,
      }
    ),
    ...
    between: $.extend(
      (haystack, from, to, tight) => {
        ...
        let i1 = from ? haystack[tight ? "lastIndexOf" : "indexOf"](from) : -1;
        let i2 = haystack[tight ? "indexOf" : "lastIndexOf"](to);
    ...
    phrase: $.extend(
      function (id, vars, lang) {
        ...
        let locale = lang
          ...
          : this?.[Mavo.mavo]?.locale ?? Mavo.Locale.default;
    ...
    filename: $.extend(
      (url) => new URL(str(url), Mavo.base).pathname.match(/[^/]+?$/)?.[0],
      {
        multiValued: true,
      }
    ),
    ...
    util: {
      numbers: function (array, args) {
        ...
        return array
          .filter(
            (number) =>
              !isNaN(number) && val(number) !== "" && val(number) !== null
          )
          .map((n) => +n);
      },
      ...
      postProcess: function (callback) {
        let multiValued = callback.multiValued;
        let newCallback;
        ...
        if (multiValued === true || multiValued?.length === 2) {
          newCallback = (...args) => {
            ...
            let idxA = multiValued[0] || 0;
            let idxB = multiValued[1] || 1;
          ...
          newCallback = function (array) {
            ...
            let ret = callback.call(this, ...arguments);
  ...
  Mavo.ready.then(() => {
    Object.getOwnPropertyNames(Mavo.Functions).forEach((property) => {
      let newCallback = $u.postProcess(Mavo.Functions[property]);
