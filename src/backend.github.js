    ...
    $.Class({
      ...
      put: function (serialized, path = this.path, o = {}) {
        ...
        let repoInfo = this.repoInfo
          ...
          : this.request("user/repos", { name: this.repo }, "POST").then(
              (repoInfo) => (this.repoInfo = repoInfo)
            );
              ...
              return this.request(
                ...
                .then((forkInfo) => {
                  ...
                  let test = (resolve, reject) => {
                    ...
                    this.request(
                      `repos/${forkInfo.full_name}/commits`,
                        ...
                        if (this.user.info.public_repos < repoInfo.forks) {
                          ...
                          return this.request(repoInfo.forks_url).then(
                            (forks) => {
                              for (let i in forks) {
                                if (
                                  forks[i].owner.login === this.user.username
                                ) {
      ...
      canPush: function () {
        ...
        return (
          this.user?.username?.toLowerCase() == this.username.toLowerCase()
        );
      },
      ...
      getURL: function (path = this.path, sha) {
        ...
        return repoInfo.pagesInfo
          ...
          .catch((xhr) => {
            ...
            return `https://cdn.jsdelivr.net/gh/${repo}@${
        ...
        this.notice = this.mavo.message(
          `
			...
			<form onsubmit="return false">
				<a href="${location.pathname}?${params}"><button>${this.mavo._(
            "gh-use-my-fork"
          )}</button></a>
			</form>`,
          {
      ...
      static: {
        ...
        test: function (url) {
          ...
          return /^((api\.)?github\.com|raw\.githubusercontent\.com)/.test(
            url.host
          );
        },
        ...
        parseURL: function (source, defaults = {}) {
          ...
          Object.defineProperties(ret, {
            apiCall: {
              get() {
                let call = `repos/${this.username}/${this.repo}/${
