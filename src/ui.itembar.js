(function ($, $$) {
  let _ = (Mavo.UI.Itembar = $.Class({
    constructor: function (item) {
      this.item = item;

      this.element = $$(
        `.mv-item-bar:not([mv-rel]), .mv-item-bar[mv-rel="${this.item.property}"]`,
        this.item.element
      ).filter((el) => {
      ...
      if (!this.element && this.item.template?.itembar) {
        ...
        let buttons = [
          ...
          },
          {
            ...
            type: "button",
            title: this.mavo._(
              `add-item-${bottomUp ? "after" : "before"}`,
              this.item
            ),
            className: "mv-add",
    ...
    handleEvent: function (evt) {
      let sticky = evt.type.indexOf("mouse") === -1;
    ...
    isWithinItem: function (element) {
      ...
      let itemBar = element.closest(".mv-item-bar");
      return itemBar
    ...
    add: function () {
      if (!this.element.parentNode && !Mavo.revocably.add(this.element)) {
        ...
        let tag = this.item.element.nodeName.toLowerCase();

        if (tag in _.container) {
          let rel = $(_.container[tag], this.item.element);
        }
