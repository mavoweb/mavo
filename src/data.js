      ...
      static: {
        ...
        stub: self.Proxy
          ? new Proxy(
              ...
              {
                get: (data, property) => {
                  let ret = Reflect.get(data, property);
                ...
                has: (data, property) => {
                  return (
                    Reflect.has(data, property) || typeof property === "string"
                  );
                },
              }
            )
        ...
        getProperty: function (data) {
          let ret = _.isItem(data) ? data[Mavo.parent] : data;
        ...
        find: function (property, data, o = {}) {
          ...
          let findDown = (prop) => {
            let ret = _.find(property, data[prop], o);
        ...
        resolve: function (property, data) {
          ...
          let ret;
          let propertyIsNumeric = !isNaN(property);
        ...
        has(property, data) {
          ...
          let objects = [data, Mavo.all, _.special];
        ...
        proxify(data) {
          ...
          return new Proxy(data, {
            ...
            set: function (data, property = "", value) {
              if (typeof property !== "symbol") {
                Mavo.warn(
                  `You cannot set data via expressions. Attempt to set ${property.toString()} to ${value} ignored.`
                );
                return value;
              }
        ...
        computeRoute(object, property, parent) {
          ...
          if (
            Mavo.isPlainObject(object) === Object.prototype ||
        ...
        special: {
          $index: function (obj) {
            let closestItem = _.closestItem(obj).value;

            if (!closestItem) {
              return -1;
            }

            let property = closestItem[Mavo.property];
          ...
          $all: function (obj) {
            let arr = _.closestArray(obj);
            let path = arr.path.reverse().slice(1); // Drop index
            let ret = arr.value.map((a) => $.value(a, ...path));
          ...
          $next: function (obj) {
            ...
            let index = arr.path[0];
            path = path.slice(1);
            let nextClosestItem = arr.value?.[index + 1];
          ...
          $previous: function (obj) {
            ...
            let index = arr.path[0];
            path = path.slice(1);
            let prevClosestItem = arr.value?.[index - 1];
