    ...
    static: {
      getControls: function (template) {
        ...
        if (template && (template = template.trim())) {
          ...
          let relative = /^with\s|\b(yes|no)-\w+\b/.test(template);
          ...
          if (relative) {
            return all.filter((id) => {
              let positive = ids.lastIndexOf(id);
              let negative = ids.lastIndexOf("no-" + id);
              let keep = positive > Math.max(-1, negative);
              let drop = negative > Math.max(-1, positive);
