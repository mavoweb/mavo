(function ($, $$) {
  ...
  let _ = (Mavo.Actions = {
    ...
    nope: () => {
      ...
      Mavo.warn(
        `Mavo actions (${actions}) can only be used in the mv-action attribute.`
      );
    },
    ...
    Functions: {
      ...
      add: function (data, ref, index) {
        if (arguments.length < 3) {
          if (arguments.length <= 1) {
            ...
            let collection = _.getCollection(ref);
      ...
      set: (ref, values) => {
        ...
        if (node) {
          ...
          if (!nodes.length) {
            Mavo.warn(
              `The first parameter of set() needs to be one or more existing properties, ${Mavo.safeToJSON(
                ref
              )} is not.`
            );
          } else {
  ...
  for (let name in _.Functions) {
    ...
    if (!(nameif in _.Functions)) {
      _.Functions[nameif] = (condition, target, ...rest) => {
        ...
        return Mavo.value(condition)
          ? _.Functions[name](target, ...rest)
          : null;
      };
    }
  }
