(function ($, $$) {
  let _ = (Mavo.Node = class Node {
    constructor(element, mavo, options = {}) {
      ...
      if (this instanceof Mavo.Group || this.collection) {
        ...
        let et = Mavo.DOMExpression.search(this.element).filter(
          (et) => et.originalAttribute == "mv-value"
        )[0];
        ...
        if (ret !== false) {
          for (let i in obj.children) {
            ...
            if (node instanceof Mavo.Node) {
              let ret = walker.call(node, node, [...path, i]);
    ...
    render(data, o = {}) {
      ...
      let env = { context: this, data, options: o };
              ...
              this.children.main instanceof Mavo.Collection
                ...
                : this.getNames((p, n) => {
                    return (
                      ...
                      !n.expressions?.[0]?.isDynamicObject
                    );
                  })[0];
      ...
      let changed = this.dataRender(env.data, o);
    ...
    pathFrom(node) {
      ...
      for (let i = 0; i < path.length && nodePath[i] == path[i]; i++) {}
    ...
    getCousin(offset, o = {}) {
      ...
      for (let i = 0; i < collection.length; i++) {
        ...
        let item = collection.children[ind];
      ...
      let relativePath = this.pathFrom(this.closestItem);
    ...
    static create(element, mavo, o = {}) {
      ...
      return new Mavo[Mavo.is("group", element) ? "Group" : "Primitive"](
        element,
        mavo,
        o
      );
    ...
    static getProperty(element) {
      let property =
        element.getAttribute("property") || element.getAttribute("itemprop");

      if (!property) {
        let multiple = element.getAttribute("mv-multiple");

        if (element.hasAttribute("property")) {
          // property used without a value
          property =
            multiple || element.name || element.id || element.classList[0];

          if (!property) {
            property = _.generatePropertyName(
              multiple === null ? "prop" : "collection",
              element
            );
          }
        }
    ...
    static get(element, prioritizePrimitive) {
      let nodes = (_.elements.get(element) || []).filter(
  ...
  $.Class(_, {
    ...
    lazy: {
      ...
      inPath: function () {
        let attribute =
          this instanceof Mavo.Collection ? "mv-multiple-path" : "mv-path";
