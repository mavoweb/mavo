(function ($, $$) {
  ...
  let _ = (Mavo.Expressions = $.Class({
    ...
    updateThrottled: function (evt) {
      ...
      let scheduled = (this.scheduled[evt.action] =
    ...
    update: function (evt) {
      ...
      if (evt instanceof Mavo.Node) {
        ...
        if (evt.action == "propertychange") {
          ...
          let collection = evt.node.collection || evt.node;
    ...
    updateById: function (property, evt, cache) {
      ...
      let exprs = this.identifiers[property];
        ...
        if (!node.matches("script:not([mv-expressions])")) {
          $$(node.childNodes).forEach((child) => {
            ...
            if (child.nodeType == 1 || child.nodeType == 3) {
              let segment = offset > 0 ? `${index}.${offset}` : index;
