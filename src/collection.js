(function ($, $$) {
  ...
  let _ = (Mavo.Collection = class Collection extends (
    Mavo.Node
  ) {
    ...
    createAddButton() {
      ...
      let selector = `button[class~="mv-add-${this.property}"]`;
      let group = this.parentGroup.element;

      let button = $$(selector, group).filter((button) => {
        return (
    ...
    init() {
      if (
        ...
        this.initialItems = +(
          this.templateElement.getAttribute("mv-initial-items") ||
          (this.like ? 0 : 1)
        );
      ...
      if (this.likeNode) {
        ...
        let templateElement =
          this.likeNode.templateElement ||
          this.likeNode.collection?.templateElement ||
          this.likeNode.element;
    ...
    createItem(element) {
      ...
      let template = this.itemTemplate || this.template?.itemTemplate || null;

      let item = Mavo.Node.create(element, this.mavo, {
        collection: this,
        template,
    ...
    add(item, index, o = {}) {
      ...
      let rel = this.children?.[index]?.element ?? this.marker;
      $.before(item.element, rel);

      let env = { context: this, item };
      ...
      if (this.mavo.expressions.active && !o.silent) {
        requestAnimationFrame(() => {
          env.changed.forEach((i) => {
            i.dataChanged(
              i == env.item && env.previousIndex === undefined ? "add" : "move"
            );
    ...
    getDragula() {
      ...
      if (this.template) {
        Mavo.pushUnique(
          this.template.getDragula().containers,
          this.marker.parentNode
        );
        return (this.dragula =
          this.template.dragula || this.template.getDragula());
      }
      ...
      this.dragula = dragula({
        ...
        accepts: function (el, target, source, next) {
          ...
          let previous =
            next?.previousElementSibling ?? target.lastElementChild;

          let collection = _.get(previous) || _.get(next);
    ...
    static get(element) {
      ...
      let collection = Mavo.data(element, "collection");
      ...
      let item = Mavo.Node.get(element);
