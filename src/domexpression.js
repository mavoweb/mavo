(function ($, $$) {
  let _ = (Mavo.DOMExpression = $.Class({
    constructor: function (o = {}) {
      ...
      for (let prop of [
        ...
        this[prop] =
          o[prop] === undefined && this.template
            ? this.template[prop]
            : o[prop];
      }
      ...
      if (this.attribute == "mv-value") {
        ...
        this.fallback =
          this.fallback ||
          Mavo.Primitive.getValue(this.element, { attribute: this.attribute });
      ...
      if (!this.expression) {
        ...
        if (this.attribute) {
          ...
          let value = Element.prototype.getAttribute.call(
            this.node,
            this.attribute
          );
          ...
          if (
            ...
            this.node?.firstChild?.nodeType === 3
          ) {
            let whitespace = this.node.firstChild.textContent.match(
              /^\s*|\s*$/g
            );
      ...
      this.oldValue = this.value = this.parsed.map((x) =>
        x instanceof Mavo.Expression ? "" : x
      );

      // Cache identifiers
      this.identifiers =
        this.identifiers || this.parsed.flatMap((x) => x.identifiers || []);
    ...
    update: function () {
      ...
      env.value = this.value = this.parsed.map((expr, i) => {
        if (expr instanceof Mavo.Expression) {
          let env = { context: this, expr, parentEnv };
          ...
          if (env.value instanceof Error) {
            return this.fallback !== undefined
              ? this.fallback
              : this.syntax.start + env.expr.expression + this.syntax.end;
          }
    ...
    static: {
      ...
      search: function (element, attribute) {
        ...
        let all = _.elements.get(element) || [];
      ...
      special: {
        add: function (domexpression, name) {
          if (name) {
            let o = this.vars[name];
            let hasName = domexpression.identifiers.indexOf(name) > -1;
            let hasUnprefixedName =
              name.startsWith("$") &&
