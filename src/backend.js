(function ($, $$) {
  ...
  let _ = (Mavo.Backend = $.Class({
    ...
    async store(data, { path, format = this.format } = {}) {
      ...
      let serialized =
        typeof data === "string" ? data : await format.stringify(data);
    ...
    equals: function (backend) {
      return (
        backend === this ||
        (backend && this.id == backend.id && this.source == backend.source)
      );
    },
      ...
      return this.ready.then(() => {
        ...
        return new Promise((resolve, reject) => {
          ...
          if (passive) {
            ...
            this.authPopup = open(
              `${this.constructor.oAuth}?client_id=${
                this.key
            ...
            addEventListener("message", (evt) => {
              if (evt.source === this.authPopup) {
                ...
                for (let appid in Mavo.all) {
                  let storage = Mavo.all[appid].primaryBackend;
