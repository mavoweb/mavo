  ...
  let _ = (Mavo.Script = {
    $fn: self.Proxy
      ? new Proxy(
          ...
          {
            get: (data, property) => {
              let propertyL = property.toLowerCase(),
                ret;
    ...
    binaryOperation: function (a, b, o = {}) {
      ...
      if (Array.isArray(b)) {
        if (Array.isArray(a)) {
          ...
          let leftUnary = o.leftUnary || o.unary;
          let rightUnary = o.rightUnary || o.unary;
          let leftDefault =
            o.leftDefault === undefined ? o.default : o.leftDefault;
          let rightDefault =
            o.rightDefault === undefined ? o.default : o.rightDefault;
    ...
    addBinaryOperator: function (name, o) {
      ...
      return (
        ...
        function (...operands) {
          ...
          let prev = o.comparison ? true : operands[0],
            result;
          ...
          for (let i = 1; i < operands.length; i++) {
            ...
            let result = _.binaryOperation(a, b, o);
    ...
    operators: {
          ...
          haystacks.map((b) => {
            if (Array.isArray(b)) {
              let op = (a) => {
                ...
                let fn = $.type(val(a)) === "object" ? Mavo.safeToJSON : val;
            ...
            let result = Mavo.Script.unaryOperation(needle, op);
      ...
      groupby: {
        ...
        code: (array, key) => {
          ...
          let property = key[Mavo.as] || key[0]?.[Mavo.toNode]?.property;
          let groups = new Mavo.BucketMap({ arrays: true });
          let ret = [];
          ...
          groups.forEach((items, value) => {
            ...
            if (Mavo.in(Mavo.route, array)) {
              items[Mavo.route] = obj[Mavo.route] = Object.assign(
                {},
                array[Mavo.route]
              );
              obj[Mavo.route] = $.each(
      ...
      as: {
        ...
        code: (property, name) => {
          if (
            ...
            let ret = property.slice();

            if (
              !Array.isArray(name) &&
              name?.[Mavo.toNode]?.property !== undefined
            ) {
    ...
    serializers: {
      BinaryExpression: (node) =>
        `${_.serialize(node.left, node)} ${node.operator} ${_.serialize(
          node.right,
          node
        )}`,
      UnaryExpression: (node) =>
        `${node.operator}${_.serialize(node.argument, node)}`,
      CallExpression: (node) => {
        ...
        let nameSerialized = _.serialize(node.callee, node);
        let argsSerialized = node.arguments.map((n) => _.serialize(n, node));
        return `${nameSerialized}(${argsSerialized.join(", ")})`;
      },
      ConditionalExpression: (node) =>
        `${_.serialize(node.test, node)}? ${_.serialize(
          node.consequent,
          node
        )} : ${_.serialize(node.alternate, node)}`,
      MemberExpression: (node, parent) => {
        ...
        if (n) {
          ...
          let property = node.computed
            ? `[${_.serialize(node.property, node)}]`
            : `.${node.property.name}`;
      ...
      CallExpression: (node) => {
        if (node.callee.type == "Identifier") {
          if (node.callee.name == "if") {
            ...
            let condition = node.arguments[0];
    ...
    serialize: (node, parent) => {
      ...
      let ret = _.transformations[node.type]?.(node, parent);
...
Mavo.Actions.running = Mavo.Actions._running;`;
    ...
    serializeScopeCall: (args) => {
      let withCode = `with (Mavo.Script.subScope(scope, $this) || {}) { return (${_.serialize(
        args[1]
      )}); }`;
      return `(function() {
    ...
    subScope: (proxy, $this) => {
      let unscopables = Object.keys($this).reduce(
        (o, k) => {
          o[k] = true;
          return o;
        },
