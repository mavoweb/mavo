...
(async function ($, $$) {
  ...
  let _ = (self.Mavo = $.Class({
    constructor: function (element) {
      ...
      this.id =
        Mavo.getAttribute(this.element, "mv-app", "id") || `mavo${this.index}`;

      if (this.id in _.all) {
        // Duplicate app name
        for (let i = 2; this.id + i in _.all; i++) {}
        this.id = this.id + i;
      }
      ...
      this.autoSaveDelay =
        (this.element.getAttribute("mv-autosave") || 0) * 1000;
      ...
      let backendTypes = ["source", "storage", "init", "uploads"]; // order is significant!
        ...
        if (attribute.indexOf("mv-") === 0) {
          ...
          if (backendTypes.includes(role)) {
            ...
            if (
              role === "source" ||
              (!this.source &&
                (role === "storage" || (role === "init" && !this.root.data)))
            ) {
              this.load();
            }
          }
      ...
      this.element.addEventListener("keydown", (evt) => {
        ...
        if (
          ...
          if (node?.closestCollection) {
            let nextNode = node.getCousin(evt.key === "ArrowUp" ? -1 : 1, {
              wrap: true,
            });
      ...
      $.bind(
        ...
        (evt) => {
          if (evt.target.matches(_.selectors.multiple)) {
            ...
            let parent = evt.target.parentNode.closest(_.selectors.multiple);
    ...
    updateBackend(role) {
      ...
      if (backend) {
        ...
        let options = Object.fromEntries(
          roleAttributes.map((a) => [a.name.replace(prefix, ""), a.value])
        );
      ...
      changed =
        changed || (backend ? !backend.equals(existing) : Boolean(existing));
      ...
      if (changed) {
        ...
        let permissions = this.storage
          ? this.storage.permissions
          : new Mavo.Permissions({ edit: true, save: false });
      ...
      try {
        ...
        if (err) {
          ...
          if (err instanceof XMLHttpRequest) {
            message +=
              ": " +
              (err.status ? this._("http-error", err) : this._("cant-connect"));
          }
    ...
    setDeleted: function (...nodes) {
      ...
      let notice = (this.deletionNotice = this.message(
        [
