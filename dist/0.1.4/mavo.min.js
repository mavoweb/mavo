...
}), location.hash&&this.element.addEventListener("mavo:load.mavo", () => {
var p=() => {
var u=document.getElementById(location.hash.slice(1));return (u||!location.hash)&&(this.element.contains(u)&&requestAnimationFrame(() => {
Mavo.scrollIntoViewIfNeeded(u);
}), q&&(q.destroy(), q=null)), u;
};if (!p()) {
var q=new Mavo.Observer(this.element, "id", p, {subtree:!0});
}
...
}, UI:{}, hooks:new b.Hooks, attributes:["mv-app", "mv-storage", "mv-source", "mv-init", "mv-path", "mv-multiple-path", "mv-format", "mv-attribute", "mv-default", "mv-mode", "mv-edit", "mv-permisssions", "mv-rel"], lazy:{locale:() => document.documentElement.lang||"en-GB"}}});Object.defineProperty(d.all, "length", {get:function() {
...
var e=[];b.each({blissfuljs:Array.from&&document.documentElement.closest&&self.URL&&"searchParams"in URL.prototype, "Intl.~locale.en":self.Intl, IntersectionObserver:self.IntersectionObserver, Symbol:self.Symbol}, (g, h) => {
h||e.push(g);
});var f="https://cdn.polyfill.io/v2/polyfill.min.js?unknown=polyfill&features="+e.map(g => g+"|gated").join(",");d.dependencies.push(b.ready(), d.Plugins.load(), b.include(!e.length, f)), d.inited=b.ready().then(() => {
...
if (h) {
var l=getComputedStyle(h).getPropertyValue(k);if (l) {
return l.trim();
}
}
}, data:function(h, k, l) {
...
if (k.length) {
var m=k[k.length-1], n=b.value(h, ...k.slice(0, -1));return Array.isArray(n)&&Array.isArray(l)?n.splice(m, 1, ...l):n[k[k.length-1]]=l, h;
} return l;
} return "object"==typeof h&&k&&k.length?k.reduce((q, t, u) => {
if (q&&t in q) {
return q[t];
...
if (this.mavo=d, this.element=a(".mv-bar", this.mavo.element), this.template=this.mavo.element.getAttribute("mv-bar"), this.element) {
this.custom=!0, this.template=this.element.getAttribute("mv-bar")||this.template||"";var e=Object.keys(c.controls).map(g => `.mv-${g}`).join(", ");for (let g in this.customControls=b(e, this.element), c.controls) {
this[g]=a(`.mv-${g}`, this.element), this[g]&&(this.template+=` yes-${g}`);
}
}
else {
this.element=a.create({className:"mv-bar mv-ui", start:this.mavo.element, innerHTML:"<button>&nbsp;</button>"});
} this.element.classList.contains("mv-compact")&&(this.noResize=!0), this.controls=c.getControls(this.mavo.element.getAttribute("mv-bar")||this.element.getAttribute("mv-bar")), this.controls.length&&(this.targetHeight=this.element.offsetHeight), this.custom||(this.element.innerHTML="");for (let g of this.controls) {
let h=c.controls[g];for (var f in this[g]?this[g].remove():h.create?this[g]=h.create.call(this.mavo):this[g]=a.create("button", {className:`mv-${g}`, textContent:this.mavo._(g)}), this.add(g), h.permission?this.permissions.can(h.permission, () => {
this.toggle(g, !h.condition||h.condition.call(this.mavo));
}, () => {
this.remove(g);
}):h.condition&&!h.condition.call(this.mavo)&&this.remove(g), h.events) {
a.events(this[g], f, h.events[f].bind(this.mavo));
}
} for (let g in c.controls) {
let h=c.controls[g];h.action&&a.delegate(this.mavo.element, "click", ".mv-"+g, i => {
(!h.permission||this.permissions.is(h.permission))&&(h.action.call(this.mavo), i.preventDefault());
});
...
var e=Object.keys(c.controls).filter(m => !c.controls[m].optional);if (d) {
var f="none"==d?[]:d.trim().split(/\s+/), g=!0, h={};for (var i of f) {
let m=Mavo.match(i, /([a-z]+)\s*$/i, 1);m in c.controls&&(h[m]=Mavo.match(i, /^(no|yes)\-/i, 1));
} for (var j in h) {
if (!h[j]) {
g=!1;break;
}
} var k=g?e:[];for (var j in h) {
var l=h[j];"no"!=l&&l||Mavo.delete(k, j), -1===k.indexOf(j)&&"no"!=l&&k.push(j);
} return k;
} return e;
...
var f=this.find(d);return void 0===f&&(f=this.walkUp(g => {
return g.property==d?g:d in g.children?g.children[d]:void 0;
})), void 0===f&&(f=this.mavo.root.find(d)), f;
...
} var k=this.resolve(j);return void 0===k?j in Mavo.all&&Mavo.all[j].root&&(g[j]=Mavo.all[j].root.getData(f)):(Array.isArray(k)?k=k.map(l => l.getData(f)).filter(l => null!==l):k instanceof Mavo.Node&&(k=k.getData(f)), g[j]=k, !0);
}, set:function(h, j="", k) {
...
var d="Collection"==this.nodeType?"mv-multiple-path":"mv-path";return (this.element.getAttribute(d)||"").split("/").filter(f => f.length);
}, properties:function() {
if (this.template) {
...
if (d.immediately) {
return f();
} var h="click focus dragover dragenter".split(" ").map(j => j+".mavo:preedit").join(" ");a.events(this.element, h, f);
...
} if ("number"===a.type(d)||"number"==f.datatype) {
var j=f.attribute||f.element&&f.element.matches("style, pre");if (!j) {
return c.formatNumber(d);
}
} return Array.isArray(d)?d.map(c.format).join(", "):"object"===a.type(d)?Mavo.toJSON(d):d;
}, lazy:{formatNumber:() => {
...
}}, "video, audio":{attribute:["autoplay", "buffered", "loop"], datatype:"boolean"}, details:{attribute:"open", datatype:"boolean"}, "a, link":{default:!0, attribute:"href"}, "input, select, button, textarea":{attribute:"disabled", datatype:"boolean"}, formControl:{selector:"select, input", default:!0, attribute:"value", modes:"edit", changeEvents:"input change", edit:() => {}, done:() => {}, init:function() {
...
Mavo.revocably.setAttribute(this.element, "role", "checkbox"), a.events(this.element, "click.mavo:edit keyup.mavo:edit keydown.mavo:edit", d => {
("click"==d.type||" "==d.key||"Enter"==d.key)&&("keydown"!=d.type&&(this.value=!this.value), d.preventDefault(), d.stopPropagation());
});
}, done:function() {
...
} return h.options.live&&Array.isArray(h.data)&&(h.data[Mavo.toNode]=this, h.data=this.relativizeData(h.data)), h.options.live||(h.data=Mavo.subset(this.data, this.inPath, h.data)), Mavo.hooks.run("node-getdata-end", h), h.data;
...
return this.function||(this.function=Mavo.Script.compile(this.expression)), this.function(c);
}
catch (d) {
return console.info("%cExpression error!", "color: red; font-weight: bold", `${d.message} in expression ${this.expression}`, `
Not an expression? Use mv-expressions="none" to disable expressions on an element and its descendants.`), Mavo.hooks.run("expression-eval-error", {context:this, exception:d}), d;
}
}, toString() {
return this.expression;
}, changedBy:function(c) {
return b.changedBy(this.identifiers, c);
}, live:{expression:function(c) {
this.function=null, this.identifiers=c.match(/[$a-z][$\w]*/ig)||[];
}}, static:{changedBy:function(c, d) {
if (!d) {
return !0;
} if (!c) {
return !1;
} if (-1<c.indexOf(d.property)) {
return !0;
} if (Mavo.Functions.intersects(d.properties, c)) {
return !0;
} if ("propertychange"!=d.action) {
if (Mavo.Functions.intersects(["$index", "$previous", "$next"], c)) {
return !0;
} var e=d.node.collection||d.node;if (Mavo.Functions.intersects(e.properties, c)) {
return !0;
}
} return !1;
}}});b.Syntax=a.Class({constructor:function(c, d) {
...
return this.identifiers||(this.identifiers=Mavo.flatten(this.parsed.map(d => d.identifiers||[])), b.special.add(this)), Mavo.Expression.changedBy(this.identifiers, c);
...
});g&&(e.value=1===e.value.length?e.value[0]:e.value.map(h => Mavo.Primitive.format(h, {attribute:this.attribute, element:this.element})).join(""), this.output(e.value), Mavo.hooks.run("domexpression-update-end", e));
}, output:function(c) {
...
}, special:{add:function(c, d) {
if (d) {
var e=this.vars[d];e&&-1<c.identifiers.indexOf(d)&&(e.all=e.all||new Set, e.all.add(c), 1===e.all.size?e.observe():!e.all.size&&e.unobserve());
}
else {
for (var d in this.vars) {
this.add(c, d);
}
}
}, delete:function(c, d) {
if (d) {
var e=this.vars[d];e.all=e.all||new Set, e.all.delete(c), e.all.size||e.unobserve();
}
else {
for (var d in this.vars) {
this.delete(c, d);
...
this.update&&this.update(...arguments);for (var c of this.all) {
c.update();
}
}, event:function(c, {type:d, update:e, target:f=document}={}) {
this.vars[c]={observe:function() {
this.callback=this.callback||b.special.update.bind(this), f.addEventListener(d, this.callback);
}, unobserve:function() {
f.removeEventListener(d, this.callback);
}}, e&&(this.vars[c].update=function(g) {
Mavo.Functions[c]=e(g);
});
}, vars:{$now:{observe:function() {
this.timer=setInterval(b.special.update.bind(this), 100);
}, unobserve:function() {
clearInterval(this.timer);
}}}}}});b.special.event("$mouse", {type:"mousemove", update:function(c) {
...
if (s=o.date(s), !s) {
return "";
} var t=q[r](s);return t=new self["year"==r?"String":"Number"](t), ("month"==r||"weekday"==r)&&(t.name=k(s, {[r]:"long"}), t.shortname=k(s, {[r]:"short"})), "weekday"!=r&&(t.twodigit=(10>t%100?"0":"")+t%100), t;
};
} var m=Mavo.Functions={operators:{"=":"eq"}, get:function(r, s) {
s=f(s);var t=Mavo.getCanonicalProperty(r, s);if (t!==void 0) {
var u=r[t];return "function"==typeof u&&0!==u.name.indexOf("bound")?u.bind(r):u;
} if (Array.isArray(r)&&isNaN(s)) {
for (var w of r) {
...
r=g(r).replace(/[^\w-:]/g);var t=s.search.match(RegExp(`[?&]${r}(?:=(.+?))?(?=$|&)`))||s.pathname.match(RegExp(`(?:^|\\/)${r}\\/([^\\/]*)`));
} return null!==t&&r?t[1]||"":null;
}, first:r => r&&r[0]||"", last:r => r&&r[r.length-1]||"", unique:function(r) {
return Array.isArray(r)?[...new Set(r.map(f))]:r;
}, intersects:function(r, s) {
if (r&&s) {
var t=new Set(s.map?s.map(f):s);return r=r.map?r.map(f):[...r], !r.every(u => !t.has(u));
}
}, sum:function(r) {
return o.numbers(r, arguments).reduce((s, t) => {
return +s+(+t||0);
}, 0);
}, average:function(r) {
return r=o.numbers(r, arguments), r.length&&m.sum(r)/r.length;
}, min:function(r) {
return Math.min(...o.numbers(r, arguments));
}, max:function(r) {
return Math.max(...o.numbers(r, arguments));
}, count:function(r) {
return Mavo.toArray(r).filter(s => !h(s)).length;
}, round:function(r, s) {
return j(r)||j(s)||!isFinite(r)?Math.round(r):+r.toLocaleString("en-US", {useGrouping:!1, maximumFractionDigits:s});
}, th:function(r) {
if (h(r)) {
return "";
} if (10>s||20<s) {
var s=["th", "st", "nd", "th"][r%10];
} return s=s||"th", r+s;
}, iff:function(r, s=r, t="") {
return Array.isArray(r)?r.map((u, w) => {
var x=f(u)?s:t;return Array.isArray(x)?x[Math.min(w, x.length-1)]:x;
}):f(r)?s:t;
}, replace:function(r, s, t="", u=1) {
if (Array.isArray(r)) {
return r.map(A => m.replace(A, s, t));
} for (var y, w=RegExp(Mavo.escapeRegExp(s), "g"), x=r, z=0;x!=y&&z++<u;) {
y=x, x=x.replace(w, t);
} return x;
}, len:r => g(r).length, search:(r, s) => r&&s?g(r).toLowerCase().indexOf((s+"").toLowerCase()):-1, starts:(r, s) => 0===m.search(g(r), g(s)), ends:function(r, s) {
[r, s]=[g(r), g(s)];var t=m.search(r, s);return -1<t&&t===r.length-s.length;
}, join:function(r, s) {
return Mavo.toArray(r).join(g(s));
}, idify:function(r) {
return g(r).replace(/\s+/g, "-").replace(/[^\w-]/g, "").toLowerCase();
}, readable:function(r) {
return g(r).replace(/([a-z])([A-Z])(?=[a-z])/g, (s, t, u) => t+" "+u.toLowerCase()).replace(/([a-z0-9])[_\/-](?=[a-z0-9])/g, "$1 ").replace(/^[a-z]/, s => s.toUpperCase());
}, uppercase:r => g(r).toUpperCase(), lowercase:r => g(r).toLowerCase(), from:(r, s) => m.between(r, s), fromlast:(r, s) => m.between(r, s, "", !0), to:(r, s) => m.between(r, "", s), tofirst:(r, s) => m.between(r, "", s, !0), between:(r, s, t, u) => {
[r, t, t]=[g(r), g(s), g(t)];var w=s?r[u?"lastIndexOf":"indexOf"](s):-1, x=r[u?"indexOf":"lastIndexOf"](t);return r.slice(w+1, -1!==x&&t?x:r.length);
}, filename:r => Mavo.match(new URL(g(r), Mavo.base).pathname, /[^/]+?$/), json:r => Mavo.safeToJSON(r), get $now() {
return new Date;
}, get $today() {
return m.date(new Date);
}, year:l("year"), month:l("month"), day:l("day"), weekday:l("weekday"), hour:l("hour"), minute:l("minute"), second:l("second"), ms:l("ms"), date:r => {
return `${m.year(r)}-${m.month(r).twodigit}-${m.day(r).twodigit}`;
}, time:r => {
return `${m.hour(r).twodigit}:${m.minute(r).twodigit}:${m.second(r).twodigit}`;
}, minutes:r => Math.floor(Math.abs(r)/60), hours:r => Math.floor(Math.abs(r)/3600), days:r => Math.floor(Math.abs(r)/86400), weeks:r => Math.floor(Math.abs(r)/604800), months:r => Math.floor(Math.abs(r)/(86400*30.4368)), years:r => Math.floor(Math.abs(r)/(12*(86400*30.4368))), localTimezone:-new Date().getTimezoneOffset(), log:(...r) => {
return console.log(...r), r[0];
}, $mouse:{x:0, y:0}, get $hash() {
return location.hash.slice(1);
}, util:{numbers:function(r, s) {
return r=Array.isArray(r)?r:s?$$(s):[r], r.filter(t => !isNaN(t)&&""!==f(t)).map(t => +t);
}, date:function(r) {
if (r=f(r), !r) {
return null;
} if ("string"===b.type(r)) {
r=r.trim(), /^\d{4}-\d{2}-\d{2}/.test(r)||(r=m.$today+" "+r), -1===r.indexOf(":")?r+="T00:00:00":r=r.replace(/\-(\d{2})\s+(?=\d{2}:)/, "-$1T"), r=r.replace(/\s+/g, "");var s=Mavo.match(r, /[+-]\d{2}:?\d{2}|Z$/);if (s) {
r=new Date(r);
}
else {
var t=r.match(/\d+/g);r=new Date(t[0], (t[1]||1)-1, t[2]||1, t[3]||0, t[4]||0, t[5]||0, t[6]||0);
}
}
else {
r=new Date(r);
...
var r={}, s=new URL(location);for (let t of s.searchParams) {
r[t[0]]=t[1];
} return Object.defineProperties(r, {path:{value:s.pathname.split("/").filter(t => !!t)}, toString:{value:() => new URL(location)}}), r;
}), m._Trap=self.Proxy?new Proxy(m, {get:(r, s) => {
var t, u=Mavo.getCanonicalProperty(r, s)||Mavo.getCanonicalProperty(Math, s);return u&&(t=r[u], void 0==t&&(t=Math[u])), void 0==t?s in self?self[s]:s:("function"==typeof t&&(t.toString=() => s), t);
}, has:(r, s) => "data"!=s}):m;var p=new Intl.NumberFormat("en", {minimumIntegerDigits:"2"});p=p.format.bind(p);var q={year:r => r.getFullYear(), month:r => r.getMonth()+1, day:r => r.getDate(), weekday:r => r.getDay()||7, hour:r => r.getHours(), minute:r => r.getMinutes(), second:r => r.getSeconds(), ms:r => r.getMilliseconds()};
})(Bliss, Mavo.value);
(function(c, d, f) {
var g=Mavo.Script={addUnaryOperator:function(j, k) {
if (k.symbol) {
for (let l of Mavo.toArray(k.symbol)) {
Mavo.Script.unarySymbols[l]=j, jsep.addUnaryOp(l);
}
} return Mavo.Functions[j]=l => Array.isArray(l)?l.map(d).map(k.scalar):k.scalar(d(l));
}, addBinaryOperator:function(j, k) {
if (k.symbol) {
for (let l of Mavo.toArray(k.symbol)) {
Mavo.Script.symbols[l]=j, k.precedence&&jsep.addBinaryOp(l, k.precedence);
}
} return k.identity=void 0===k.identity?0:k.identity, Mavo.Functions[j]=k.code||function(...l) {
1===l.length&&Array.isArray(l[0])&&(l=[...l[0]]), k.raw||(l=l.map(d));var q, m=k.logical?k.identity:l[0];for (let s=1;s<l.length;s++) {
let t=k.logical?l[s-1]:m, u=l[s];Array.isArray(u)?("number"==typeof k.identity&&(u=f.numbers(u)), q=Array.isArray(t)?[...u.map((v, w) => k.scalar(void 0===t[w]?k.identity:t[w], v)), ...t.slice(u.length)]:u.map(v => k.scalar(t, v))):Array.isArray(t)?q=t.map(v => k.scalar(v, u)):q=k.scalar(t, u), m=k.reduce?k.reduce(m, q, t, u):k.logical?m&&q:q;
} return m;
};
}, symbols:{}, unarySymbols:{}, getOperatorName:(j, k) => Mavo.Script[k?"unarySymbols":"symbols"][j]||j, operators:{not:{symbol:"!", scalar:j => !j}, multiply:{scalar:(j, k) => j*k, identity:1, symbol:"*"}, divide:{scalar:(j, k) => j/k, identity:1, symbol:"/"}, add:{scalar:(j, k) => +j+ +k, symbol:"+"}, plus:{scalar:j => +j, symbol:"+"}, subtract:{scalar:(j, k) => {
if (isNaN(j)||isNaN(k)) {
var l=f.date(j), m=f.date(k);if (l&&m) {
return (l-m)/1e3;
}
} return j-k;
}, symbol:"-"}, minus:{scalar:j => -j, symbol:"-"}, mod:{scalar:(j, k) => {
var l=j%k;return l+=0>l?k:0, l;
}, symbol:"mod", precedence:6}, lte:{logical:!0, scalar:(j, k) => {
return [j, k]=Mavo.Script.getNumericalOperands(j, k), j<=k;
}, identity:!0, symbol:"<="}, lt:{logical:!0, scalar:(j, k) => {
return [j, k]=Mavo.Script.getNumericalOperands(j, k), j<k;
}, identity:!0, symbol:"<"}, gte:{logical:!0, scalar:(j, k) => {
return [j, k]=Mavo.Script.getNumericalOperands(j, k), j>=k;
}, identity:!0, symbol:">="}, gt:{logical:!0, scalar:(j, k) => {
return [j, k]=Mavo.Script.getNumericalOperands(j, k), j>k;
}, identity:!0, symbol:">"}, eq:{logical:!0, scalar:(j, k) => j==k, symbol:["=", "=="], identity:!0, precedence:6}, neq:{logical:!0, scalar:(j, k) => j!=k, symbol:["!="], identity:!0}, and:{logical:!0, scalar:(j, k) => !!j&&!!k, identity:!0, symbol:["&&", "and"], precedence:2}, or:{logical:!0, scalar:(j, k) => j||k, reduce:(j, k) => j||k, identity:!1, symbol:["||", "or"], precedence:2}, concatenate:{symbol:"&", identity:"", scalar:(j, k) => ""+(j||"")+(k||""), precedence:10}, filter:{scalar:(j, k) => d(k)?j:null, raw:!0}}, getNumericalOperands:function(j, k) {
if (isNaN(j)||isNaN(k)) {
var l=f.date(j), m=f.date(k);if (l&&m) {
return [l, m];
}
} return [j, k];
}, serializers:{BinaryExpression:j => `${g.serialize(j.left)} ${j.operator} ${g.serialize(j.right)}`, UnaryExpression:j => `${j.operator}${g.serialize(j.argument)}`, CallExpression:j => `${g.serialize(j.callee)}(${j.arguments.map(g.serialize).join(", ")})`, ConditionalExpression:j => `${g.serialize(j.test)}? ${g.serialize(j.consequent)} : ${g.serialize(j.alternate)}`, MemberExpression:j => {
var k=j.computed?g.serialize(j.property):`"${j.property.name}"`;return `get(${g.serialize(j.object)}, ${k})`;
}, ArrayExpression:j => `[${j.elements.map(g.serialize).join(", ")}]`, Literal:j => j.raw, Identifier:j => j.name, ThisExpression:() => "this", Compound:j => j.body.map(g.serialize).join(" ")}, transformations:{BinaryExpression:j => {
let k=Mavo.Script.getOperatorName(j.operator);var l=j, m=[];do {
m.unshift(l.right), l=l.left;
} while (Mavo.Script.getOperatorName(l.operator)===k);if (m.unshift(l), 1<m.length) {
return `${k}(${m.map(g.serialize).join(", ")})`;
}
}, UnaryExpression:j => {
var k=Mavo.Script.getOperatorName(j.operator, !0);if (k) {
return `${k}(${g.serialize(j.argument)})`;
}
}, CallExpression:j => {
"Identifier"==j.callee.type&&("if"==j.callee.name&&(j.callee.name="iff"), j.callee.name="Mavo.Functions._Trap."+j.callee.name);
}}, serialize:j => {
var k=g.transformations[j.type]&&g.transformations[j.type](j);return void 0===k?g.serializers[j.type](j):k;
}, rewrite:function(j) {
try {
return g.serialize(g.parse(j));
}
catch (k) {
return j;
}
}, compile:function(j) {
return j=g.rewrite(j), new Function("data", `with(Mavo.Functions._Trap)
				with (data || {}) {
					return ${j};
				}`);
}, parse:self.jsep};for (let j in g.serializers.LogicalExpression=g.serializers.BinaryExpression, g.transformations.LogicalExpression=g.transformations.BinaryExpression, Mavo.Script.operators) {
let k=Mavo.Script.operators[j];2>k.scalar.length?Mavo.Script.addUnaryOperator(j, k):Mavo.Script.addBinaryOperator(j, k);
} var h={average:"avg", iff:"iff IF", multiply:"mult product", divide:"div", lt:"smaller", gt:"larger bigger", eq:"equal equality", th:"ordinal"};for (let j in h) {
h[j].split(/\s+/g).forEach(k => Mavo.Functions[k]=Mavo.Functions[j]);
}
