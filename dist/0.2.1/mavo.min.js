...
if (this.treeBuilt=Mavo.promise(), this.dataLoaded=Mavo.promise(), this.deleted=[], this.element=element, this.inProgress=!1, this.index=Object.keys(_.all).length+1, Object.defineProperty(_.all, this.index-1, {value:this, configurable:!0}), Mavo.attributeStartsWith("data-mv-", this.element).forEach(attribute => {
...
}), this.id=Mavo.getAttribute(this.element, "mv-app", "id")||`mavo${this.index}`, this.id in _.all) {
...
}), this.expressions=new Mavo.Expressions(this), Mavo.hooks.run("init-tree-before", this), this.root=new Mavo.Group(this.element, this), this.treeBuilt.resolve(), Mavo.hooks.run("init-tree-after", this), this.permissions=new Mavo.Permissions;var backendTypes=["source", "storage", "init", "uploads"];backendTypes.forEach(role => this.updateBackend(role)), this.backendObserver=new Mavo.Observer(this.element, backendTypes.map(role => "mv-"+role), records => {
var changed={}, roles=records.map(record => {
var role=record.attributeName.replace(/^mv-/, "");return changed[role]=this.updateBackend(role), role;
});changed.source?this.load():!this.source&&(changed.storage||changed.init&&!this.root.data)&&this.load();
}), this.permissions.can("login", () => {
(null!==Mavo.Functions.url("login")&&1==this.index||null!==Mavo.Functions.url(this.id+"-login"))&&this.primaryBackend.login();
}), $.bind(this.element, "mv-login.mavo", evt => {
evt.backend!=(this.source||this.storage)||this.root.data||this.unsavedChanges||this.load();
}), this.bar=new Mavo.UI.Bar(this), $("summary [property]:not([typeof])")&&this.element.addEventListener("click", evt => {
evt.target!=document.activeElement&&evt.preventDefault();
}), this.needsEdit=this.calculateNeedsEdit(), this.setUnsavedChanges(!1), this.permissions.onchange(({action, value}) => {
var permissions=this.element.getAttribute("mv-permissions")||"";permissions=permissions.trim().split(/\s+/).filter(a => a!=action), value&&permissions.push(action), this.element.setAttribute("mv-permissions", permissions.join(" "));
}), this.permissions.can(["edit", "add", "delete"], () => {
this.modeObserver=new Mavo.Observer(this.element, "mv-mode", records => {
records.forEach(record => {
let element=record.target, nodes=_.Node.children(element);nodeloop:for (let i=0;i<nodes.length;i++) {
let mode, node=nodes[i], previousMode=node.mode;if (node.element==element) {
...
this.dataLoaded.resolve(), this.expressions.update(), $.fire(this.element, "mv-load");
}), $.bind(this.element, "mv-load.mavo", () => {
if (location.hash) {
var callback=() => {
var target=document.getElementById(location.hash.slice(1));return (target||!location.hash)&&(this.element.contains(target)&&requestAnimationFrame(() => {
Mavo.scrollIntoViewIfNeeded(target);
}), this.idObserver&&(this.idObserver.destroy(), this.idObserver=null)), target;
};callback()||(this.idObserver=new Mavo.Observer(this.element, "id", callback, {subtree:!0}));
}requestAnimationFrame(() => Stretchy.resizeAll());
}), this.autoSave&&this.dataLoaded.then(() => {
var debouncedSave=_.debounce(() => {
this.save();
}, this.autoSaveDelay), callback=evt => {
...
if (this.permissions.save&&83==evt.keyCode&&evt[_.superKey]&&!evt.altKey) {
...
var nextNode=node.getCousin(38==evt.keyCode?-1:1, {wrap:!0});nextNode&&(editor&&nextNode.editing?nextNode.edit({immediately:!0}).then(() => nextNode.editor.focus()):nextNode.element.focus(), evt.preventDefault());
}
}
...
this.message(message, {type:"error", dismiss:["button", "timeout"]}), 0<log.length&&console.log(`%c${this.id}: ${message}`, "color: red; font-weight: bold", ...log);
}, render:function(data) {
var env={context:this, data};_.hooks.run("render-start", env), env.data&&this.root.render(env.data), this.unsavedChanges=!1, _.hooks.run("render-end", env);
}, edit:function() {
this.root.edit(), $.bind(this.element, "mouseenter.mavo:edit mouseleave.mavo:edit", evt => {
...
var unsavedChanges=!!value;return value||this.walk(obj => {
if (obj.unsavedChanges) {
return unsavedChanges=!0, !1===value&&(obj.unsavedChanges=!1), !1;
}
}), this.unsavedChanges=unsavedChanges;
}, updateBackend:function(role) {
var backend, changed, previous=this[role];if (1==this.index&&(backend=_.Functions.url(role)), backend||(backend=_.Functions.url(`${this.id}-${role}`)||this.element.getAttribute("mv-"+role)||null), backend&&(backend=backend.trim(), "none"==backend&&(backend=null)), !backend||previous&&previous.equals(backend)?!backend&&(this[role]=null):(this[role]=backend=_.Backend.create(backend, {mavo:this, format:this.element.getAttribute(`mv-${role}-format`)||this.element.getAttribute("mv-format")}, this.element.getAttribute(`mv-${role}-type`), this[role]), changed=!0), changed=changed||(backend?!backend.equals(previous):!!previous), changed) {
...
var backend=this.source||this.storage;return backend?(this.inProgress="Loading", backend.ready.then(() => backend.load()).catch(err => this.init&&this.init!=backend?(backend=this.init, this.init.ready.then(() => this.init.load())):Promise.reject(err)).catch(err => {
if (err) {
var xhr=err instanceof XMLHttpRequest?err:err.xhr;if (xhr&&404==xhr.status) {
this.render(null);
}
else {
var message=this._("problem-loading");xhr&&(message+=xhr.status?this._("http-error", err):": "+this._("cant-connect")), this.error(message, err);
}
} return null;
}).then(data => this.render(data)).then(() => {
this.inProgress=!1, requestAnimationFrame(() => {
this.dataLoaded.resolve(), $.fire(this.element, "mv-load");
});
})):Promise.resolve();
}, store:function() {
return this.storage?(this.inProgress="Saving", this.storage.store(this.getData()).catch(err => {
if (err) {
var message=this._("problem-saving");err instanceof XMLHttpRequest&&(message+=": "+(err.status?this._("http-error", err):this._("cant-connect"))), this.error(message, err);
} return null;
}).then(saved => (this.inProgress=!1, saved))):Promise.resolve();
}, upload:function(file, path="images/"+file.name) {
return this.uploadBackend?(this.inProgress=this._("uploading"), this.uploadBackend.upload(file, path).then(url => (this.inProgress=!1, url)).catch(err => (this.error(this._("error-uploading"), err), this.inProgress=!1, null))):Promise.reject();
}, save:function() {
return this.store().then(saved => {
saved&&($.fire(this.element, "mv-save", saved), this.lastSaved=Date.now(), this.root.save(), this.unsavedChanges=!1);
});
}, walk:function() {
return this.root.walk(...arguments);
}, calculateNeedsEdit:function() {
var needsEdit=!1;return this.walk(obj => !needsEdit&&(needsEdit=!obj.modes&&!(obj instanceof Mavo.Group), !obj.modes), void 0, {descentReturn:!0}), needsEdit;
}, changed:function(change) {
!this.root||this.expressions.active&&this.expressions.updateThrottled(change);
}, setDeleted:function(...nodes) {
if (this.deleted.forEach(node => node.destroy()), this.deleted.length=[], this.deletionNotice&&this.deletionNotice.close(), !!nodes.length) {
if (this.deleted.push(...nodes), 1==nodes.length) {
var phrase=nodes[0].name;
}
else {
var counts={}, ret=[];for (var name in nodes.forEach(n => {
counts[n.name]=(counts[n.name]||0)+1;
}), counts) {
ret.push(this._("n-items", {name, n:counts[name]}));
} var phrase=ret.join(", ");
} var notice=this.deletionNotice=this.message([this._("item-deleted", {name:phrase}), {tag:"button", type:"button", textContent:this._("undo"), events:{click:() => {
this.undoDelete(), this.deletionNotice.close(!0);
}}}], {classes:"mv-deleted", dismiss:{button:!0, timeout:2e4}});notice.closed.then(undone => {
!undone&&this.deleted.length&&(this.deleted.forEach(node => node.destroy()), this.deleted.length=0), this.deletionNotice==notice&&(this.deletionNotice=null);
});
}
}, undoDelete:function() {
this.deleted.forEach(node => node.collection.add(node, node.index)), this.deleted.length=0;
}, live:{inProgress:function(value) {
$.toggleAttribute(this.element, "mv-progress", value, value), $.toggleAttribute(this.element, "aria-busy", !!value, !!value), this.element.style.setProperty("--mv-progress-text", value?`"${this._(value)}"`:"");
...
const backend=this.uploads;return backend&&backend.upload?(backend.permissions.login&&backend.login(), this.uploads):this.storage&&this.storage.upload?this.storage:void 0;
...
}, superKey:0===navigator.platform.indexOf("Mac")?"metaKey":"ctrlKey", base:"about:"==location.protocol?document.currentScript?document.currentScript.src:"http://mavo.io":location, dependencies:[$.ready().then(() => _.Plugins.load())], polyfills:[], init:function(container=document) {
var mavos=Array.isArray(arguments[0])?arguments[0]:$$(_.selectors.init, container), ret=mavos.filter(element => !_.get(element)).map(element => new _(element));return ret;
}, warn:function warn(message, o={}) {
warn.history=warn.history||new Set, _.warn.history.has(message)||console.warn(message), !1!==o.once&&warn.history.add(message);
}, thenAll:function(iterable) {
return $$(iterable).forEach(promise => {
"promise"==$.type(promise)&&(promise=promise.catch(err => err));
}), Promise.all(iterable).then(resolved => iterable.length==resolved.length?resolved:_.thenAll(iterable));
}, promise:function(constructor) {
var res, rej, promise=new Promise((resolve, reject) => {
constructor&&constructor(resolve, reject), res=resolve, rej=reject;
});return promise.resolve=a => (res(a), promise), promise.reject=a => (rej(a), promise), promise;
}, defer:delay => new Promise(resolve => delay===void 0?requestAnimationFrame(resolve):setTimeout(resolve, delay)), UI:{}, hooks:new $.Hooks, properties:new Set, attributes:["mv-app", "mv-storage", "mv-source", "mv-init", "mv-path", "mv-multiple-path", "mv-format", "mv-attribute", "mv-default", "mv-mode", "mv-edit", "mv-permisssions", "mv-rel", "mv-value"], lazy:{locale:() => document.documentElement.lang||"en-GB"}}});["toNode", "isProxy", "route", "parent", "property", "mavo", "groupedBy", "as"].forEach(symbol => {
$.lazy(_, symbol, () => Symbol(symbol));
}), Object.defineProperty(_.all, "length", {get:function() {
return Object.keys(this).length;
}}); {let s=_.selectors={init:"[mv-app], [data-mv-app]", property:"[property]", specificProperty:name => `[property=${name}]`, group:"[typeof], [mv-group]", primitive:"[property]:not([typeof]):not([mv-group])", childGroup:"[typeof][property], [mv-group][property]", multiple:"[mv-multiple]", formControl:"input, select, option, textarea", textInput:["text", "email", "url", "tel", "search", "number"].map(t => `input[type=${t}]`).join(", ")+", input:not([type]), textarea", ui:".mv-ui", container:{tr:"table", option:"select"}};$.extend(_.selectors, {item:s.multiple+", "+s.group, output:s.specificProperty("output")+", .mv-output"});}$.each({blissfuljs:Array.from&&document.documentElement.closest&&self.URL&&"searchParams"in URL.prototype, "Intl.~locale.en":self.Intl, IntersectionObserver:self.IntersectionObserver, Symbol:self.Symbol, "Element.prototype.remove":Element.prototype.remove, "Element.prototype.before":Element.prototype.before, "Element.prototype.after":Element.prototype.after, "Element.prototype.prepend":Element.prototype.prepend}, (id, supported) => {
supported||_.polyfills.push(id);
}), _.dependencies.push(_.defer().then(() => {
var polyfillURL="https://cdn.polyfill.io/v2/polyfill.min.js?unknown=polyfill&features="+_.polyfills.map(a => a+"|gated").join(",");_.dependencies.push($.include(!_.polyfills.length, polyfillURL)), $.ready().then(() => ($$(_.selectors.init).forEach(function(elem) {
_.get(elem)||elem.setAttribute("mv-progress", "Loading");
}), _.ready)).catch(console.error).then(() => {
...
var element=_.getTarget();const cl="mv-target-within";for ($$(".mv-target-within").forEach(el => el.classList.remove(cl));element&&element.classList;) {
element.classList.add(cl), element=element.parentNode;
}
} var _=$.extend(Mavo, {load:(url, base=document.currentScript?document.currentScript.src:location) => $.load(url, base), readFile:(file, format="DataURL") => {
var reader=new FileReader;return new Promise((resolve, reject) => {
reader.onload=() => resolve(reader.result), reader.onerror=reader.onabort=reject, reader["readAs"+format](file);
});
}, toJSON:data => {
if (null===data) {
return "";
...
var cache=self.WeakSet?new WeakSet:new Set;return JSON.stringify(o, (key, value) => {
...
}, primitivify:(object, primitive) => (object&&(primitive&&"object"==typeof primitive&&(Object.assign(object, primitive), primitive=Mavo.value(primitive)), object.valueOf=object.toJSON=object[Symbol.toPrimitive]=() => primitive), object), objectify:(value, properties) => {
var primitive=Mavo.value(value);if ("object"!=typeof value||null===value) {
if (null===value) {
...
}, value:value => value&&value.valueOf?value.valueOf():value, toArray:arr => void 0===arr?[]:Array.isArray(arr)?arr:[arr], delete:(arr, element, all) => {
do {
var index=arr&&arr.indexOf(element);-1<index&&arr.splice(index, 1);
} while (-1<index&&all);
}, flatten:arr => Array.isArray(arr)?arr.reduce((prev, c) => _.toArray(prev).concat(_.flatten(c)), []):[arr], pushUnique:(arr, item) => {
-1===arr.indexOf(item)&&arr.push(item);
}, union:(set1, set2) => set1 instanceof Set&&set2?(set2.forEach(x => set1.add(x)), set1):new Set([...(set1||[]), ...(set2||[])]), filter:(arr, callback) => {
for (var i=0;i<arr.length;i++) {
callback(arr[i])||(arr.splice(i, 1), i--);
}
}, is:function(thing, ...elements) {
for (let element, i=0;i<elements.length;i++) {
if (element=elements[i], element&&element.matches&&element.matches(_.selectors[thing])) {
return !0;
}
} return !1;
...
if (!element) {
return null;
} var ret, data=_.elementData.get(element)||{};return 2==arguments.length?ret=data[name]:void 0===value?delete data[name]:ret=data[name]=value, _.elementData.set(element, data), ret;
}, elementData:new WeakMap, elementPath:function(ancestor, element) {
if (Array.isArray(element)) {
var path=element, ret=path.reduce((acc, cur) => acc.children[cur>>0]||acc, ancestor), last=path[path.length-1];if (last!=last>>0) {
...
for (var index=0, countNonElementSiblings=parent===element&&1!==element.nodeType, offset=countNonElementSiblings?1:0, sibling=parent;sibling=sibling[`previous${countNonElementSiblings?"":"Element"}Sibling`];) {
...
observer.unobserve(entry.target), $.fire(entry.target, "mv-inview", {entry});
});
});return new Promise(resolve => {
_.is(element)&&resolve(), observer.observe(element);var callback=evt => {
...
for (let attribute, value, i=0;attribute=attributes[i];i++) {
if (value=element.getAttribute(attribute), value) {
return value;
}
} return null;
}, getClosestAttribute:function(element, attribute) {
return element=element.closest(`[${attribute}]`), element?element.getAttribute(attribute):null;
}, getTarget:function() {
var id=location.hash.substr(1);return document.getElementById(id);
}, XPath:function(query, context=document) {
var node, doc=context.ownerDocument||context, ret=[];if (doc.evaluate) {
...
if (obj) {
return "object"==typeof obj&&property in obj||void 0!==obj[property];
}
...
return propertyL;
} var properties=Object.keys(obj), i=properties.map(p => p.toLowerCase()).indexOf(propertyL);if (-1<i) {
return properties[i];
}
}
}
...
if (path.length) {
var last=path[path.length-1], parent=$.value(obj, ...path.slice(0, -1));return Array.isArray(parent)&&Array.isArray(value)?parent.splice(last, 1, ...value):parent&&(parent[path[path.length-1]]=value), obj;
} return value;
} return "object"==typeof obj&&path&&path.length?path.reduce((obj, property, i) => {
var meta={}, ret=Mavo.Functions.get(obj, property, meta);return path[i]=Array.isArray(meta.property)?meta.property[0]:meta.property, void 0===ret&&meta.query&&(ret={[meta.query.property]:meta.query.value}), ret;
}, obj):obj;
}, clone:function(o) {
return o&&"object"==typeof o?JSON.parse(_.safeToJSON(o)):o;
}, shallowClone:function(o) {
return o&&"object"==typeof o?Array.isArray(o)?[...o]:$.extend({}, o):o;
}, debounce:function(fn, delay) {
if (!delay) {
return fn;
} var code, timer=null;return function() {
var context=this, args=arguments;code=function() {
fn.apply(context, args), removeEventListener("beforeunload", code);
}, clearTimeout(timer), timer=setTimeout(code, delay), addEventListener("beforeunload", code);
};
}, escapeRegExp:s => s.replace(/[-\/\\^$*+?.()|[\]{}]/g, "\\$&"), matches:(str, regex) => {
var ret=(str+"").match(regex);return ret?ret:[];
}, match:(str, regex, i=0) => _.matches(str, regex)[i]||"", observeResize:function(element, callbackOrObserver) {
if (self.ResizeObserver) {
var previousRect=null, ro=callbackOrObserver instanceof ResizeObserver?callbackOrObserver:new ResizeObserver(entries => {
...
}, Observer:$.Class({constructor:function(element, attribute, callback, o={}) {
callback instanceof MutationObserver&&(this.observer=callback), this.observer=this.observer||new MutationObserver(callback), this.callback=callback, this.update(element, attribute, o), this.run();
...
return f(), f;
}, wrap:(index, length) => 0>index?length-1:index>=length?0:index, options:str => {
var ret={};return (str.trim().match(/(?:\\[,;]|[^,;])+/g)||[]).forEach(option => {
if (option) {
...
this.map=new Map, this[Symbol.iterator]=this.map[Symbol.iterator], this.arrays=arrays;
}set(key, value) {
if (this.arrays) {
...
var keys=Mavo.matches(phrase, /\{\w+(?=\})/g).map(v => v.slice(1));Mavo.Functions.unique(keys).forEach(name => {
name in vars&&(phrase=phrase.replace(RegExp(`{${name}}`, "gi"), vars[name]));
});
} return phrase;
}, live:{lang:function(lang) {
this.baseLang=_.getBaseLang(lang), lang==this.baseLang&&(this.baseLang=null);
...
return lang.split("-")[0];
}, lazy:{default:() => _.match(Mavo.locale)||_.all.en}}});Mavo.prototype._=function(id, vars) {
return this.locale&&id?this.locale.phrase(id, vars):id;
}, Mavo.ready.then(() => {
$$("datalist.mv-phrases[lang]").forEach(datalist => {
var phrases=$$("option", datalist).reduce((o, option) => (o[option.value]=option.textContent.trim(), o), {});Mavo.Locale.register(datalist.lang, phrases);
});
});
...
return _.plugins=new Set, $$("[mv-plugins]").forEach(element => {
element.getAttribute("mv-plugins").trim().split(/\s+/).forEach(plugin => _.plugins.add(plugin));
}), _.plugins.size?$.fetch(_.url+"/plugins.json", {responseType:"json"}).then(xhr => Mavo.thenAll(xhr.response.plugin.filter(plugin => _.plugins.has(plugin.id)).map(plugin => {
var filename=`mavo-${plugin.id}.js`;if (plugin.repo) {
var url=`https://raw.githubusercontent.com/${plugin.repo}/master/${filename}`;return _.loaded[plugin.id]?Promise.resolve():$.fetch(url).then(xhr => {
$.create("script", {textContent:xhr.responseText, inside:document.head});
});
} var url=`${_.url}/${plugin.id}/${filename}`;return $.include(_.loaded[plugin.id], url);
}))):Promise.resolve();
}, register:function(name, o={}) {
if (!_.loaded[name]) {
for (let Class in Mavo.hooks.add(o.hooks), o.extend) {
let existing="Mavo"==Class?Mavo:Mavo[Class];"function"===$.type(existing)?$.Class(existing, o.extend[Class]):$.extend(existing, o.extend[Class]);
} var ready=[];if (o.ready&&ready.push(o.ready), o.dependencies) {
var base=document.currentScript?document.currentScript.src:location, dependencies=o.dependencies.map(url => Mavo.load(url, base));ready.push(...dependencies);
}ready.length&&Mavo.dependencies.push(...ready), _.loaded[name]=o, o.init&&Promise.all(ready).then(() => o.init());
}
...
if (this.mavo=mavo, this.element=$(".mv-bar", this.mavo.element), this.template=this.mavo.element.getAttribute("mv-bar")||"", this.element) {
for (let id in this.custom=!0, this.template+=" "+(this.element.getAttribute("mv-bar")||""), this.template=this.template.trim(), _.controls) {
this[id]=$(`.mv-${id}`, this.element), this[id]&&(this.template=this.template||"with", this.template+=` ${id}`);
}
}
else {
this.element=$.create({className:"mv-bar mv-ui", start:this.mavo.element, innerHTML:"<button>&nbsp;</button>"});
} for (let id in this.element.classList.contains("mv-compact")&&(this.noResize=!0), this.controls=_.getControls(this.template), this.controls.length&&(this.targetHeight=this.element.offsetHeight), this.custom||(this.element.innerHTML=""), this.controls.forEach(id => {
let o=_.controls[id];for (var events in this[id]&&this[id].remove(), o.create?this[id]=o.create.call(this.mavo, this[id]):!this[id]&&(this[id]=$.create("button", {type:"button", className:`mv-${id}`, textContent:this.mavo._(id)})), this.add(id), o.permission?this.permissions.can(o.permission, () => {
this.toggle(id, !o.condition||o.condition.call(this.mavo));
}, () => {
this.remove(id);
}):o.condition&&!o.condition.call(this.mavo)&&this.remove(id), o.events) {
$.bind(this[id], events, o.events[events].bind(this.mavo));
}
}), _.controls) {
let o=_.controls[id];o.action&&$.delegate(this.mavo.element, "click", ".mv-"+id, evt => {
(!o.permission||this.permissions.is(o.permission))&&(o.action.call(this.mavo), evt.preventDefault());
});
} this.controls.length&&!this.noResize&&(this.resize(), self.ResizeObserver&&(this.resizeObserver=Mavo.observeResize(this.element, () => {
this.resize();
})));
}, resize:function() {
return this.targetHeight?void(this.resizeObserver&&this.resizeObserver.disconnect(), this.element.classList.remove("mv-compact", "mv-tiny"), $$("button, .mv-button", this.element).forEach(button => {
button.title===button.textContent&&(button.title="");
}), this.element.offsetHeight>1.6*this.targetHeight&&(this.element.classList.add("mv-compact"), this.element.offsetHeight>1.2*this.targetHeight&&(this.element.classList.add("mv-tiny"), $$("button, .mv-button", this.element).forEach(button => {
...
var o=_.controls[id];o.prepare&&o.prepare.call(this.mavo), Mavo.revocably.add(this[id], this.element), this.resizeObserver||this.noResize||requestAnimationFrame(() => this.resize());
}, remove:function(id) {
var o=_.controls[id];Mavo.revocably.remove(this[id], "mv-"+id), o.cleanup&&o.cleanup.call(this.mavo), this.resizeObserver||this.noResize||requestAnimationFrame(() => this.resize());
...
if ("none"==template) {
return [];
} var relative=/^with\s|\b(yes|no)-\w+\b/.test(template);template=template.replace(/\byes-|^with\s+/g, "");var ids=template.split(/\s+/);return ids=Mavo.Functions.unique(ids.reverse()).reverse(), relative?all.filter(id => {
...
var user=backend.user, html=[user.name||""];user.avatar&&html.unshift($.create("img", {className:"mv-avatar", src:user.avatar}), " "), user.url&&(html=[$.create("a", {href:user.url, target:"_blank", contents:html})]), this.bar.status.textContent="", $.contents(this.bar.status, [{tag:"span", innerHTML:this._("logged-in-as", backend)}, " ", ...html]);
}
...
var button=custom||$.create("span", {role:"button", tabIndex:"0", className:"mv-import mv-button", textContent:this._("import"), events:{focus:() => {
input.focus();
}}}), input=$.create("input", {type:"file", inside:button, events:{change:evt => {
var file=evt.target.files[0];if (file) {
var reader=$.extend(new FileReader, {onload:() => {
this.inProgress=!1;try {
var json=JSON.parse(reader.result);this.render(json);
}
catch (e) {
...
}});this.inProgress=this._("uploading"), reader.readAsText(file);
}
}}});return button;
...
Mavo.UI.Message=$.Class({constructor:function(mavo, message, o={}) {
if (this.mavo=mavo, this.message=message, this.closed=Mavo.promise(), this.options=o, this.element=$.create({className:"mv-ui mv-message"+(o.type?" mv-"+o.type:""), ["string"==$.type(this.message)?"innerHTML":"contents"]:this.message, events:{click:() => Mavo.scrollIntoViewIfNeeded(this.mavo.element)}, [this.mavo.bar?"after":"start"]:(this.mavo.bar||this.mavo).element}), o.style&&$.style(this.element, o.style), o.classes&&this.element.classList.add(...o.classes.split(/\s+/)), "error"==o.type?this.element.setAttribute("role", "alert"):this.element.setAttribute("aria-live", "polite"), o.dismiss=o.dismiss||{}, "string"==typeof o.dismiss||Array.isArray(o.dismiss)) {
var dismiss={};Mavo.toArray(o.dismiss).forEach(prop => {
dismiss[prop]=!0;
}), o.dismiss=dismiss;
} if (o.dismiss.button&&$.create("button", {type:"button", className:"mv-close mv-ui", textContent:"\xD7", events:{click:() => this.close()}, start:this.element, title:this.mavo._("dismiss")}), o.dismiss.timeout) {
var timeout="number"==typeof o.dismiss.timeout?o.dismiss.timeout:5e3;$.bind(this.element, {mouseenter:() => clearTimeout(this.closeTimeout), mouseleave:Mavo.rr(() => this.closeTimeout=setTimeout(() => this.close(), timeout))});
}o.dismiss.submit&&this.element.addEventListener("submit", evt => {
evt.preventDefault(), this.close(evt.target);
});
}, close:function(resolve) {
clearTimeout(this.closeTimeout);var duration=this.element.style.transition?1e3*parseFloat(window.getComputedStyle(this.element, null).transitionDuration):400;$.transition(this.element, {opacity:0}, duration).then(() => {
$.remove(this.element), this.closed.resolve(resolve);
});
}});
}(Bliss, Bliss.$), function($) {
...
return Mavo.toArray(actions).forEach(action => this[action]=!0), this;
}, off:function(actions) {
return actions=Array.isArray(actions)?actions:[actions], actions.forEach(action => this[action]=!1), this;
...
actions=Mavo.toArray(actions), this.is(actions, value)&&callback(), this.triggers.push({actions, value, callback, active:!0});
}, is:function(actions, able=!0) {
var or=Mavo.toArray(actions).map(action => !!this[action]).reduce((prev, current) => prev||current);return able?or:!or;
}, onchange:function(callback) {
this.hooks.add("change", callback), _.actions.forEach(action => {
callback.call(this, {action, value:this[action]});
});
}, parentChanged:function(o={}) {
var localValue=this["_"+o.action];void 0!==localValue||o.from==o.value||(this.fireTriggers(o.action), this.hooks.run("change", $.extend({context:this}, o)));
}, changed:function(action, value, from) {
from=!!from, value=!!value;value==from||(this["_"+action]=value, this.fireTriggers(action), this.hooks.run("change", {action, value, from, context:this}));
}, fireTriggers:function(action) {
this.triggers.forEach(trigger => {
var match=this.is(trigger.actions, trigger.value);trigger.active&&-1<trigger.actions.indexOf(action)&&match?(trigger.active=!1, trigger.callback()):!match&&(trigger.active=!0);
});
}, or:function(permissions) {
return _.actions.forEach(action => {
this[action]=this[action]||permissions[action];
}), this;
}, live:{parent:function(parent) {
var oldParent=this._parent;oldParent==parent||(this._parent=parent, oldParent&&Mavo.delete(oldParent.hooks.change, this.parentChanged), _.actions.forEach(action => {
this.parentChanged({action, value:parent?parent[action]:void 0, from:oldParent?oldParent[action]:void 0});
}), parent&&parent.onchange(this.parentChanged));
}}, static:{actions:[], register:function(action, setter) {
return Array.isArray(action)?void action.forEach(action => _.register(action, setter)):void($.live(_.prototype, action, {get:function() {
...
var _Mathmin=Math.min, _=Mavo.Backend=$.Class({constructor:function(url, o={}) {
this.update(url, o), this.permissions=new Mavo.Permissions;
}, update:function(url, o={}) {
this.source=url, this.url=new URL(this.source, Mavo.base), this.mavo=o.mavo, this.format=Mavo.Formats.create(o.format, this);
}, get:function(url=new URL(this.url)) {
return "data:"!=url.protocol&&url.searchParams.set("timestamp", Date.now()), $.fetch(url.href).then(xhr => Promise.resolve(xhr.responseText), () => Promise.resolve(null));
}, load:function() {
return this.ready.then(() => this.get()).then(response => "string"==typeof response?(response=response.replace(/^\ufeff/, ""), this.format.parse(response)):response);
}, store:function(data, {path, format=this.format}={}) {
return this.ready.then(() => {
var serialize="string"==typeof data?Promise.resolve(data):format.stringify(data);return serialize.then(serialized => this.put(serialized, path).then(() => ({data, serialized})));
});
...
if (req=$.extend({}, req), req.method=req.method||method, req.responseType=req.responseType||"json", req.headers=$.extend({"Content-Type":"application/json; charset=utf-8"}, req.headers||{}), this.isAuthenticated()&&(req.headers.Authorization=req.headers.Authorization||`Bearer ${this.accessToken}`), req.data=data, call=new URL(call, this.constructor.apiDomain), "GET"==req.method&&call.searchParams.set("timestamp", Date.now()), "object"===$.type(req.data)) {
if ("GET"==req.method) {
for (let p in req.data) {
let action=void 0===req.data[p]?"delete":"set";call.searchParams[action](p, req.data[p]);
} delete req.data;
}
else {
req.data=JSON.stringify(req.data);
}
} return $.fetch(call, req).catch(err => err&&err.xhr?Promise.reject(err.xhr):void this.mavo.error("Something went wrong while connecting to "+this.id, err)).then(xhr => "HEAD"==req.method?xhr:xhr.response);
}, oAuthenticate:function(passive) {
return this.ready.then(() => this.isAuthenticated()?Promise.resolve():new Promise((resolve, reject) => {
var id=this.id.toLowerCase();if (passive) {
this.accessToken=localStorage[`mavo:${id}token`], this.accessToken&&resolve(this.accessToken);
}
else {
var popup={width:_Mathmin(1e3, innerWidth-100), height:_Mathmin(800, innerHeight-100)};popup.top=(screen.height-popup.height)/2, popup.left=(screen.width-popup.width)/2;var state={url:location.href, backend:this.id};(this.authPopup=open(`${this.constructor.oAuth}?client_id=${this.key}&state=${encodeURIComponent(JSON.stringify(state))}`+this.oAuthParams(), "popup", `width=${popup.width},height=${popup.height},left=${popup.left},top=${popup.top}`), !this.authPopup)&&(this.mavo.error("Login popup was blocked! Please check your popup blocker settings."), reject(Error("Login popup was blocked! Please check your popup blocker settings."))), addEventListener("message", evt => {
if (evt.source===this.authPopup) {
for (var appid in evt.data.backend==this.id&&(this.accessToken=localStorage[`mavo:${id}token`]=evt.data.token), this.accessToken||reject(Error("Authentication error")), resolve(this.accessToken), Mavo.all) {
var storage=Mavo.all[appid].primaryBackend;storage&&storage.id===this.id&&storage!==this&&!storage.isAuthenticated()&&storage.login(!0);
}
}
});
}
...
var id=this.id.toLowerCase();localStorage.removeItem(`mavo:${id}token`), delete this.accessToken, this.permissions.off(["edit", "add", "delete", "save"]).on("login"), $.fire(this.mavo.element, "mv-logout", {backend:this});
} return Promise.resolve();
}, static:{create:function(url, o, type, existing) {
var Backend;return type&&(Backend=Mavo.Functions.get(_, type)), url&&!Backend&&(Backend=_.types.filter(Backend => Backend.test(url))[0]||_.Remote), Backend&&existing&&existing.constructor===Backend&&existing.constructor.prototype.hasOwnProperty("update")?(existing.update(url, o), existing):Backend?new Backend(url, o):null;
...
}, static:{test:url => 0===url.indexOf("#")}})), _.register($.Class({id:"Remote", extends:_, constructor:function() {
this.permissions.on("read");
}, static:{test:() => !1}})), _.register($.Class({extends:_, id:"Local", constructor:function() {
...
}, static:{parse:serialized => Promise.resolve(serialized), stringify:data => Promise.resolve(data), extensions:[], dependencies:[], ready:function() {
return Promise.all(this.dependencies.map(d => $.include(d.test(), d.url)));
}}}), json=_.JSON=$.Class({extends:_.Base, static:{parse:serialized => Promise.resolve(serialized?JSON.parse(serialized):null), stringify:data => Promise.resolve(Mavo.toJSON(data)), extensions:[".json", ".jsonld"]}}), text=_.Text=$.Class({extends:_.Base, constructor:function() {
this.property=this.mavo.root.getNames("Primitive")[0];
}, static:{extensions:[".txt"], parse:(serialized, me) => Promise.resolve({[me?me.property:"content"]:serialized}), stringify:(data, me) => Promise.resolve(data[me?me.property:"content"])}}), csv=_.CSV=$.Class({extends:_.Base, constructor:function() {
this.property=this.mavo.root.getNames("Collection")[0], this.options=$.extend({}, _.CSV.defaultOptions);
}, static:{extensions:[".csv", ".tsv"], defaultOptions:{header:!0, dynamicTyping:!0, skipEmptyLines:!0}, dependencies:[{test:() => self.Papa, url:"https://cdnjs.cloudflare.com/ajax/libs/PapaParse/4.1.4/papaparse.min.js"}], ready:base.ready, parse:(serialized, me) => csv.ready().then(() => {
...
} var env={context:this, options};this.uid=_.all.push(this)-1, this.property=null, this.element=element, this.isHelperVariable=this.element.matches("meta"), $.extend(this, env.options), _.elements.set(element, [...(_.elements.get(this.element)||[]), this]), this.mavo=mavo, this.group=this.parent=this.parentGroup=env.options.group, this.template=env.options.template, this.alias=this.element.getAttribute("mv-alias"), this.template?this.template.copies.push(this):this.copies=[], this.fromTemplate("property", "type", "storage", "path")||(this.property=_.getProperty(element), this.type=Mavo.Group.normalize(element), this.storage=this.element.getAttribute("mv-storage"), this.path=this.getPath()), this.modes=this.element.getAttribute("mv-mode"), Mavo.hooks.run("node-init-start", env), this.mode=Mavo.getStyle(this.element, "--mv-mode")||"read", this.collection=env.options.collection, this.collection&&(this.group=this.parentGroup=this.collection.parentGroup);var template=this.template;if (template&&template.expressions&&(this.expressions=template.expressions.map(et => new Mavo.DOMExpression({template:et, item:this, mavo:this.mavo}))), this instanceof Mavo.Group||this.collection) {
var et=Mavo.DOMExpression.search(this.element).filter(et => "mv-value"==et.originalAttribute)[0];et&&(et.mavoNode=this, this.expressionText=et, this.storage=this.storage||"none", this.modes="read", this.collection&&(this.collection.expressions=[...(this.collection.expressions||[]), et], et.mavoNode=this.collection, this.collection.storage=this.collection.storage||"none", this.collection.modes="read"));
} this.element.hasAttribute("mv-storage")&&(this.storageObserver=new Mavo.Observer(this.element, "mv-storage", () => {
...
this.template&&Mavo.delete(this.template.copies, this), this.expressions&&this.expressions.forEach(expression => expression.destroy()), this.itembar&&this.itembar.destroy(), delete _.all[this.uid], this.propagate("destroy");
}getLiveData() {
return this.liveData.proxy;
}isDataNull(o={}) {
var env={context:this, options:o, result:!this.saved&&!o.live};return Mavo.hooks.run("node-isdatanull", env), env.result;
}walk(callback, path=[], o={}) {
var walker=(obj, path) => {
var ret=callback(obj, path);if (!1!==ret) {
for (let i in obj.children) {
let node=obj.children[i];if (node instanceof Mavo.Node) {
var ret=walker.call(node, node, [...path, i]);if (!1===ret&&!o.descentReturn) {
return !1;
}
}
}
...
var ret=callback(group);if (ret!==void 0) {
return ret;
}
}
}edit() {
...
let node=this.children[i];node instanceof Mavo.Node&&("function"==typeof callback?callback.call(node, node):callback in node&&node[callback]());
}
}fromTemplate(...properties) {
return this.template&&properties.forEach(property => this[property]=this.template[property]), !!this.template;
}render(data, o={}) {
o.live=o.live||Mavo.in(Mavo.isProxy, data), o.root=o.root||this, o.live&&(data=Mavo.clone(data)), this.oldData=this.data, this.data=data, o.live||(data=Mavo.subset(data, this.inPath));var env={context:this, data, options:o};if (Mavo.hooks.run("node-render-start", env), !this.isHelperVariable) {
if (!Array.isArray(this.children)&&Array.isArray(env.data)) {
if (this.isRoot) {
var mainProperty=this.children.main instanceof Mavo.Collection?"main":this.getNames((p, n) => n instanceof Mavo.Collection&&!$.value(n.expressions, 0, "isDynamicObject"))[0];mainProperty&&(env.data={[mainProperty]:env.data});
...
} this===o.root&&(this.expressionsEnabled=!1);var editing=this.editing;editing&&this.done();var changed=this.dataRender(env.data, o);return editing&&this.edit(), this===o.root&&(this.save(), this.expressionsEnabled=!0, changed&&requestAnimationFrame(() => this.mavo.expressions.update(this))), Mavo.hooks.run("node-render-end", env), changed;
}dataChanged(action, o={}) {
var change=$.extend({action, property:this.property, mavo:this.mavo, node:this}, o);$.fire(o.element||this.element, "mv-change", change), this.mavo.changed(change);
}toString() {
...
var path=this.parent?this.parent.path:[];return this.property?[...path, this.property]:path;
}pathFrom(node) {
...
return null;
} var collection=this.closestCollection, distance=Math.abs(offset), direction=0>offset?-1:1;if (collection.length<distance+1) {
return null;
} var index=this.closestItem.index+offset;o.wrap&&(index=Mavo.wrap(index, collection.length));for (var ind, i=0;i<collection.length;i++) {
...
return Mavo.is("multiple", element)&&!o.collection?new Mavo.Collection(element, mavo, o):new Mavo[Mavo.is("group", element)?"Group":"Primitive"](element, mavo, o);
...
for (var name, root=element.closest(Mavo.selectors.init), i="";1e4>i;i++) {
if (name=prefix+i, !$(Mavo.selectors.specificProperty(name), root)) {
return name;
}
}
}static get(element, prioritizePrimitive) {
var nodes=(_.elements.get(element)||[]).filter(node => !Array.isArray(node.children));return 2>nodes.length||!prioritizePrimitive?nodes[0]:nodes[0]instanceof Mavo.Group?nodes[1]:void 0;
...
var ret=Mavo.Node.get(element);return ret?[ret]:(ret=$$(Mavo.selectors.property, element).map(e => Mavo.Node.get(e)).filter(e => !element.contains(e.parentGroup.element)).map(e => e.collection||e), Mavo.Functions.unique(ret));
}
...
var attribute=this instanceof Mavo.Collection?"mv-multiple-path":"mv-path";return (this.element.getAttribute(attribute)||"").split("/").filter(p => p.length);
...
var set=this.modes||"edit"==value;Mavo.Observer.sneak(this.mavo.modeObserver, () => {
...
var _=Mavo.Group=class extends Mavo.Node {
constructor(element, mavo, o) {
if (super(element, mavo, o), this.children={}, this.group=this, Mavo.hooks.run("group-init-start", this), Mavo.Primitive.getValueAttribute(this.element)) {
this.children[this.property]=new Mavo.Primitive(this.element, this.mavo, {group:this});
} var properties=$$(Mavo.selectors.property+", "+Mavo.selectors.multiple, this.element).filter(element => this.element===(element.parentNode.closest(Mavo.selectors.childGroup)||this.mavo.element)), collections={};properties.forEach(element => {
...
var property=Mavo.Node.getProperty(element), template=this.template?this.template.children[property]:null, options={template, group:this}, isCollection=collections[property];if ("multiple"===isCollection) {
var existing=this.children[property];existing instanceof Mavo.Collection?existing.add(element):Mavo.is("multiple", element)?(this.children[property]=new Mavo.Collection(element, this.mavo, options), (existing||[]).forEach((e, i) => this.children[property].add(e, i))):this.children[property]=[...(existing||[]), element];
}
else {
1<isCollection?this.children[property]?this.children[property].add(element):this.children[property]=new Mavo.ImplicitCollection(element, this.mavo, options):this.children[property]=Mavo.Node.create(element, this.mavo, options);
}
}), this.childrenNames=Object.keys(this.children), this.vocab=Mavo.getClosestAttribute(this.element, "vocab"), this.postInit(), Mavo.hooks.run("group-init-end", this);
} get isRoot() {
return !this.property;
}getNames(type="Node") {
var filter="function"==typeof type?type:(p, n) => n instanceof Mavo[type];return Object.keys(this.children).filter(p => filter(p, this.children[p]));
}getData(o={}) {
var env={context:this, options:o};if (this.isDataNull(o)) {
return null;
...
} return this.childrenNames.length||this.isRoot||this.collection?1===this.childrenNames.length&&this.property in this.children?env.data=env.data[this.property]:env.data&&"object"==typeof env.data&&(this.type&&this.type!=_.DEFAULT_TYPE&&(env.data["@type"]=this.type), this.vocab&&(env.data["@context"]=this.vocab)):env.data=null, env.data=Mavo.subset(this.data, this.inPath, env.data), Mavo.hooks.run("node-getdata-end", env), env.data;
}edit(o={}) {
return !1!==super.edit()&&Promise.all(Object.keys(this.children).map(prop => this.children[prop].edit(o)));
}dataRender(data, o={}) {
if (data) {
var changed=!1;if ("object"!=typeof data) {
if (this.property in this.children) {
var property=this.property;
}
else {
var type=$.type(data), score=prop => (this.children[prop]instanceof Mavo.Primitive)+(this.children[prop].datatype==type), property=Object.keys(this.children).filter(p => !this.children[p].expressionText).sort((prop1, prop2) => score(prop1)-score(prop2)).reverse()[0];
} if (!property) {
property=this.property;
}data={[property]:data}, this.data=Mavo.subset(this.data, this.inPath, data);
} var copy;this.propagate(obj => {
var propertyData=data[obj.property];if (obj.alias) {
var aliasesArr=obj.alias.split(" ");for (let i=0;i<aliasesArr.length;i++) {
var currentAlias=aliasesArr[i];if (void 0!==data[currentAlias]) {
...
}), copy&&this.propagate(obj => {
obj.currentAlias&&(data[obj.property]=copy[obj.currentAlias], !(obj.currentAlias in this.children)&&delete data[obj.currentAlias]);
});var oldData=Mavo.subset(this.oldData, this.inPath);for (var property in data) {
if (!(property in this.children)) {
var value=data[property];changed=changed||data[property]!==this.liveData.data[property], this.liveData.set(property, value), "object"==typeof value||oldData&&oldData[property]==value||this.dataChanged("propertychange", {property});
}
} return changed;
}
...
};$.Class(_, {lazy:{liveData:function() {
return new Mavo.Data(this, {});
}}, static:{all:new WeakMap, DEFAULT_TYPE:"Item"}});
}(Bliss, Bliss.$), function($, $$) {
var _=Mavo.Primitive=class extends Mavo.Node {
constructor(element, mavo, o) {
super(element, mavo, o), this.liveData=new Mavo.Data(this), this.fromTemplate("config", "attribute", "templateValue", "originalEditor")||(this.config=_.getConfig(element), this.attribute=this.config.attribute, this.attribute&&!document.xmlVersion&&(this.attribute=this.attribute.toLowerCase())), this.datatype=this.config.datatype, "modes"in this.config&&(this.modes=this.config.modes, this.element.setAttribute("mv-mode", this.config.modes)), Mavo.hooks.run("primitive-init-start", this), this.expressionText=this.expressionText||Mavo.DOMExpression.search(this.element, this.attribute), this.expressionText&&!this.expressionText.mavoNode&&(this.expressionText.primitive=this, this.storage=this.storage||"none", this.modes="read", this.element.setAttribute("aria-live", "polite")), !this.editor&&this.element.hasAttribute("mv-edit")&&(!this.originalEditor&&(this.originalEditor=$(this.element.getAttribute("mv-edit"))), this.originalEditor&&!this.template&&(this.originalEditorObserver=new Mavo.Observer(this.originalEditor, "all", () => {
this.copies.concat(this).forEach(primitive => {
"editor"==primitive.defaultSource&&(primitive.default=this.originalEditor.value), primitive.editor&&(primitive.editor=this.originalEditor.cloneNode(!0)), primitive.setValue(primitive.value, {force:!0, silent:!0});
});
}))), this.editor||this.originalEditor||this.attribute||(this.editor=$$(this.element.children).filter(function(el) {
return el.matches(Mavo.selectors.formControl)&&!el.matches(Mavo.selectors.property);
})[0], this.editor&&$.remove(this.editor));var editorValue=this.editorValue;if (this.datatype||"number"!=typeof editorValue&&"boolean"!=typeof editorValue||(this.datatype=typeof editorValue), this.config.init&&this.config.init.call(this, this.element), this.config.changeEvents&&$.bind(this.element, this.config.changeEvents, evt => {
evt.target===this.element&&(this.value=this.getValue());
}), this.templateValue=this.getValue(), this._default=this.element.getAttribute("mv-default"), null===this.default) {
this._default=this.modes?this.templateValue:editorValue, this.defaultSource=this.modes?"template":"editor";
}
else if (""===this.default) {
this._default=this.templateValue, this.defaultSource="template";
}
else {
var defaultExpression=Mavo.DOMExpression.search(this.element, "mv-default");defaultExpression?defaultExpression.output=value => this.default=value:this.defaultObserver=new Mavo.Observer(this.element, "mv-default", () => {
this.default=this.element.getAttribute("mv-default");
}), this.defaultSource="attribute";
} var keepTemplateValue=!this.template||this.template.templateValue!=this.templateValue||"edit"==this.modes;this.initialValue=void 0===this.default&&keepTemplateValue?this.templateValue:this.default, void 0===this.initialValue&&(this.initialValue=this.emptyValue), this.setValue(this.initialValue, {silent:!0}), this.element.hasAttribute("aria-label")?$.lazy(this, "label", () => (this.labelObserver=new Mavo.Observer(this.element, "aria-label", () => {
this.label=this.element.getAttribute("aria-label"), "placeholder"in this.editor&&(this.editor.placeholder=`(${this.label})`);
}), this.element.getAttribute("aria-label"))):(this.label=Mavo.Functions.readable(this.property), this.element.setAttribute("aria-label", this.label)), this.attribute||Mavo.setAttributeShy(this.element, "mv-attribute", "none"), this.postInit(), Mavo.hooks.run("primitive-init-end", this);
} get editorValue() {
...
var options={subtree:this.config.subtree, childList:this.config.subtree};this.observer?((this.observer.attribute!==this.attribute||this.observer.options.subtree!==options.subtree)&&this.observer.update(this.element, this.attribute, options), !this.observer.running&&this.observer.run()):this.observer=new Mavo.Observer(this.element, this.attribute, () => {
!1===this._config.observer?this.observer.stop():(this.attribute||!this.editing||this.config.subtree)&&(this.value=this.getValue());
}, options);
}
}
}destroy() {
super.destroy(), ["defaultObserver", "observer", "originalEditorObserver", "labelObserver"].forEach(observer => {
this[observer]&&this[observer].destroy();
});
}isDataNull(o) {
return super.isDataNull(o)||null===this._value||this._value===void 0;
}getData(o={}) {
var env={context:this, options:o};return this.isDataNull(o)?null:(env.data=this.value, ""!==env.data||this.templateValue&&this.initialValue===this.templateValue||(env.data=null), this.inPath.length&&(env.data=Mavo.subset(this.data, this.inPath, env.data)), Mavo.hooks.run("node-getdata-end", env), env.data);
}sneak(callback) {
...
}$.bind(this.editor, {"input change":() => {
this.value=this.editorValue;
}, "mv-change":evt => {
"output"===evt.property&&(evt.stopPropagation(), $.fire(this.editor, "input"));
}});var multiline=this.editor.matches("textarea");multiline||this.editor.addEventListener("focus", () => {
this.editor.select&&this.editor.select();
}), "placeholder"in this.editor&&(this.editor.placeholder=`(${this.label})`), Mavo.attributeStartsWith("mv-edit-", this.element).forEach(attribute => {
...
var wasEditing=this.editing;if (!1===super.edit()) {
return !1;
} if (wasEditing&&!this.initEdit) {
return this.preEdit;
} this.preEdit||(this.preEdit=Mavo.promise(), this.afterPreEdit=null, this.preEdit.then(() => {
$.unbind(this.element, ".mavo:preedit"), requestAnimationFrame(() => {
if (!this.popup&&this.closestCollection&&this.editor&&this.editor.matches(Mavo.selectors.textInput)) {
var multiline=this.editor.matches("textarea");multiline||$.bind(this.editor, "paste.mavo:edit", evt => {
if (this.closestCollection.editing&&evt.clipboardData) {
var text=evt.clipboardData.getData("text/plain");const CRLF=/\r?\n|\r/;if (CRLF.test(text)) {
evt.preventDefault();var lines=text.split(CRLF);this.editor.setRangeText(lines[0]), $.fire(this.editor, "input");for (var collection=this.closestCollection, index=closestItem&&closestItem.index||0, i=1;i<lines.length;i++) {
var closestItem=this.closestItem, next=collection.add(void 0, index+i);collection.editItem(next);var copy=this.getCousin(i);copy.render(lines[i]);
}
}
}
}), $.bind(this.editor, "keydown.mavo:edit", evt => {
if (this.closestCollection.editing&&-1!==!["Backspace", "Enter"].indexOf(evt.key)) {
if ("Enter"==evt.key&&(evt.shiftKey||!multiline)) {
if (this.bottomUp) {
return;
} var closestItem=this.closestItem, next=this.closestCollection.add(void 0, closestItem&&closestItem.index+1);this.closestCollection.editItem(next);var copy=this.getCousin(1);requestAnimationFrame(() => {
copy.edit({immediately:!0}).then(() => copy.editor.focus());
}), multiline&&evt.preventDefault();
}
else if ("Backspace"==evt.key&&(this.empty||evt[Mavo.superKey])) {
var sibling=this.getCousin(1)||this.getCousin(-1);this.closestCollection.delete(this.closestItem), sibling&&sibling.edit({immediately:!0}).then(() => sibling.editor.focus()), evt.preventDefault();
}
}
});
}
});
})), wasEditing||(-1===this.element.tabIndex&&Mavo.revocably.setAttribute(this.element, "tabindex", "0"), this.element.classList.add("mv-pending-edit"), !this.modes&&$.bind(this.element, "click.mavo:edit", evt => evt.preventDefault()));var events=["mousedown", "focus", "dragover", "dragenter"].map(e => e+".mavo:preedit").join(" ");return ($.bind(this.element, events, evt => this.preEdit.resolve(evt)), o.immediately&&this.preEdit.resolve(), this.config.edit)?(this.config.edit.call(this), this.initEdit=null, this.preEdit.resolve()):(this.afterPreEdit||(this.afterPreEdit=this.preEdit.then(evt => {
this.sneak(() => {
this.element.classList.remove("mv-pending-edit"), this.initEdit&&this.initEdit(), this.popup&&(this.popup.prepare(), this.popup.show()), this.attribute||this.popup||(this.editor.parentNode!=this.element&&(this.editorValue=this.value, this.config.hasChildren?this.element.textContent="":_.setText(this.element, ""), this.element.prepend(this.editor)), (evt&&"mousedown"==evt.type||document.activeElement===this.element)&&requestAnimationFrame(() => this.editor.focus()), !this.collection&&Mavo.revocably.restoreAttribute(this.element, "tabindex"));
});
})), this.afterPreEdit);
}done() {
return !1!==super.done()&&void(this.afterPreEdit=null, "preEdit"in this&&$.unbind(this.element, ".mavo:preedit .mavo:edit"), this.sneak(() => this.config.done?void this.config.done.call(this):void(this.popup?this.popup.close():!this.attribute&&this.editor&&($.remove(this.editor), _.setValue(this.element, this.editorValue, {config:this.config, attribute:this.attribute, datatype:this.datatype, map:this.originalEditor||this.editor, node:this})))), !this.collection&&Mavo.revocably.restoreAttribute(this.element, "tabindex"));
}dataRender(data, {live, root}={}) {
var previousValue=this._value;if ("object"===$.type(data)) {
if (Symbol.toPrimitive in data) {
data=data[Symbol.toPrimitive]("default");
}
else if (!this.isHelperVariable&&Mavo.isPlainObject(data)) {
var property, properties=Object.keys(data);if (1===properties.length) {
...
let type=$.type(data[p]);if (type===this.datatype||!this.datatype&&"string"==type) {
property=p;break;
}
}
...
return this.sneak(() => {
void 0===value&&(value=null);var oldDatatype=this.datatype;return this.datatype||"number"!=typeof value&&"boolean"!=typeof value||(this.datatype=typeof value), value=_.safeCast(value, this.datatype), o.force||value!==this._value||oldDatatype!=this.datatype?void(this.editor&&this.editorValue!=value&&(this.editorValue=value), (this.popup||!this.editor||this.editor!==document.activeElement&&!this.element.contains(this.editor))&&(this.config.setValue?this.config.setValue.call(this, this.element, value):!o.dataOnly&&_.setValue(this.element, value, {config:this.config, attribute:this.attribute, datatype:this.datatype, map:this.originalEditor||this.editor, node:this})), this.empty=!value&&0!==value, this._value=value, this.liveData.update(), !o.silent&&(this.saved&&(this.unsavedChanges=this.mavo.unsavedChanges=!0), this.dataChanged("propertychange", {value}))):value;
}), value;
}dataChanged(action="propertychange", o) {
return super.dataChanged(action, o);
}upload(file, name=file.name) {
if (this.mavo.uploadBackend&&self.FileReader) {
var tempURL=URL.createObjectURL(file);this.sneak(() => this.element.setAttribute(this.attribute, tempURL));var path=this.element.getAttribute("mv-upload-path")||"", relative=path+"/"+name;this.mavo.upload(file, relative).then(url => {
var base=Mavo.getClosestAttribute(this.element, "mv-upload-url");base&&(url=new URL(relative, new URL(base, location))+""), this.value=url, this.element.matches("a")||this.sneak(() => this.element.setAttribute(this.attribute, tempURL));
});
}
}createUploadPopup(type, kind="file", ext) {
var env={context:this, type, kind, ext, mainInput:$.create("input", {type:"url", placeholder:`http://example.com/${kind}.${ext}`, className:"mv-output", "aria-label":`URL to ${kind}`})};if (this.mavo.uploadBackend&&self.FileReader) {
var checkType=file => file&&(!type||0===file.type.indexOf(type.replace("*", "")));return env.events={paste:evt => {
var item=evt.clipboardData.items[0], ext=item.type.split("/")[1];if ("file"==item.kind&&checkType(item)) {
var defaultName=`pasted-${kind}-${Date.now()}.${ext}`, name=prompt(this.mavo._("filename"), defaultName);""===name&&(name=defaultName), null!==name&&(this.upload(item.getAsFile(), name, type), evt.preventDefault());
}
}, "drag dragstart dragend dragover dragenter dragleave drop":evt => {
evt.preventDefault(), evt.stopPropagation();
}, "dragover dragenter":() => {
env.popup.classList.add("mv-dragover"), this.element.classList.add("mv-dragover");
}, "dragleave dragend drop":() => {
env.popup.classList.remove("mv-dragover"), this.element.classList.remove("mv-dragover");
}, drop:evt => {
var file=evt.dataTransfer.files[0];file&&checkType(file)&&this.upload(file);
}}, Mavo.hooks.run("primitive-createuploadpopup-beforecreate", env), env.popup=$.create({className:"mv-upload-popup", contents:[env.mainInput, {tag:"input", type:"file", "aria-label":`Upload ${kind}`, accept:type, events:{change:evt => {
var file=evt.target.files[0];file&&checkType(file)&&this.upload(file);
...
var ret=element.getAttribute("mv-attribute")||config.attribute;return ret&&"null"!==ret&&"none"!==ret||(ret=null), ret;
}static safeCast(value, datatype) {
var cast=_.cast(value, datatype);return "boolean"==datatype?!!value&&(!!("true"===value||0<value)||value):"number"==datatype?/^[-+]?[0-9.e]+$/i.test(value+"")?cast:value:null===value||void 0===value?value:cast;
...
if (config||(config=_.getConfig(element, attribute)), attribute=config.attribute, datatype=config.datatype, config.getValue&&attribute==config.attribute) {
...
if (1===element.nodeType&&(o.config||(o.config=_.getConfig(element, o.attribute)), o.attribute=void 0===o.attribute?o.config.attribute:o.attribute, o.datatype=void 0===o.datatype?o.config.datatype:o.datatype, o.config.setValue&&o.attribute==o.config.attribute)) {
...
catch (e) {}previousValue!=newValue&&o.config.changeEvents&&o.config.changeEvents.split(/\s+/).forEach(type => $.fire(element, type));
...
for (var option, i=0;option=o.map.options[i];i++) {
...
};$.Class(_, {lazy:{emptyValue:function() {
...
this._config!==config&&(this._config=config, this.updateObserver());
}, attribute:function(attribute) {
this._attribute!==attribute&&(this._attribute=attribute, this.updateObserver());
...
}}, static:{all:new WeakMap, lazy:{formatNumber:() => {
...
this.primitive=primitive, this.position=() => {
var bounds=this.primitive.element.getBoundingClientRect(), x=bounds.left, y=bounds.bottom, pointDown=!1;if (this.element.offsetHeight&&(this.height=this.element.getBoundingClientRect().height||this.height), this.height+y+20>innerHeight) {
if (20<bounds.top-this.height) {
var pointDown=!0;y=bounds.top-this.height-20;
}
else {
y=innerHeight-this.height-20;
}
} this.element.classList.toggle("mv-point-down", pointDown), $.style(this.element, {top:`${y}px`, left:`${x}px`});
}, this.element=$.create("div", {className:"mv-popup", hidden:!0, contents:{tag:"fieldset", contents:[{tag:"legend", textContent:this.primitive.label+":"}, this.editor]}, events:{keyup:evt => {
(13==evt.keyCode||27==evt.keyCode)&&(this.element.contains(document.activeElement)&&this.primitive.element.focus(), evt.stopPropagation(), this.hide());
}, transitionend:this.position}}), this.editor.matches("select")&&(this.editor.size=Math.min(10, this.editor.children.length)), this.hideCallback=evt => {
this.element.contains(evt.target)||this.primitive.element.contains(evt.target)||this.hide();
};
}, show:function() {
$.unbind([this.primitive.element, this.element], ".mavo:showpopup"), this.shown=!0, this.element.style.transition="none", this.element.removeAttribute("hidden"), this.position(), this.element.setAttribute("hidden", ""), this.element.style.transition="", document.body.appendChild(this.element), setTimeout(() => {
this.element.removeAttribute("hidden");
}, 100), $.bind(document, "focus click", this.hideCallback, !0), window.addEventListener("scroll", this.position, {passive:!0});
}, hide:function() {
$.unbind(document, "focus click", this.hideCallback, !0), window.removeEventListener("scroll", this.position, {passive:!0}), this.element.setAttribute("hidden", ""), this.shown=!1, setTimeout(() => {
$.remove(this.element);
}, 1e3*parseFloat(getComputedStyle(this.element).transitionDuration)||400);
}, prepare:function() {
$.bind(this.primitive.element, {"click.mavo:edit":() => {
this.show();
}, "keyup.mavo:edit":evt => {
-1<[13, 113].indexOf(evt.keyCode)&&(this.show(), this.editor.focus());
}});
}, close:function() {
...
} return config.selector=config.selector||id, config.id=id, Array.isArray(config.attribute)?config.attribute.forEach(attribute => {
var o=$.extend({}, config);o.attribute=attribute, _[`${id}@${attribute}`]=o;
}):_[id]=config, _;
...
}}, "video, audio":{attribute:["autoplay", "buffered", "loop"], datatype:"boolean"}, details:{attribute:"open", datatype:"boolean"}, "input, select, button, textarea":{attribute:"disabled", datatype:"boolean"}, formControl:{selector:"input", default:!0, attribute:"value", modes:"edit", changeEvents:"input change", edit:() => {}, done:() => {}, init:function() {
this.editor=this.element;
}}, select:{extend:"formControl", selector:"select", subtree:!0}, "select[multiple]":{extend:"select", selector:"select[multiple]", getValue:element => Array.from(element.selectedOptions).map(option => option.value).join(), setValue:(element, value) => {
value=Array.isArray(value)?value:(value+"").split(/\s*,/), Array.from(element.options).forEach(option => {
option.selected=!1, value=value.map(v => v+""), value.includes(option.value)&&(option.selected=!0);
});
}}, textarea:{extend:"formControl", selector:"textarea", attribute:null, getValue:element => element.value, setValue:(element, value) => element.value=value}, formNumber:{extend:"formControl", selector:"input[type=range], input[type=number]", datatype:"number", setValue:function(element, value) {
...
var observer=new Mavo.Observer(element, attribute, () => {
element.value=value;
});requestAnimationFrame(() => {
...
}}, checkbox:{extend:"formControl", selector:"input[type=checkbox]", attribute:"checked", datatype:"boolean", changeEvents:"click"}, radio:{extend:"formControl", selector:"input[type=radio]", attribute:"checked", modes:"edit", getValue:element => {
if (element.form) {
return element.form[element.name].value;
} var checked=$(`input[type=radio][name="${element.name}"]:checked`);return checked&&checked.value;
}, setValue:(element, value) => {
if (element.form) {
return void(element.form[element.name].value=value);
} var toCheck=$(`input[type=radio][name="${element.name}"][value="${value}"]`);$.properties(toCheck, {checked:!0});
}, init:function(element) {
this.mavo.element.addEventListener("change", evt => {
evt.target.name==element.name&&(this.value=this.getValue());
});
}}, counter:{extend:"formControl", selector:"button, .counter", attribute:"mv-clicked", datatype:"number", init:function(element) {
"mv-clicked"===this.attribute&&(element.setAttribute("mv-clicked", "0"), element.addEventListener("click", () => {
let clicked=+element.getAttribute("mv-clicked")||0;this.value=++clicked;
}));
}}, "meter, progress":{default:!0, attribute:"value", datatype:"number", edit:function() {
var min=+this.element.getAttribute("min")||0, max=+this.element.getAttribute("max")||1, range=max-min, step=+this.element.getAttribute("mv-edit-step")||(1<range?1:range/100);$.bind(this.element, "mousemove.mavo:edit", evt => {
var left=this.element.getBoundingClientRect().left, offset=_Mathmax2(0, (evt.clientX-left)/this.element.offsetWidth), newValue=min+range*offset, mod=newValue%step;newValue+=mod>step/2?step-mod:-mod, newValue=_Mathmax2(min, _Mathmin2(newValue, max)), this.sneak(() => this.element.setAttribute("value", newValue));
...
if (evt.target==this.element&&(37==evt.keyCode||39==evt.keyCode)) {
var increment=step*(39==evt.keyCode?1:-1)*(evt.shiftKey?10:1), newValue=this.value+increment;newValue=_Mathmax2(min, _Mathmin2(newValue, max)), this.element.setAttribute("value", newValue), evt.preventDefault();
}
});
}, done:function() {
$.unbind(this.element, ".mavo:edit");
}}, meta:{default:!0, attribute:"content"}, block:{default:!0, selector:"p, div, dt, dd, h1, h2, h3, h4, h5, h6, article, section, address, pre", editor:function() {
...
} this.dateType=type, dateFormat||(this.element.textContent=this.config.defaultFormats[this.dateType](this.property), this.mavo.expressions.extract(this.element, null), (dateFormat=Mavo.DOMExpression.search(this.element, null))&&this.mavo.treeBuilt.then(() => {
dateFormat.update();
}));
}
}, dateTypes:{month:/^[Y\d]{4}-[M\d]{2}$/i, time:/^[H\d]{2}:[M\d]{2}/i, "datetime-local":/^[Y\d]{4}-[M\d]{2}-[D\d]{2} [H\d]{2}:[Mi\d]{2}/i, date:/^[Y\d]{4}-[M\d]{2}-[D\d]{2}$/i}, defaultFormats:{date:name => `[day(${name})] [month(${name}, 'shortname')] [year(${name})]`, month:name => `[month(${name}, 'name')] [year(${name})]`, time:name => `[hour(${name}, '00')]:[minute(${name}, '00')]`, "datetime-local":function(name) {
...
Mavo.revocably.setAttribute(this.element, "role", "checkbox"), $.bind(this.element, "click.mavo:edit keyup.mavo:edit keydown.mavo:edit", evt => {
("click"==evt.type||" "==evt.key||"Enter"==evt.key)&&("keydown"!=evt.type&&(this.value=!this.value), evt.preventDefault(), evt.stopPropagation());
});
}, done:function() {
Mavo.revocably.restoreAttribute(this.element, "role"), $.unbind(this.element, ".mavo:edit");
}}});
}(Bliss, Bliss.$), function($, $$) {
Mavo.attributes.push("mv-multiple", "mv-order", "mv-accepts", "mv-initial-items", "mv-like");var _=Mavo.Collection=class extends Mavo.Node {
constructor(element, mavo, o) {
super(element, mavo, o), this.templateElement=this.element, this.children=[], this.liveData=new Mavo.Data(this, []), this.marker=document.createComment("mv-marker"), Mavo.data(this.marker, "collection", this), this.templateElement.after(this.marker), this.addButton=this.createAddButton(), this.mavo.root||!this.templateElement.hasAttribute("mv-like")?this.init():this.mavo.treeBuilt.then(() => this.init());
}createAddButton() {
var selector=`button.mv-add-${this.property}`, group=this.parentGroup.element, button=$$(selector, group).filter(button => !this.templateElement.contains(button)&&!Mavo.data(button, "collection"))[0];return button?(button.compareDocumentPosition(this.marker)&Node.DOCUMENT_POSITION_FOLLOWING&&Mavo.setAttributeShy(this.templateElement, "mv-order", "desc"), Mavo.revocably.remove(button)):button=$.create("button", {type:"button", className:"mv-ui", textContent:this.mavo._("add-item", this)}), button.classList.add("mv-add", `mv-add-${this.property}`), Mavo.data(button, "collection", this), Mavo.setAttributeShy(button, "mv-action", `add(${this.property})`), button;
}init() {
...
var candidates=[];this.mavo.walk(obj => {
obj instanceof _&&obj.property===this.like&&obj!==this&&candidates.push(obj);
}), 0<candidates.length?(this.likeNode=candidates.sort((a, b) => a.pathFrom(this).length-b.pathFrom(this).length)[0], this.likeNode=this.likeNode.likeNode||this.likeNode, this.likeNode=this.likeNode.template||this.likeNode):this.like=null;
...
item?this.delete(item, {silent:!0}):this.element.remove();
}
...
var env={context:this, options:o};return env.data=this.children.map(item => item.getData(env.options)).filter(itemData => null!==Mavo.value(itemData)), env.data=Mavo.subset(this.data, this.inPath, env.data), Mavo.hooks.run("node-getdata-end", env), env.data;
}createItem(element) {
element||(element=this.templateElement.cloneNode(!0));var template=this.itemTemplate||(this.template?this.template.itemTemplate:null), item=Mavo.Node.create(element, this.mavo, {collection:this, template, property:this.property, type:this.type});return this.itemTemplate||(this.itemTemplate=template||item), item;
}add(item, index, o={}) {
item=item instanceof Node?Mavo.Node.get(item)||this.createItem(item):item||this.createItem(), item.collection!=this&&this.adopt(item), void 0===index&&(index=this.bottomUp?0:this.length);var rel=this.children[index]?this.children[index].element:this.marker;$.before(item.element, rel);var env={context:this, item};return env.previousIndex=item.index, env.changed=this.splice({remove:env.item}, {index:index, add:env.item}), this.mavo.expressions.active&&!o.silent&&requestAnimationFrame(() => {
env.changed.forEach(i => {
i.dataChanged(i==env.item&&void 0===env.previousIndex?"add":"move"), i.unsavedChanges=!0;
}), this.unsavedChanges=this.mavo.unsavedChanges=!0, this.mavo.expressions.update(env.item);
}), Mavo.hooks.run("collection-add-end", env), env.item;
}splice(...actions) {
actions.forEach(action => {
void 0===action.index&&action.remove&&isNaN(action.remove)&&(action.index=this.children.indexOf(action.remove), action.remove=1);
}), actions.sort((a, b) => b.index-a.index);var changed=[], deleted=[];actions.forEach(action => {
-1<action.index&&(action.remove||action.add)&&(action.remove=action.remove||0, action.add=Mavo.toArray(action.add), deleted.push(...this.children.splice(action.index, +action.remove, ...action.add)));
}), deleted=new Set(deleted);for (let item, i=0;i<this.length;i++) {
item=this.children[i], deleted.delete(item), item&&item.index!==i&&(item.index=i, changed.push(item));
} return deleted.forEach(item => {
item.expressions&&item.expressions.forEach(domexpression => {
item.mavo.expressions.unregister(domexpression);
});
}), this.liveData.update(), changed;
}adopt(item) {
item.collection&&(item.collection.splice({remove:item}), item.collection.dataChanged("delete")), item.collection=this, this.walk(obj => {
obj.closestCollection===item.collection&&(obj.closestCollection=this), item.mavo!=this.mavo&&(obj.mavo=this.mavo), obj.path=obj.getPath();
...
if (item.element.classList.remove("mv-highlight"), this.splice({remove:item}), !silent&&transition) {
...
} return stage2.then(() => ($.remove(item.element), silent||(this.unsavedChanges=item.unsavedChanges=this.mavo.unsavedChanges=!0, item.collection.dataChanged("delete", {index:item.index})), undoable?this.mavo.setDeleted(item):destroy&&item.destroy(), item));
}move(item, offset) {
var index=item.index+offset+(0<offset);index=Mavo.wrap(index, this.children.length+1), this.add(item, index);
}editItem(item, o={}) {
var when=o.immediately?Promise.resolve():Mavo.inView.when(item.element);return when.then(() => (item.itembar||(item.itembar=new Mavo.UI.Itembar(item)), item.itembar.add(), item.edit(o)));
}edit(o={}) {
if (!1===super.edit()) {
return !1;
...
}rel=rel||this.marker, Mavo.revocably.add(this.addButton, e => $[this.bottomUp?"before":"after"](e, rel));
} return _.dragula.then(() => {
this.getDragula();
}), Promise.all(this.children.map(item => this.editItem(item, o)));
}done() {
return !1!==super.done()&&void(Mavo.revocably.remove(this.addButton), this.propagate(item => {
item.itembar&&item.itembar.remove();
}));
}dataChanged(action, o={}) {
return o.element=o.element||this.marker, super.dataChanged(action, o);
}dataRender(data, o={}) {
if (void 0!==data) {
data=null===data?[]:Mavo.toArray(data).filter(i => null!==i);for (var item, changed=!1, i=0;i<this.children.length;i++) {
item=this.children[i], i<data.length?changed=item.render(data[i], o)||changed:(changed=!0, this.delete(item, {silent:!0}), i--);
} if (data.length>i) {
for (var item, fragment=document.createDocumentFragment(), j=i;j<data.length;j++) {
item=this.createItem(), changed=item.render(data[j], o)||changed, this.children.push(item), item.index=j, fragment.appendChild(item.element);var env={context:this, item};Mavo.hooks.run("collection-add-end", env);
...
super.destroy(), this.dragula&&(this.dragula.destroy(), this.dragula=null), this.propagate("destroy");
}getDragula() {
if (this.dragula) {
return this.dragula;
} if (this.template) {
...
} this;return this.dragula=dragula({containers:[this.marker.parentNode], isContainer:el => !!this.accepts.size&&Array.from(el.childNodes).some(child => {
var collection=_.get(child);return collection&&this.accepts.has(collection.property);
}), moves:(el, container, handle) => handle.classList.contains("mv-drag-handle")&&handle.closest(Mavo.selectors.multiple)==el, accepts:function(el, target, source, next) {
...
var deleted=new Mavo.BucketMap({arrays:!0}), collections=new Set, options={silent:!0, undoable:!1, destroy:!1}, promises=nodes.filter(node => !!node.collection).map(node => (collections.add(node.collection), node.collection.delete(node, options).then(node => deleted.set(node.mavo, node))));o.silent||Promise.all(promises).then(() => {
collections.forEach(collection => {
collection.dataChanged("delete");
}), !1!==o.undoable&&deleted.forEach((nodes, mavo) => {
mavo.setDeleted(...nodes);
});
});
}
};$.Class(_, {lazy:{bottomUp:function() {
return /^desc\b/i.test(this.templateElement.getAttribute("mv-order"));
}}, static:{dragulas:[], lazy:{dragula:() => $.include(self.dragula, "https://cdnjs.cloudflare.com/ajax/libs/dragula/3.7.2/dragula.min.js")}}});
}(Bliss, Bliss.$), function() {
Mavo.ImplicitCollection=class extends Mavo.Node {
constructor(element, mavo, o) {
super(element, mavo, o), this.children=[], this.liveData=new Mavo.Data(this, []), this.add(element), this.postInit(), Mavo.hooks.run("implicit-collection-init-end", this);
} get length() {
return this.children.length;
}getData(o={}) {
var env={context:this, options:o, data:[]};if (this.children.forEach(node => {
...
return !1!==super.edit()&&Promise.all(this.children.map(item => item.edit(o)));
}dataRender(data, o={}) {
if (data!==void 0) {
data=null===data?[]:Mavo.toArray(data).filter(i => null!==i);var changed=data.length!==this.liveData.length;this.children.forEach((item, i) => changed=item.render(data&&data[i], o)||changed);
} this.liveData.update();
}
};
}(Bliss, Bliss.$), function($, $$) {
var _=Mavo.UI.Itembar=$.Class({constructor:function(item) {
if (this.item=item, this.element=$$(`.mv-item-bar:not([mv-rel]), .mv-item-bar[mv-rel="${this.item.property}"]`, this.item.element).filter(el => el.closest(Mavo.selectors.multiple)==this.item.element&&!Mavo.data(el, "item"))[0], !this.element&&this.item.template&&this.item.template.itembar) {
...
} this.element.setAttribute("hidden", ""), $.bind([this.item.element, this.element], "focusin mouseover", this), $.bind(this.element, {mouseenter:() => {
...
evt.target===this.dragHandle&&this.item.editing&&37<=evt.keyCode&&40>=evt.keyCode&&(this.collection.move(this.item, 38>=evt.keyCode?-1:1), evt.stopPropagation(), evt.preventDefault(), evt.target.focus());
}), this.dragHandle!==this.item.element&&this.dragHandle.addEventListener("click", evt => evt.target.focus()), Mavo.data(this.element, "item", this.item);
}, destroy:function() {
this.hide();
}, show:function(sticky) {
_.visible.forEach(instance => {
instance!=this&&(!this.sticky||instance.sticky)&&(clearTimeout(instance.hideTimeout), instance.hide(sticky, _.DELAY));
...
(!this.sticky||sticky)&&(timeout?this.hideTimeout=setTimeout(() => this.hide(sticky), timeout):(this.element.setAttribute("hidden", ""), $.unbind([this.item.element, this.element], "focusout mouseleave", this), this.sticky=!1, _.visible.delete(this)));
...
if (Mavo.hooks.run("expression-eval-beforeeval", this), !this.function) {
try {
this.function=Mavo.Script.compile(this.expression, o);
}
catch (error) {
return this.error(`There is something wrong with the expression ${this.expression}`, error.message, "Not an expression? See https://mavo.io/docs/expressions/#disabling-expressions for information on how to disable expressions."), Mavo.hooks.run("expression-compile-error", {context:this, error}), this.function=error;
}
}
...
return this.error(`Something went wrong with the expression ${this.expression}`, error.message, `Data was: ${JSON.stringify(data)}`), Mavo.hooks.run("expression-eval-error", {context:this, error}), error;
}
}, error(title, ...message) {
...
this.start=start, this.end=end, this.regex=RegExp(`${Mavo.escapeRegExp(start)}([\\S\\s]*?)${Mavo.escapeRegExp(end)}`, "gi");
}, test:function(str) {
return this.regex.lastIndex=0, this.regex.test(str);
}, tokenize:function(str) {
var match, ret=[], lastIndex=0;for (this.regex.lastIndex=0;null!==(match=this.regex.exec(str));) {
...
return syntax=syntax.trim(), /\s/.test(syntax)?new _.Syntax(...syntax.split(/\s+/)):_.Syntax.ESCAPE;
}
}
}, ESCAPE:-1}}), _.Syntax.default=new _.Syntax("[", "]");
}(Bliss, Bliss.$), function($) {
var _=Mavo.DOMExpression=$.Class({constructor:function(o={}) {
this.mavo=o.mavo, this.template=o.template&&o.template.template||o.template;for (let prop of ["item", "path", "syntax", "fallback", "attribute", "originalAttribute", "expression", "parsed", "identifiers"]) {
this[prop]=void 0===o[prop]&&this.template?this.template[prop]:o[prop];
...
} this.oldValue=this.value=this.parsed.map(x => x instanceof Mavo.Expression?x.expression:x), this.identifiers=this.identifiers||Mavo.flatten(this.parsed.map(x => x.identifiers||[])), _.special.add(this), this.mavo.treeBuilt.then(() => {
this.template||this.item||(this.item=Mavo.Node.getClosestItem(this.element)), "mv-value"==this.originalAttribute&&this.mavoNode&&this.mavoNode==this.item.collection&&Mavo.delete(this.item.expressions, this), this.mavo.expressions.register(this), Mavo.hooks.run("domexpression-init-treebuilt", this);
}), Mavo.hooks.run("domexpression-init-end", this), _.elements.set(this.element, [...(_.elements.get(this.element)||[]), this]);
}, destroy:function() {
...
var env={context:this}, parentEnv=env;if (this.item) {
...
}Mavo.hooks.run("domexpression-update-start", env), this.oldValue=this.value;var changed=!1;env.value=this.value=this.parsed.map(expr => {
if (expr instanceof Mavo.Expression) {
var env={context:this, expr, parentEnv};return Mavo.hooks.run("domexpression-update-beforeeval", env), env.value=Mavo.value(env.expr.eval(data)), Mavo.hooks.run("domexpression-update-aftereval", env), changed=!0, env.value instanceof Error?void 0===this.fallback?this.syntax.start+env.expr.expression+this.syntax.end:this.fallback:void 0===env.value||null===env.value?"":env.value;
} return expr;
});changed&&(env.value=1===env.value.length?env.value[0]:env.value.map(v => Mavo.Primitive.format(v, {attribute:this.attribute, element:this.element})).join(""), this.output(env.value), Mavo.hooks.run("domexpression-update-end", env));
...
}, live:{item:function(item) {
item&&this._item!=item&&(this._item&&Mavo.delete(this._item.expressions, this), item.expressions=item.expressions||[], item.expressions.push(this));
}}, static:{elements:new WeakMap, search:function(element, attribute) {
if (null===element) {
return element;
}attribute&&!element.ownerDocument.xmlVersion&&(attribute=attribute.toLowerCase());var all=_.elements.get(element)||[];return 1<arguments.length?all.length?all.filter(et => et.attribute===attribute)[0]||null:null:all;
...
this.update&&this.update(...arguments), this.all.forEach(domexpression => domexpression.update());
}, event:function(name, {type, update, target=document}={}) {
this.vars[name]={observe:function() {
...
var callback=() => {
_.special.update.call(this), this.timer=requestAnimationFrame(callback);
...
this.mavo=mavo, this.active=!0, this.expressions=[], this.identifiers={};var syntax=Mavo.Expression.Syntax.create(this.mavo.element.closest("[mv-expressions]"))||Mavo.Expression.Syntax.default;this.traverse(this.mavo.element, void 0, syntax), this.scheduled={}, this.mavo.treeBuilt.then(() => {
this.expressions=[], this.update();
});
}, register:function(domexpression) {
var ids=this.identifiers;domexpression.registeredApp=domexpression.registeredApp||new Set, domexpression.identifiers.forEach(id => {
ids[id]instanceof Set||(ids[id]=new Set), ids[id].add(domexpression), Mavo.all[id]instanceof Mavo&&Mavo.all[id]!==this.mavo&&!domexpression.registeredApp.has(id)&&(domexpression.registeredApp.add(id), Mavo.all[id].expressions.register(domexpression));
});
}, unregister:function(domexpression) {
var ids=this.identifiers;domexpression.identifiers.forEach(id => {
...
if (this.active) {
var scheduled=this.scheduled[evt.action]=this.scheduled[evt.action]||new Set;evt.node.template?!scheduled.has(evt.node.template)&&(setTimeout(() => {
scheduled.delete(evt.node.template), this.update(evt);
}, _.THROTTLE), scheduled.add(evt.node.template)):requestAnimationFrame(() => this.update(evt));
}
}, update:function(evt) {
if (this.active) {
...
}rootObject.walk(obj => !!obj.expressionsEnabled&&void(obj.expressions&&obj.expressions.forEach(et => {
...
var scheduled=this.scheduledIds=this.scheduledIds||new Set;scheduled.has(property)||(setTimeout(() => {
scheduled.delete(property), this.updateById(property, evt, cache);
...
} var exprs=this.identifiers[property];exprs&&exprs.forEach(expr => {
...
attribute&&-1<_.skip.indexOf(attribute.name)||(attribute&&-1<_.directives.indexOf(attribute.name)||syntax!==Mavo.Expression.Syntax.ESCAPE&&syntax.test(attribute?attribute.value:node.textContent))&&(path===void 0&&(path=Mavo.elementPath(node.closest(Mavo.selectors.item), node)), this.expressions.push(new Mavo.DOMExpression({node, syntax, path, attribute:attribute&&attribute.name, mavo:this.mavo})));
...
ignore&&ignore.has(attribute.name)||this.extract(node, attribute, path, syntax);
});var index=-1, offset=0;node.matches("script:not([mv-expressions])")||$$(node.childNodes).forEach(child => {
if (1==child.nodeType?(offset=0, index++):offset++, 1==child.nodeType||3==child.nodeType) {
var segment=0<offset?`${index}.${offset}`:index;this.traverse(child, [...(path||[]), segment], syntax);
}
});
}
}
...
var properties=$$(Mavo.selectors.property, this.element).filter(el => el.closest("[mv-if]")==this.element).map(el => Mavo.Node.get(el));return this.element.addEventListener("mv-change", evt => {
requestAnimationFrame(() => {
this.element.parentNode||this.item.element.dispatchEvent(evt);
});
}), properties;
...
var value=this.value[0], oldValue=this.oldValue[0];this.item.mavo.treeBuilt.then(() => {
if (this.parentIf) {
var parentValue=this.parentIf.value[0];this.value[0]=value=value&&parentValue;
}!1!==parentValue&&(value?Mavo.revocably.add(this.element):this.element.parentNode&&Mavo.revocably.remove(this.element, "mv-if")), value!==oldValue&&(this.childProperties&&this.childProperties.forEach(property => property.hidden=!value), this.childIfs&&this.childIfs.forEach(childIf => childIf.update()));
});
}
...
var _Mathround=Math.round, _Mathfloor=Math.floor, _Mathabs=Math.abs, _Mathmin3=Math.min;function str(str="") {
return str=val(str), str||0===str?str+"":"";
} function empty(v) {
return v=Mavo.value(v), null===v||!1===v||""===v;
} function not(v) {
return !val(v);
} var _=Mavo.Functions={operators:{"=":"eq"}, get:function(obj, property, meta={}) {
property=meta.property=val(property);var canonicalProperty=Mavo.getCanonicalProperty(obj, property);if (canonicalProperty!==void 0) {
...
var eqIndex=property.indexOf("=");return -1<eqIndex?(meta.query={property:property.slice(0, eqIndex), value:property.slice(eqIndex+1)}, meta.property=[], ret=obj.filter((e, i) => {
var passes=_.get(e, meta.query.property)==meta.query.value;return passes&&meta.property.push(i), passes;
}), "id"==meta.query.property&&(meta.property=meta.property[0], ret=ret[0]), void 0===ret?meta.property=obj.length:0===ret.length&&(meta.property=[obj.length]), ret):obj.map(e => _.get(e, property));
} return null;
}, url:(id, url=location) => {
if (id===void 0) {
return location.href;
} if (id) {
id=str(id).replace(/[^\w-:]/g);var ret=url.search.match(RegExp(`[?&]${id}(?:=(.+?))?(?=$|&)`))||url.pathname.match(RegExp(`(?:^|\\/)${id}\\/([^\\/]*)`));
...
return _.first(_Mathabs(n), arr);
} for (var ret=[], numReturn=void 0===n?1:_Mathfloor(n), i=arr.length-1;0<=i&&ret.length<numReturn;i--) {
null!==Mavo.value(arr[i])&&ret.push(arr[i]);
} return void 0===n?void 0===ret[0]?null:ret[0]:ret;
}, condense:arr => _.first(arr.length, arr), unique:function(arr) {
return Array.isArray(arr)?[...new Set(arr.map(val))]:arr;
}, intersects:function(arr1, arr2) {
if (arr1&&arr2) {
var set2=new Set(Mavo.toArray(arr2).map(val));return arr1=Mavo.toArray(arr1).map(val), !arr1.every(el => !set2.has(el));
}
}, sum:$.extend(function(array) {
return $u.numbers(array, arguments).reduce((prev, current) => +prev+(+current||0), 0);
...
var _Mathceil=Math.ceil;array=$u.numbers(array, arguments).sort((a, b) => a-b);var mi=(array.length-1)/2;return [m1, m2]=[array[_Mathfloor(mi)], array[_Mathceil(mi)]], (m1+m2)/2||0;
...
}, {isAggregate:!0}), atan2:$.extend((dividend, divisor) => Math.atan2(dividend, divisor), {multiValued:!0, rightUnary:b => b, default:1}), pow:$.extend((base, exponent) => Math.pow(base, exponent), {multiValued:!0, default:1}), imul:$.extend((a, b) => Math.imul(a, b), {multiValued:!0, default:1}), count:$.extend(function(array) {
...
}, round:$.extend((num, decimals) => not(num)||not(decimals)||!isFinite(num)?_Mathround(num):+(+num).toLocaleString("en-US", {useGrouping:!1, maximumFractionDigits:decimals}), {multiValued:!0, rightUnary:b => b, default:0}), ordinal:$.extend(num => {
if (empty(num)) {
return "";
} if (10>num||20<num) {
var ord=["th", "st", "nd", "rd", "th"][num%10];
} return ord||"th";
}, {multiValued:!0, alias:"th"}), digits:$.extend((digits, decimals, num) => {
...
}, {multiValued:!0}), iff:function(condition, iftrue=condition, iffalse=null) {
return Array.isArray(condition)?condition.map((c, i) => {
var ret=val(c)?iftrue:iffalse;return Array.isArray(ret)?ret[_Mathmin3(i, ret.length-1)]:ret;
}):val(condition)?iftrue:iffalse;
}, group:(...objects) => Object.assign(...objects), list:(...items) => Mavo.flatten(items), random:$.extend((min=0, max=100, step=1) => {
1==arguments.length&&(max=min, min=0);var rand=Math.random(), range=(max-min)/step;return _Mathfloor(rand*(range+1))*step+min;
}, {multiValued:!0}), range:(a, b, step) => {
step===void 0&&(b===void 0&&(b=a, a=0<=b?1:-1), step=a<=b?1:-1);var steps=_Mathfloor((b-a)/step+1);if (0>=steps||!isFinite(steps)) {
return [a];
} var ret=[];for (let i=0, n=a;i++<steps;n+=step) {
ret.push(n);
} return ret;
}, shuffle:list => {
if (Array.isArray(list)) {
for (var j, ret=list.slice(), i=ret.length-1;0<i;i--) {
j=_Mathfloor(Math.random()*(i+1)), [ret[i], ret[j]]=[ret[j], ret[i]];
} return ret;
} return list;
}, replace:$.extend((haystack, needle, replacement="", iterations=1) => {
if (Array.isArray(haystack)) {
return haystack.map(item => _.replace(item, needle, replacement));
} for (var prev, needleRegex=RegExp(Mavo.escapeRegExp(needle), "g"), ret=haystack, counter=0;ret!=prev&&counter++<iterations;) {
prev=ret, ret=ret.replace(needleRegex, replacement);
} return ret;
}, {multiValued:!0}), len:$.extend(text => str(text).length, {multiValued:!0}), contains:$.extend((haystack, needle) => {
let ret, haystackType=$.type(haystack);if ("object"===$.type(needle)) {
...
}, {multiValued:!0}), search:$.extend((haystack, needle) => (haystack=str(haystack), needle=str(needle), haystack&&needle?haystack.toLowerCase().indexOf(needle.toLowerCase()):-1), {multiValued:!0}), starts:$.extend((haystack, needle) => 0===_.search(str(haystack), str(needle)), {multiValued:!0}), ends:$.extend((haystack, needle) => {
[haystack, needle]=[str(haystack), str(needle)];var i=_.search(haystack, needle);return -1<i&&i===haystack.length-needle.length;
}, {multiValued:!0}), join:function(array, glue) {
return Mavo.toArray(array).filter(a => !empty(a)).join(str(glue));
}, idify:$.extend(readable => str(readable).normalize("NFD").replace(/[\u0300-\u036f]/g, "").replace(/[^\w\s-]/g, "").trim().replace(/\s+/g, "-").toLowerCase(), {multiValued:!0}), readable:$.extend(identifier => str(identifier).replace(/([a-z])([A-Z])(?=[a-z])/g, ($0, $1, $2) => $1+" "+$2.toLowerCase()).replace(/([a-z0-9])[_\/-](?=[a-z0-9])/g, "$1 ").replace(/^[a-z]/, $0 => $0.toUpperCase()), {multiValued:!0}), uppercase:$.extend(text => str(text).toUpperCase(), {multiValued:!0}), lowercase:$.extend(text => str(text).toLowerCase(), {multiValued:!0}), from:$.extend((haystack, needle) => _.between(haystack, needle), {multiValued:!0}), fromlast:$.extend((haystack, needle) => _.between(haystack, needle, "", !0), {multiValued:!0}), to:$.extend((haystack, needle) => _.between(haystack, "", needle), {multiValued:!0}), tofirst:$.extend((haystack, needle) => _.between(haystack, "", needle, !0), {multiValued:!0}), between:$.extend((haystack, from, to, tight) => {
[haystack, from, to]=[str(haystack), str(from), str(to)];var i1=from?haystack[tight?"lastIndexOf":"indexOf"](from):-1, i2=haystack[tight?"indexOf":"lastIndexOf"](to);return from&&-1===i1||-1===i2?"":haystack.slice(i1+1, -1!==i2&&to?i2:haystack.length);
}, {multiValued:!0}), phrase:$.extend(function($this, id, vars, lang) {
3===arguments.length&&"string"===$.type(vars)&&([lang, vars]=[vars]);var locale=lang?Mavo.Locale.get(lang):$this&&$this[Mavo.mavo]?$this[Mavo.mavo].locale:Mavo.Locale.default;return locale.phrase(id, vars);
}, {needsContext:!0}), filename:$.extend(url => Mavo.match(new URL(str(url), Mavo.base).pathname, /[^/]+?$/), {multiValued:!0}), json:data => Mavo.safeToJSON(data), split:$.extend((text, separator=/\s+/) => (text=str(text), text.split(separator)), {multiValued:!0}), log:(...args) => (console.log(...args.map(val)), args[0]), $mouse:{x:0, y:0}, get $hash() {
...
return array=Array.isArray(array)?array:args?$$(args):[array], array.filter(number => !isNaN(number)&&""!==val(number)&&null!==val(number)).map(n => +n);
}, postProcess:function(callback) {
var newCallback, multiValued=callback.multiValued;if (!0===multiValued||multiValued&&2===multiValued.length?newCallback=(...args) => {
var idxA=multiValued[0]||0, idxB=multiValued[1]||1;return Mavo.Script.binaryOperation(args[idxA], args[idxB], {scalar:(a, b) => (idxA in args&&(args[idxA]=a), idxB in args&&(args[idxB]=b), callback(...args)), ...callback});
}:callback.isAggregate&&(newCallback=function(array) {
if (Mavo.in(Mavo.groupedBy, array)) {
return array.map(e => newCallback(e.$items));
} var ret=callback.call(this, ...arguments);return void 0===ret?array:ret;
}), newCallback&&($.extend(newCallback, callback), newCallback.original=callback), callback.alias) {
for (let alias of Mavo.toArray(callback.alias)) {
Mavo.Functions[alias]=newCallback||callback;
}
} return newCallback;
}}}, $u=_.util;Mavo.ready.then(() => {
...
}, {multiValued:!0}), week:() => 1e3*s.weeks, day:$.extend(function() {
...
}, {multiValued:!0}), date:$.extend(date => (date=$u.date(date), date?`${_.year(date)}-${_.month(date, "00")}-${_.day(date, "00")}`:""), {multiValued:!0}), time:$.extend((date, precision="seconds") => {
if (date=$u.date(date), !date) {
return "";
} var ret=`${_.hour(date, "00")}:${"hours"==precision?"00":_.minute(date, "00")}`;return ("seconds"==precision||"ms"==precision)&&(ret+=`:${_.second(date, "00")}`, "ms"==precision&&(ret+=`.${_.ms(date)}`)), ret;
}, {multiValued:!0}), localTimezone:-new Date().getTimezoneOffset()}), _.msTo=(what, ms) => _Mathfloor2(_Mathabs2(ms)/(1e3*s[what]))||0, s) {
_[unit]=$.extend(function(ms) {
return 0===arguments.length?1e3*s[unit]:_.msTo(unit, ms);
}, {multiValued:!0});
}_.duration=$.extend(function($this, ms) {
1===arguments.length&&([ms, $this]=[$this, null]);var count=ms||0, unit="ms";for (let nextUnit in s) {
...
return "";
} var ret=ret||numeric[component](dateO);return format?/^0+$/.test(format)?(ret+"").padStart(format.length, "0").slice(-format.length):(format={name:"long", shortname:"short"}[format]||format, ret=dateO.toLocaleString(Mavo.locale, {[component]:format}), ret=ret.replace(/\u200e/g, ""), ret):"year"===component?ret:+ret;
}, date:function(date) {
if (date=val(date), !date) {
return null;
} var object=new Date(date);if ("string"!==$.type(date)||!isNaN(object)&&object+""==date) {
return object;
} if (date=$u.fixDateString(date), null===date) {
return null;
} var timezone=Mavo.match(date, /[+-]\d{2}:?\d{2}|Z$/);if (timezone) {
date=new Date(date);
}
else {
var fields=date.match(/\d+/g);date=new Date(fields[0], (fields[1]||1)-1, fields[2]||1, fields[3]||0, fields[4]||0, fields[5]||0, fields[6]||0);
} return isNaN(date)?null:date;
}});
}(Bliss, Mavo.value, Mavo.Functions), function($, val, $u) {
var _Mathmax3=Math.max, _=Mavo.Script={$fn:self.Proxy?new Proxy({[Symbol.unscopables]:{undefined:!0}}, {get:(data, property) => {
var ret, propertyL=property.toLowerCase();return propertyL in Mavo.Actions.Functions&&(Mavo.Actions.running?ret=Mavo.Actions.Functions[propertyL]:ret=Mavo.Actions.nope), void 0===ret&&(propertyL in Mavo.Functions?ret=Mavo.Functions[propertyL]:ret=Math[property]||Math[propertyL]), ret;
}, has:(data, property) => {
...
return o.symbol&&Mavo.toArray(o.symbol).forEach(symbol => {
_.unarySymbols[symbol]=name, jsep.addUnaryOp(symbol);
}), operand => _.unaryOperation(operand, operand => o.scalar(val(operand)));
...
o.scalar="function"==typeof o?o:o.scalar;var result;if (!Array.isArray(b)) {
result=Array.isArray(a)?a.map(n => o.scalar(n, b)):o.scalar(a, b);
}
else if (Array.isArray(a)) {
result=[];var max=_Mathmax3(a.length, b.length), leftUnary=o.leftUnary||o.unary, rightUnary=o.rightUnary||o.unary, leftDefault=void 0===o.leftDefault?o.default:o.leftDefault, rightDefault=void 0===o.rightDefault?o.default:o.rightDefault;for (let i=0;i<max;i++) {
...
}), o.default=void 0===o.default?0:o.default, o.code||function(...operands) {
1===operands.length&&Array.isArray(operands[0])&&(operands=[...operands[0]]), o.raw||(operands=operands.map(val));var result, prev=!!o.comparison||operands[0];for (let i=1;i<operands.length;i++) {
let a=o.comparison?operands[i-1]:prev, b=operands[i];Array.isArray(b)&&"number"==typeof o.default&&(b=$u.numbers(b));var result=_.binaryOperation(a, b, o);prev=o.comparison?_.binaryOperation(prev, result, _.operators.and):result;
} return prev;
};
}, symbols:{}, unarySymbols:{}, getOperatorName:(op, unary) => _[unary?"unarySymbols":"symbols"][op]||op, isComparisonOperator:op => {
if (op) {
let operatorDefinition=_.operators[_.symbols[op]];return operatorDefinition&&operatorDefinition.comparison;
}
}, isStatic:node => {
if ("Identifier"===node.type) {
return !1;
} for (let property of _.childProperties) {
if (node[property]&&"callee"!==property&&!_.isStatic(node[property])) {
return !1;
}
} return !0;
}, operators:{not:{symbol:"!", scalar:a => !val(a)}, multiply:{scalar:(a, b) => a*b, default:1, symbol:"*"}, divide:{scalar:(a, b) => a/b, rightUnary:b => b, default:1, symbol:"/"}, addition:{scalar:(a, b) => {
if (isNaN(a)||isNaN(b)) {
var dateA=$u.date(a), dateB=$u.date(b);if (dateA||dateB) {
return +dateA+ +dateB;
}
} return +a+ +b;
}, symbol:"+"}, plus:{scalar:a => +a, symbol:"+"}, subtract:{scalar:(a, b) => {
if (isNaN(a)||isNaN(b)) {
var dateA=$u.date(a), dateB=$u.date(b);if (dateA&&dateB) {
return dateA-dateB;
}
} return a-b;
}, symbol:"-"}, minus:{scalar:a => -a, symbol:"-"}, mod:{scalar:(a, b) => {
var ret=a%b;return ret+=0>ret?b:0, ret;
}, symbol:"mod", precedence:10}, lte:{comparison:!0, scalar:(a, b) => ([a, b]=_.getNumericalOperands(a, b), a<=b), default:!1, symbol:"<="}, lt:{comparison:!0, scalar:(a, b) => ([a, b]=_.getNumericalOperands(a, b), a<b), default:!1, symbol:"<"}, gte:{comparison:!0, scalar:(a, b) => ([a, b]=_.getNumericalOperands(a, b), a>=b), default:!1, symbol:">="}, gt:{comparison:!0, scalar:(a, b) => ([a, b]=_.getNumericalOperands(a, b), a>b), default:!1, symbol:">"}, eq:{comparison:!0, scalar:(a, b) => a==b||Mavo.safeToJSON(a)===Mavo.safeToJSON(b), symbol:["=", "=="], default:!1, precedence:7}, neq:{comparison:!0, scalar:(a, b) => a!=b&&Mavo.safeToJSON(a)!==Mavo.safeToJSON(b), symbol:["!="], default:!0, precedence:7}, and:{scalar:(a, b) => a&&b, default:!1, symbol:["&&", "and"], precedence:2}, or:{scalar:(a, b) => a||b, default:!1, symbol:["||", "or"], precedence:2}, concatenate:{symbol:"&", default:"", scalar:(a, b) => ""+(a||"")+(b||""), precedence:10}, keyvalue:{symbol:":", code:(...operands) => {
for (var i=operands.length-1, value=operands[i];i--;) {
...
}, precedence:4}, filter:{symbol:"where", code:(a, ...filters) => {
for (let b of filters) {
Array.isArray(a)?Array.isArray(b)?a=a.map((v, i) => val(b[i])?v:null):(b=val(b), a="boolean"==typeof b?b?a:a.map(() => null):a.map(v => v==b?v:null)):a=val(b)?a:null;
} return a;
}, precedence:1, postFlattenTransformation:node => {
var object=node.arguments[0];for (let i=1;i<node.arguments.length;i++) {
_.isStatic(node.arguments[i])||(node.arguments[i]=Object.assign(_.parse("scope()"), {arguments:[object, node.arguments[i]]}));
}
}}, range:{symbol:"..", scalar:(a, b) => Mavo.Functions.range(a, b), precedence:2, export:!1}, has:{symbol:"in", code:function(needle, ...haystacks) {
var ret;return haystacks.map(b => {
if (Array.isArray(b)) {
var op=a => {
...
}, precedence:3}, groupby:{symbol:"by", code:(array, key) => {
array=Mavo.toArray(array), key=Mavo.toArray(key);var property=key[Mavo.as]||$.value(key[0], Mavo.toNode, "property"), groups=new Mavo.BucketMap({arrays:!0}), ret=[];return ret[Mavo.groupedBy]=!0, array.forEach((item, i) => {
let k=i<key.length?Mavo.value(key[i]):null;groups.set(k, item);
}), Mavo.in(Mavo.route, array)&&(ret[Mavo.route]=Object.assign({}, array[Mavo.route])), groups.forEach((items, value) => {
var obj={$value:value, [property||"$value"]:value, $items:items};Mavo.in(Mavo.route, array)&&(items[Mavo.route]=obj[Mavo.route]=Object.assign({}, array[Mavo.route]), obj[Mavo.route]=$.each(items[Mavo.route], () => new Set(["$items"]))), ret.push(obj);
}), Mavo.Data.proxify(ret);
}, precedence:2}, as:{symbol:"as", code:(property, name) => {
...
}, childProperties:["arguments", "argument", "callee", "left", "right", "elements", "test", "consequent", "alternate", "object", "body"], walk:function(node, callback, o={}, property, parent) {
if (!o.type||node.type===o.type) {
var ret=callback(node, property, parent);
} return o.ignore&&-1!==o.ignore.indexOf(node.type)||_.childProperties.forEach(property => {
node[property]&&_.walk(node[property], callback, o, property, node);
}), void 0!==ret&&parent&&(parent[property]=ret), ret;
}, serializers:{BinaryExpression:node => `${_.serialize(node.left)} ${node.operator} ${_.serialize(node.right)}`, UnaryExpression:node => `${node.operator}${_.serialize(node.argument)}`, CallExpression:node => {
...
}, ConditionalExpression:node => `${_.serialize(node.test)}? ${_.serialize(node.consequent)} : ${_.serialize(node.alternate)}`, MemberExpression:node => {
if ("Identifier"===node.object.type&&"$fn"===node.object.name) {
var property=node.computed?`[${_.serialize(node.property)}]`:`.${node.property.name}`;return `$fn${property}`;
} var property=node.computed?_.serialize(node.property):`"${node.property.name}"`;return `$fn.get(${_.serialize(node.object)}, ${property})`;
}, ArrayExpression:node => `[${node.elements.map(_.serialize).join(", ")}]`, Literal:node => node.raw.replace(/\r/g, "\\r").replace(/\n/g, "\\n"), Identifier:node => node.name, ThisExpression:() => "this", Compound:node => node.body.map(_.serialize).join(", ")}, transformations:{BinaryExpression:node => {
let name=_.getOperatorName(node.operator), def=_.operators[name];def.transformation&&def.transformation(node);var nodeLeft=node, ret={type:"CallExpression", arguments:[], callee:{type:"Identifier", name}};if (def.comparison) {
let comparisonOperands=[];do {
let operatorName=_.getOperatorName(nodeLeft.operator);comparisonOperands.unshift({comparison:operatorName, operand:nodeLeft.right}), nodeLeft=nodeLeft.left;
} while (!1!==def.flatten&&_.isComparisonOperator(nodeLeft.operator));let comparisonsHeterogeneous=!1;for (let i=0;i<comparisonOperands.length-1;i++) {
if (comparisonOperands[i].comparison!=comparisonOperands[i+1].comparison) {
comparisonsHeterogeneous=!0;break;
}
}ret.arguments.push(nodeLeft), comparisonsHeterogeneous?(ret.callee.name="compare", comparisonOperands.forEach(co => {
ret.arguments.push({type:"Literal", value:co.comparison, raw:`"${co.comparison}"`}), ret.arguments.push(co.operand);
...
return {type:"CallExpression", arguments:[node.argument], callee:{type:"Identifier", name}};
}
}, CallExpression:node => {
if ("Identifier"==node.callee.type) {
if ("if"==node.callee.name) {
node.callee.name="iff";var condition=node.arguments[0];for (let i=1;i<node.arguments.length;i++) {
2==i&&(condition=_.parse("not()"), condition.arguments.push(node.arguments[0])), _.walk(node.arguments[i], n => {
...
}, ThisExpression:() => ({type:"Identifier", name:"$this"})}, serialize:node => {
if ("string"==typeof node) {
return node;
} var ret=_.transformations[node.type]&&_.transformations[node.type](node);if ("object"==typeof ret&&ret&&ret.type) {
node=ret;
}
else if (void 0!==ret) {
return ret;
...
return /\S/.test(code)?(code=_.rewrite(code), code=`with (Mavo.Data.stub)
	with (data || {}) {
		return (${code});
	}`, o.actions&&(code=`
...
${code}
Mavo.Actions.running = Mavo.Actions._running;`), new Function("data", code)):() => "";
}, parse:self.jsep, serializeScopeCall:args => {
var withCode=`with (Mavo.Script.subScope(scope, $this) || {}) { return (${_.serialize(args[1])}); }`;return `(function() {
...
var unscopables=Object.keys($this).reduce((o, k) => (o[k]=!0, o), {$this:!0});return proxy&&"object"==typeof proxy?new Proxy(proxy, {get:(t, property, r) => property===Symbol.unscopables?unscopables:Reflect.get(t, property, r)}):proxy;
}};for (let name in _.serializers.LogicalExpression=_.serializers.BinaryExpression, _.transformations.LogicalExpression=_.transformations.BinaryExpression, _.operators) {
let details=_.operators[name];if (details.scalar&&2>details.scalar.length) {
...
let result=!0;for (let i=2;i<operands.length;i+=2) {
let a=operands[i-2], op=operands[i-1], b=operands[i], term=_.binaryOperation(a, b, Mavo.Script.operators[op]);result=_.binaryOperation(result, term, Mavo.Script.operators.and);
} return result;
};
}(Bliss, Mavo.value, Mavo.Functions.util), function($) {
Mavo.attributes.push("mv-action");var _=Mavo.Actions={listener:evt => {
...
var node=_.getNode(ref);return node?[node]:Mavo.toArray(ref).map(n => _.getNode(n)).filter(n => n!==void 0);
}, getNode:node => {
...
var actions=Object.keys(_.Functions).map(name => `${name}()`);Mavo.warn(`Mavo actions (${actions}) can only be used in the mv-action attribute.`);
...
var collection=_.getCollection(ref);collection||(collection=_.getCollection(data), collection&&([data, ref, index]=[void 0, data, ref]));
}
} if (ref) {
...
if (from&&void 0!==to) {
"number"!=$.type(to)||toNode||([index, to]=[to]);var toNode=_.getNode(to), fromNodes=Mavo.toArray(from).map(_.getNode).filter(n => n&&n.closestCollection), collection=(toNode||fromNodes[0]).closestCollection;if (!fromNodes.length) {
...
if (ref.length&&ref[0]) {
var nodes=_.getNodes(Mavo.flatten(ref)), itemsToDelete=[];return nodes.forEach(node => {
node&&(node instanceof Mavo.Collection?itemsToDelete.push(...node.children):node.collection?itemsToDelete.push(node):node.walk(n => {
n instanceof Mavo.Primitive?n.value=null:n!==node&&_.clear(n);
}));
}), Mavo.Collection.delete(itemsToDelete), nodes.map(n => n.getLiveData());
}
}, clearif:(condition, ...targets) => (targets=targets.map(t => Mavo.Functions.iff(condition, t)), _.Functions.clear(...targets)), set:(ref, values) => {
if (ref) {
var node=_.getNode(ref);if (node) {
node.render(values);
}
else {
var wasArray=Array.isArray(ref), nodes=_.getNodes(ref);nodes.length?Mavo.Script.binaryOperation(wasArray?nodes:nodes[0], values, {scalar:(node, value) => node?node.render(value):null}):Mavo.warn(`The first parameter of set() needs to be one or more existing properties, ${Mavo.safeToJSON(ref)} is not.`);
} return values;
}
}}};for (let name in _.Functions) {
let nameif=name+"if";nameif in _.Functions||(_.Functions[nameif]=(condition, target, ...rest) => (target=Mavo.Functions.iff(condition, target), Mavo.value(condition)?_.Functions[name](target, ...rest):null));
}_.Functions.deleteif=_.Functions.clearif;
}(Bliss, Bliss.$), function($) {
var _=Mavo.Data=$.Class(class {
...
} this.node instanceof Mavo.ImplicitCollection&&(Mavo.filter(this.data, data => null!==Mavo.value(data)), this.updateParent());
}
...
}, {live:{data:function(data) {
if (data!==this._data) {
return this.isArray=Array.isArray(data), this._data=data, data[Mavo.toNode]=this.node, data[Mavo.parent]=this.parent&&this.parent.data, data[Mavo.mavo]=this.node.mavo, this.proxy=this.proxify(), this.updateParent(), this._data;
}
}}, static:{stub:self.Proxy?new Proxy({[Symbol.unscopables]:{data:!0, undefined:!0}}, {get:(data, property) => {
var ret=Reflect.get(data, property);if (ret!==void 0||"string"!=typeof property) {
return ret;
...
}, has:(data, property) => Reflect.has(data, property)||"string"==typeof property}):Mavo.Functions, isItem:function(data) {
...
} while (obj=obj[Mavo.parent]);return {value:null, path};
}, root(obj) {
...
var ret=data.map(a => _.find(property, a)).filter(x => void 0!==x);if (ret.length) {
return Mavo.flatten(ret);
}
} return;
} var ret, results=[], returnArray=Array.isArray(data);results[Mavo.route]={}, results[Mavo.mavo]=data[Mavo.mavo];var findDown=prop => {
...
}Array.isArray(ret)?(results.push(...ret), returnArray=!0):results.push(ret);
}
...
var child, parent=data;do {
var ret=_.find(property, parent, {exclude:child});if (ret!==void 0) {
return ret;
...
return data[property];
} var ret, propertyIsNumeric=!isNaN(property);if (property in data) {
ret=data[property];
}
...
var proxify=null!==ret&&"object"==typeof ret&&(Mavo.route in ret||Mavo.toNode in ret);return proxify?_.proxify(ret):ret;
} if (!propertyIsNumeric) {
...
} var objects=[data, Mavo.all, _.special];if (objects.some(obj => property in obj)) {
return !0;
} if ("string"==typeof property) {
var propertyL=property.toLowerCase();if (propertyL!==property&&objects.some(obj => propertyL in obj)) {
return !0;
...
return data&&"object"==typeof data&&self.Proxy&&!data[Mavo.isProxy]?new Proxy(data, {get:(data, property) => _.resolve(property, data), has:(data, property) => _.has(property, data), set:function(data, property="", value) {
return "symbol"==typeof property?Reflect.set(data, property, value):(Mavo.warn(`You cannot set data via expressions. Attempt to set ${property.toString()} to ${value} ignored.`), value);
}}):data;
}, computeMetadata(object, property, parent) {
object&&"object"==typeof object&&(property!==void 0&&(object[Mavo.property]=property), parent&&!object[Mavo.parent]&&(object[Mavo.parent]=parent));
...
var arr=_.closestArray(obj), path=arr.path.reverse().slice(1), ret=arr.value.map(a => $.value(a, ...path));return 0<ret.length&&ret[0][Mavo.route]&&(ret[Mavo.route]=$.each(ret[0][Mavo.route], () => !0), ret[Mavo.mavo]=ret[0][Mavo.mavo]), ret;
}, $next:function(obj) {
var arr=_.closestArray(obj), path=arr.path.reverse(), index=arr.path[0];path=path.slice(1);var nextClosestItem=$.value(arr.value, index+1);return nextClosestItem?$.value(nextClosestItem, ...path):null;
}, $previous:function(obj) {
var arr=_.closestArray(obj), path=arr.path.reverse(), index=arr.path[0];path=path.slice(1);var prevClosestItem=$.value(arr.value, index-1);return prevClosestItem?$.value(prevClosestItem, ...path):null;
}, $this:function(obj) {
return obj;
}}}});
}(Bliss, Bliss.$), function($) {
...
return path=this.path.replace(/[^/]+$/, "")+path, this.put(file, path).then(() => this.getURL(path));
}, getURL:function(path) {
return this.request("sharing/create_shared_link_with_settings", {path}, "POST").then(shareInfo => _.fixShareURL(shareInfo.url));
}, put:function(serialized, path=this.path, o={}) {
return this.request("https://content.dropboxapi.com/2/files/upload", serialized, "POST", {headers:{"Dropbox-API-Arg":JSON.stringify({path, mode:"overwrite"}), "Content-Type":"application/octet-stream"}});
}, oAuthParams:() => `&redirect_uri=${encodeURIComponent("https://auth.mavo.io")}&response_type=code`, getUser:function() {
return this.user?Promise.resolve(this.user):this.request("users/get_current_account", "null", "POST").then(info => {
this.user={username:info.email, name:info.name.display_name, avatar:info.profile_photo_url, info}, $.fire(this.mavo.element, "mv-login", {backend:this});
});
}, login:function(passive) {
return this.oAuthenticate(passive).then(() => this.getUser()).then(() => {
this.user&&(this.permissions.logout=!0, this.request("sharing/get_shared_link_metadata", {url:this.source}, "POST").then(info => {
this.path=info.path_lower, this.permissions.on(["edit", "save"]);
}));
});
}, logout:function() {
...
}, fixShareURL:url => (url=new URL(url, Mavo.base), url.hostname="dl.dropboxusercontent.com", url.search=url.search.replace(/\bdl=0|^$/, "raw=1"), url)}}));
}(Bliss, Bliss.$), function($) {
var _=Mavo.Backend.register($.Class({extends:Mavo.Backend, id:"Github", constructor:function() {
this.permissions.on(["login", "read"]), this.key=this.mavo.element.getAttribute("mv-github-key")||"7e08e016048000bc594e";var extension=this.format.constructor.extensions[0]||".json";this.defaults={repo:"mv-data", filename:`${this.mavo.id}${extension}`}, this.info=_.parseURL(this.source, this.defaults), $.extend(this, this.info), this.login(!0);
}, update:function(url, o) {
...
var info=url?_.parseURL(url):this.info;return info.apiData?this.request(info.apiCall, info.apiData, "POST").then(response => response.errors&&response.errors.length?Promise.reject(response.errors.map(x => x.message).join("\n")):response.data):this.request(info.apiCall, {ref:this.branch}, "GET", {headers:{Accept:"application/vnd.github.squirrel-girl-preview"}}).then(response => Promise.resolve(info.repo?_.atob(response.content):response));
} return url=new URL(`https://raw.githubusercontent.com/${this.username}/${this.repo}/${this.branch||"master"}/${this.path}`), url.searchParams.set("timestamp", Date.now()), $.fetch(url.href).then(xhr => Promise.resolve(xhr.responseText), () => Promise.resolve(null));
}, upload:function(file, path=this.path) {
return Mavo.readFile(file).then(dataURL => {
var base64=dataURL.slice(5), media=base64.match(/^\w+\/[\w+]+/)[0];return base64=base64.replace(RegExp(`^${media}(;base64)?,`), ""), path=this.path.replace(/[^/]+$/, "")+path, this.put(base64, path, {isEncoded:!0});
}).then(fileInfo => this.getURL(path, fileInfo.commit.sha));
}, put:function(serialized, path=this.path, o={}) {
if (path) {
var repoCall=`repos/${this.username}/${this.repo}`, fileCall=`${repoCall}/contents/${path}`, commitPrefix=this.mavo.element.getAttribute("mv-github-commit-prefix")||"", repoInfo=this.repoInfo?Promise.resolve(this.repoInfo):this.request("user/repos", {name:this.repo}, "POST").then(repoInfo => this.repoInfo=repoInfo);return serialized=o.isEncoded?serialized:_.btoa(serialized), repoInfo.then(repoInfo => this.canPush()?repoInfo:this.request(`${repoCall}/forks`, {name:this.repo}, "POST").then(forkInfo => (fileCall=`repos/${forkInfo.full_name}/contents/${path}`, this.forkInfo=forkInfo)).then(forkInfo => {
var timeout, test=resolve => {
clearTimeout(timeout), this.request(`repos/${forkInfo.full_name}/commits`, {until:"1970-01-01T00:00:00Z"}, "HEAD").then(() => {
resolve(forkInfo);
}).catch(() => {
timeout=setTimeout(test, 1e3);
});
};return new Promise(test);
})).then(() => this.request(fileCall, {ref:this.branch}).then(fileInfo => this.request(fileCall, {message:commitPrefix+this.mavo._("gh-updated-file", {name:fileInfo.name||"file"}), content:serialized, branch:this.branch, sha:fileInfo.sha}, "PUT"), xhr => 404==xhr.status?this.request(fileCall, {message:commitPrefix+"Created file", content:serialized, branch:this.branch}, "PUT"):xhr)).then(fileInfo => {
if (this.repoInfo.fork) {
this.forkInfo=this.repoInfo.parent, this.request(`repos/${this.repoInfo.parent.owner.login}/${this.repoInfo.parent.name}/pulls`, {head:`${this.user.username}:${this.branch}`, base:this.repoInfo.parent.default_branch}).then(prs => {
this.pullRequest(prs[0]);
});
}
else if (this.forkInfo) {
let params=new URL(location).searchParams;params.append("storage", fileInfo.content.download_url), history.pushState({}, "", `${location.pathname}?${params}`), location.replace(`${location.pathname}?${params}`), this.request(`repos/${this.username}/${this.repo}/pulls`, {head:`${this.user.username}:${this.branch}`, base:this.branch}).then(prs => {
this.pullRequest(prs[0]);
});
} return fileInfo;
});
...
var previewURL=new URL(location);previewURL.searchParams.set(this.mavo.id+"-storage", `https://github.com/${this.forkInfo.full_name}/${this.path}`);var message=this.mavo._("gh-edit-suggestion-saved-in-profile", {previewURL}), lastNoticeName="";if (this.notice&&(lastNoticeName=this.notice.options.name, this.notice.element.style.transition="none", this.notice.close()), existing) {
var style="closePR"===lastNoticeName?{animation:"none", transition:"none"}:{};this.notice=this.mavo.message(`${message}
				${this.mavo._("gh-edit-suggestion-notreviewed")}
				<form onsubmit="return false">
					<button class="mv-danger">${this.mavo._("gh-edit-suggestion-revoke")}</button>
				</form>`, {classes:"mv-inline", dismiss:["button", "submit"], style:style, name:"closePR"}), this.notice.element.style.transitionDuration="400ms", this.notice.closed.then(form => {
if (form) {
var username, repo;this.repoInfo.fork?(username=this.repoInfo.parent.owner.login, repo=this.repoInfo.parent.name):(username=this.username, repo=this.repo), this.request(`repos/${username}/${repo}/pulls/${existing.number}`, {state:"closed"}, "POST").then(prInfo => {
new Mavo.UI.Message(this.mavo, `<a href="${prInfo.html_url}">${this.mavo._("gh-edit-suggestion-cancelled")}</a>`, {dismiss:["button", "timeout"], style:style}), this.pullRequest();
});
}
});
}
else {
var style="createPR"===lastNoticeName?{animation:"none", transition:"none"}:{};this.notice=this.mavo.message(`${message}
				...
				</form>`, {classes:"mv-inline", dismiss:["button", "submit"], style:style, name:"createPR"}), this.notice.element.style.transitionDuration="400ms", this.notice.closed.then(form => {
if (form) {
var username, repo, base;this.repoInfo.fork?(username=this.repoInfo.parent.owner.login, repo=this.repoInfo.parent.name, base=this.repoInfo.parent.default_branch):(username=this.username, repo=this.repo, base=this.branch), this.request(`repos/${username}/${repo}/pulls`, {title:this.mavo._("gh-edit-suggestion-title"), body:this.mavo._("gh-edit-suggestion-body", {description:form.elements.edits.value, previewURL}), head:`${this.user.username}:${this.branch}`, base:base}, "POST").then(prInfo => {
new Mavo.UI.Message(this.mavo, `<a href="${prInfo.html_url}">${this.mavo._("gh-edit-suggestion-sent")}</a>`, {dismiss:["button", "timeout"], style:style}), this.pullRequest(prInfo);
});
}
});
}
...
if (this.user&&(this.permissions.on("logout"), this.info.path&&this.permissions.on(["edit", "save"]), this.repo)) {
return this.request(`repos/${this.username}/${this.repo}`).then(repoInfo => {
void 0===this.branch&&(this.branch=repoInfo.default_branch), this.repoInfo=repoInfo;new URL(location).searchParams;if (!this.mavo.source) {
if (repoInfo.fork) {
return this.forkInfo=repoInfo.parent, this.request(`repos/${repoInfo.parent.owner.login}/${repoInfo.parent.name}/pulls`, {head:`${this.user.username}:${this.branch}`, base:repoInfo.parent.default_branch}).then(prs => {
this.pullRequest(prs[0]);
}), repoInfo;
} if (!this.canPush()) {
...
return this.switchToMyForkDialog(repos[i].url), repoInfo;
}
} return repoInfo;
});
...
return this.switchToMyForkDialog(forks[i].html_url), repoInfo;
}
} return repoInfo;
});
}
...
return this.user?Promise.resolve(this.user):this.request("user").then(info => {
this.user={username:info.login, name:info.name||info.login, avatar:info.avatar_url, url:"https://github.com/"+info.login, info}, $.fire(this.mavo.element, "mv-login", {backend:this});
});
}, getURL:function(path=this.path, sha) {
var repoInfo=this.forkInfo||this.repoInfo, repo=repoInfo.full_name;return path=path.replace(/ /g, "%20"), repoInfo.pagesInfo=repoInfo.pagesInfo||this.request(`repos/${repo}/pages`, {}, "GET", {headers:{Accept:"application/vnd.github.mister-fantastic-preview+json"}}), repoInfo.pagesInfo.then(pagesInfo => pagesInfo.html_url+path).catch(() => `https://cdn.jsdelivr.net/gh/${repo}@${sha||this.branch||"latest"}/${path}`);
}, switchToMyForkDialog:function(forkURL) {
let params=new URL(location).searchParams;return params.append("storage", forkURL+"/"+this.path), this.notice=this.mavo.message(`
			${this.mavo._("gh-login-fork-options")}
			<form onsubmit="return false">
				<a href="${location.pathname}?${params}"><button>${this.mavo._("gh-use-my-fork")}</button></a>
			</form>`, {classes:"mv-inline", dismiss:["button", "submit"]}), void this.notice.closed.then(form => {
form&&(history.pushState({}, "", `${location.pathname}?${params}`), location.replace(`${location.pathname}?${params}`));
});
...
var ret={}, url=new URL(source, Mavo.base), path=url.pathname.slice(1).split("/");if (ret.username=path.shift(), ret.repo=path.shift()||defaults.repo, /raw.githubusercontent.com$/.test(url.host)) {
ret.branch=path.shift();
}
else {
if (/api.github.com$/.test(url.host)) {
var apiCall=url.pathname.slice(1)+url.search, data=Mavo.Functions.from(source, "#");return {apiCall, apiData:"graphql"==apiCall?{query:data}:data};
}"blob"==path[0]&&(path.shift(), ret.branch=path.shift());
} var lastSegment=path[path.length-1];return /\.\w+$/.test(lastSegment)?(ret.filename=lastSegment, path.splice(path.length-1, 1)):ret.filename=defaults.filename, ret.filepath=path.join("/")||defaults.filepath||"", ret.path=(ret.filepath?ret.filepath+"/":"")+ret.filename, ret.apiCall=`repos/${ret.username}/${ret.repo}/contents/${ret.path}`, ret;
}, btoa:str => btoa(unescape(encodeURIComponent(str))), atob:str => decodeURIComponent(escape(window.atob(str)))}}));
}(Bliss, Bliss.$);
