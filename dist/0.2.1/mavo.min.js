...
Mavo.Actions.running = Mavo.Actions._running;`),new Function("data",code)):()=>""},parse:self.jsep,serializeScopeCall:args=>{var withCode=`with (Mavo.Script.subScope(scope, $this) || {}) { return (${_.serialize(args[1])}); }`;return`(function() {
	...
	${withCode}
				...
				</form>`,{classes:"mv-inline",dismiss:["button","submit"],style:style,name:"createPR"}),this.notice.element.style.transitionDuration="400ms",this.notice.closed.then(form=>{if(form){var username,repo,base;this.repoInfo.fork?(username=this.repoInfo.parent.owner.login,repo=this.repoInfo.parent.name,base=this.repoInfo.parent.default_branch):(username=this.username,repo=this.repo,base=this.branch),this.request(`repos/${username}/${repo}/pulls`,{title:this.mavo._("gh-edit-suggestion-title"),body:this.mavo._("gh-edit-suggestion-body",{description:form.elements.edits.value,previewURL}),head:`${this.user.username}:${this.branch}`,base:base},"POST").then(prInfo=>{new Mavo.UI.Message(this.mavo,`<a href="${prInfo.html_url}">${this.mavo._("gh-edit-suggestion-sent")}</a>`,{dismiss:["button","timeout"],style:style}),this.pullRequest(prInfo)})}})}},login:function(passive){return this.oAuthenticate(passive).then(()=>this.getUser()).catch(xhr=>{401==xhr.status&&this.logout()}).then(()=>{if(this.user&&(this.permissions.on("logout"),this.info.path&&this.permissions.on(["edit","save"]),this.repo))return this.request(`repos/${this.username}/${this.repo}`).then(repoInfo=>{void 0===this.branch&&(this.branch=repoInfo.default_branch),this.repoInfo=repoInfo;new URL(location).searchParams;if(!this.mavo.source){if(repoInfo.fork)return this.forkInfo=repoInfo.parent,this.request(`repos/${repoInfo.parent.owner.login}/${repoInfo.parent.name}/pulls`,{head:`${this.user.username}:${this.branch}`,base:repoInfo.parent.default_branch}).then(prs=>{this.pullRequest(prs[0])}),repoInfo;if(!this.canPush()){if(this.user.info.public_repos<repoInfo.forks){return this.request("https://api.github.com/graphql",{query:"query {\n\t\t\t\t\t\t\t\t\t\t\t\t\t  viewer {\n\t\t\t\t\t\t\t\t\t\t\t\t\t    name\n\t\t\t\t\t\t\t\t\t\t\t\t\t      repositories(last: 100, isFork: true) {\n\t\t\t\t\t\t\t\t\t\t\t\t\t      nodes {\n\t\t\t\t\t\t\t\t\t\t\t\t\t        url\n\t\t\t\t\t\t\t\t\t\t\t\t\t        parent {\n\t\t\t\t\t\t\t\t\t\t\t\t\t          nameWithOwner\n\t\t\t\t\t\t\t\t\t\t\t\t\t        }\n\t\t\t\t\t\t\t\t\t\t\t\t\t      }\n\t\t\t\t\t\t\t\t\t\t\t\t\t    }\n\t\t\t\t\t\t\t\t\t\t\t\t\t  }\n\t\t\t\t\t\t\t\t\t\t\t\t\t}"},"POST").then(data=>{var repos=data.data.viewer.repositories.nodes;for(var i in repos)if(repos[i].parent.nameWithOwner===repoInfo.full_name)return this.switchToMyForkDialog(repos[i].url),repoInfo;return repoInfo})}return this.request(repoInfo.forks_url).then(forks=>{for(var i in forks)if(forks[i].owner.login===this.user.username)return this.switchToMyForkDialog(forks[i].html_url),repoInfo;return repoInfo})}}return repoInfo})})},canPush:function(){return this.repoInfo?this.repoInfo.permissions.push:this.user&&this.user.username.toLowerCase()==this.username.toLowerCase()},oAuthParams:()=>"&scope=repo,gist",logout:function(){return this.oAuthLogout().then(()=>{this.user=null})},getUser:function(){return this.user?Promise.resolve(this.user):this.request("user").then(info=>{this.user={username:info.login,name:info.name||info.login,avatar:info.avatar_url,url:"https://github.com/"+info.login,info},$.fire(this.mavo.element,"mv-login",{backend:this})})},getURL:function(path=this.path,sha){var repoInfo=this.forkInfo||this.repoInfo,repo=repoInfo.full_name;return path=path.replace(/ /g,"%20"),repoInfo.pagesInfo=repoInfo.pagesInfo||this.request(`repos/${repo}/pages`,{},"GET",{headers:{Accept:"application/vnd.github.mister-fantastic-preview+json"}}),repoInfo.pagesInfo.then(pagesInfo=>pagesInfo.html_url+path).catch(()=>`https://cdn.jsdelivr.net/gh/${repo}@${sha||this.branch||"latest"}/${path}`)},switchToMyForkDialog:function(forkURL){let params=new URL(location).searchParams;return params.append("storage",forkURL+"/"+this.path),this.notice=this.mavo.message(`
			${this.mavo._("gh-login-fork-options")}
			<form onsubmit="return false">
				<a href="${location.pathname}?${params}"><button>${this.mavo._("gh-use-my-fork")}</button></a>
			</form>`,{classes:"mv-inline",dismiss:["button","submit"]}),void this.notice.closed.then(form=>{form&&(history.pushState({},"",`${location.pathname}?${params}`),location.replace(`${location.pathname}?${params}`))})},static:{apiDomain:"https://api.github.com/",oAuth:"https://github.com/login/oauth/authorize",test:function(url){return url=new URL(url,Mavo.base),/\bgithub.com|raw.githubusercontent.com/.test(url.host)},parseURL:function(source,defaults={}){var ret={},url=new URL(source,Mavo.base),path=url.pathname.slice(1).split("/");if(ret.username=path.shift(),ret.repo=path.shift()||defaults.repo,/raw.githubusercontent.com$/.test(url.host))ret.branch=path.shift();else{if(/api.github.com$/.test(url.host)){var apiCall=url.pathname.slice(1)+url.search,data=Mavo.Functions.from(source,"#");return{apiCall,apiData:"graphql"==apiCall?{query:data}:data}}"blob"==path[0]&&(path.shift(),ret.branch=path.shift())}var lastSegment=path[path.length-1];return /\.\w+$/.test(lastSegment)?(ret.filename=lastSegment,path.splice(path.length-1,1)):ret.filename=defaults.filename,ret.filepath=path.join("/")||defaults.filepath||"",ret.path=(ret.filepath?ret.filepath+"/":"")+ret.filename,ret.apiCall=`repos/${ret.username}/${ret.repo}/contents/${ret.path}`,ret},btoa:str=>btoa(unescape(encodeURIComponent(str))),atob:str=>decodeURIComponent(escape(window.atob(str)))}}))}(Bliss,Bliss.$);
