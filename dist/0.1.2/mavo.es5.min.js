...
"use strict";function _toConsumableArray(a){if(Array.isArray(a)){for(var b=0,c=Array(a.length);b<a.length;b++)c[b]=a[b];return c}return Array.from(a)}(function(a){Mavo.attributes.push("mv-expressions");var b=Mavo.Expression=a.Class({constructor:function(d){this.expression=d},eval:function(d){this.oldValue=this.value,Mavo.hooks.run("expression-eval-beforeeval",this);try{this.function||(this.function=Mavo.Script.compile(this.expression),this.identifiers=this.expression.match(/[$a-z][$\w]*/ig)||[]),this.value=this.function(d)}catch(e){console.info("%cExpression error!","color: red; font-weight: bold",e.message+" in expression "+this.expression,"\nNot an expression? Use mv-expressions=\"none\" to disable expressions on an element and its descendants."),Mavo.hooks.run("expression-eval-error",{context:this,exception:e}),this.value=e}return this.value},toString:function(){return this.expression},changedBy:function(d){if(!d)return!0;if(!this.identifiers)return!1;if(-1<this.identifiers.indexOf(d.property))return!0;if(Mavo.Functions.intersects(d.properties,this.identifiers))return!0;if("propertychange"!=d.action){if(Mavo.Functions.intersects(["$index","$previous","$next"],this.identifiers))return!0;var e=d.node.collection||d.node;if(Mavo.Functions.intersects(e.properties,this.identifiers))return!0}return!1},live:{expression:function(){this.function=null}},static:{}});b.Syntax=a.Class({constructor:function(d,e){this.start=d,this.end=e,this.regex=RegExp(Mavo.escapeRegExp(d)+"([\\S\\s]+?)"+Mavo.escapeRegExp(e),"gi")},test:function(d){return this.regex.lastIndex=0,this.regex.test(d)},tokenize:function(d){var e,f=[],g=0;for(this.regex.lastIndex=0;null!==(e=this.regex.exec(d));)e.index>g&&f.push(d.substring(g,e.index)),g=this.regex.lastIndex,f.push(new Mavo.Expression(e[1]));return g<d.length&&f.push(d.substring(g)),f},static:{create:function(d){if(d){var e=d.getAttribute("mv-expressions");if(e)return e=e.trim(),/\s/.test(e)?new(Function.prototype.bind.apply(b.Syntax,[null].concat(_toConsumableArray(e.split(/\s+/))))):b.Syntax.ESCAPE}},ESCAPE:-1}}),b.Syntax.default=new b.Syntax("[","]")})(Bliss);
"use strict";function _toConsumableArray(a){if(Array.isArray(a)){for(var b=0,c=Array(a.length);b<a.length;b++)c[b]=a[b];return c}return Array.from(a)}(function(a){var b=Mavo.DOMExpression=a.Class({constructor:function(){var d=this,e=0<arguments.length&&void 0!==arguments[0]?arguments[0]:{};this.mavo=e.mavo,this.template=e.template&&e.template.template||e.template;for(var h,f=["item","path","syntax","fallback","attribute","originalAttribute","expression","parsed"],g=0;g<f.length;g++)h=f[g],this[h]=void 0===e[h]&&this.template?this.template[h]:e[h];if(this.node=e.node,this.node||(this.node=Mavo.elementPath(this.item.element,this.path)),this.element=this.node,this.attribute=this.attribute||null,Mavo.hooks.run("domexpression-init-start",this),3===this.node.nodeType&&this.element===this.node&&(this.element=this.node.parentNode,(!this.node.parentNode.children.length||this.attribute)&&(this.node=this.element,this.element.normalize())),!this.expression){if(this.attribute)this.expression=this.node.getAttribute(this.attribute).trim();else{if(this.node.normalize(),this.node.firstChild&&1===this.node.childNodes.length&&3===this.node.firstChild.nodeType){var j=this.node.firstChild.textContent.match(/^\s*|\s*$/g);j[1]&&(this.node.firstChild.splitText(this.node.firstChild.textContent.length-j[1].length),a.after(this.node.lastChild,this.node)),j[0]&&(this.node.firstChild.splitText(j[0].length),this.node.parentNode.insertBefore(this.node.firstChild,this.node))}this.expression=this.node.textContent}this.parsed=e.template?e.template.parsed:this.syntax.tokenize(this.expression)}this.oldValue=this.value=this.parsed.map(function(k){return k instanceof Mavo.Expression?k.expression:k}),this.item=Mavo.Node.get(this.element.closest(Mavo.selectors.item)),this.mavo.treeBuilt.then(function(){d.template||d.item||(d.item=Mavo.Node.get(d.element.closest(Mavo.selectors.item))),Mavo.hooks.run("domexpression-init-treebuilt",d)}),Mavo.hooks.run("domexpression-init-end",this),b.elements.set(this.element,[].concat(_toConsumableArray(b.elements.get(this.element)||[]),[this]))},changedBy:function(d){return!this.parsed.every(function(e){return!(e instanceof Mavo.Expression)||!e.changedBy(d)})},update:function(){var d=this,e=0<arguments.length&&arguments[0]!==void 0?arguments[0]:this.data,f=arguments[1],g={context:this,event:f},h=g;this.data=e,Mavo.hooks.run("domexpression-update-start",g),this.oldValue=this.value;var j=!1;g.value=this.value=this.parsed.map(function(k,l){if(k instanceof Mavo.Expression){if(k.changedBy(h.event)){var m={context:d,expr:k,parentEnv:h};return Mavo.hooks.run("domexpression-update-beforeeval",m),m.value=Mavo.value(m.expr.eval(e)),Mavo.hooks.run("domexpression-update-aftereval",m),j=!0,m.value instanceof Error?void 0===d.fallback?d.syntax.start+m.expr.expression+d.syntax.end:d.fallback:void 0===m.value||null===m.value?"":m.value}return d.oldValue[l]}return k});j&&(g.value=1===g.value.length?g.value[0]:g.value.map(Mavo.Primitive.format).join(""),this.output(g.value),Mavo.hooks.run("domexpression-update-end",g))},output:function(d){this.primitive?this.primitive.value=d:Mavo.Primitive.setValue(this.node,d,{attribute:this.attribute})},live:{item:function(d){d&&this._item!=d&&(this._item&&Mavo.delete(this._item.expressions,this),d.expressions=d.expressions||[],d.expressions.push(this))}},static:{elements:new WeakMap,search:function(d,e){var f=b.elements.get(d)||[];return 1<arguments.length?f.length?f.filter(function(g){return g.attribute===e})[0]||null:null:f}}})})(Bliss);
...
"use strict";var _typeof="function"==typeof Symbol&&"symbol"==typeof Symbol.iterator?function(b){return typeof b}:function(b){return b&&"function"==typeof Symbol&&b.constructor===Symbol&&b!==Symbol.prototype?"symbol":typeof b};function _defineProperty(b,f,g){return f in b?Object.defineProperty(b,f,{value:g,enumerable:!0,configurable:!0,writable:!0}):b[f]=g,b}function _toConsumableArray(b){if(Array.isArray(b)){for(var f=0,g=Array(b.length);f<b.length;f++)g[f]=b[f];return g}return Array.from(b)}(function(b,f){function g(){var u=0<arguments.length&&void 0!==arguments[0]?arguments[0]:"";return u=f(u),u||0===u?u+"":""}function h(u){return u=Mavo.value(u),null===u||!1===u||""===u}function j(u){return!f(u)}function k(u,w){var x=u.toLocaleString(Mavo.locale,w);return x=x.replace(/\u200e/g,""),x}function l(u){return function(w){if(w=o.date(w),!w)return"";var x=t[u](w);return x=new self["year"==u?"String":"Number"](x),("month"==u||"weekday"==u)&&(x.name=k(w,_defineProperty({},u,"long")),x.shortname=k(w,_defineProperty({},u,"short"))),"weekday"!=u&&(x.twodigit=(10>x?"0":"")+(1>x?"0":"")+x%100),x}}var m=Mavo.Functions={operators:{"=":"eq"},get:function(w,x){x=f(x);var y=Mavo.getCanonicalProperty(w,x);if(y!==void 0){var z=w[y];return"function"==typeof z&&0!==z.name.indexOf("bound")?z.bind(w):z}if(Array.isArray(w)&&isNaN(x)){var C,A=!0,B=!1;try{for(var E,F,D=w[Symbol.iterator]();!(A=(E=D.next()).done)&&(F=E.value,!(F&&"object"===("undefined"==typeof F?"undefined":_typeof(F))));A=!0);}catch(H){B=!0,C=H}finally{try{!A&&D.return&&D.return()}finally{if(B)throw C}}if(F){if("id"in F)for(var G=0;G<w.length;G++)if(w[G]&&w[G].id==x)return w[G];return w.map(function(H){return m.get(H,x)})}}return null},first:function(w){return w&&w[0]||""},last:function(w){return w&&w[w.length-1]||""},unique:function(w){return Array.isArray(w)?[].concat(_toConsumableArray(new Set(w.map(f)))):w},intersects:function(w,x){if(w&&x){var y=new Set(x.map?x.map(f):x);return w=w.map?w.map(f):[].concat(_toConsumableArray(w)),!w.every(function(z){return!y.has(z)})}},sum:function(w){return o.numbers(w,arguments).reduce(function(x,y){return+x+(+y||0)},0)},average:function(w){return w=o.numbers(w,arguments),w.length&&m.sum(w)/w.length},min:function(w){return Math.min.apply(Math,_toConsumableArray(o.numbers(w,arguments)))},max:function(w){return Math.max.apply(Math,_toConsumableArray(o.numbers(w,arguments)))},count:function(w){return Mavo.toArray(w).filter(function(x){return!h(x)}).length},round:function(w,x){return j(w)||j(x)||!isFinite(w)?Math.round(w):+w.toLocaleString("en-US",{useGrouping:!1,maximumFractionDigits:x})},th:function(w){if(h(w))return"";if(10>x||20<x)var x=["th","st","nd","th"][w%10];return x=x||"th",w+x},iff:function(w,x){var y=2<arguments.length&&arguments[2]!==void 0?arguments[2]:"";return Array.isArray(w)?w.map(function(z,A){var B=f(z)?x:y;return Array.isArray(B)?B[Math.min(A,B.length-1)]:B}):f(w)?x:y},replace:function(w,x){var y=2<arguments.length&&arguments[2]!==void 0?arguments[2]:"",z=3<arguments.length&&arguments[3]!==void 0?arguments[3]:1;if(Array.isArray(w))return w.map(function(E){return m.replace(E,x,y)});for(var C,A=RegExp(Mavo.escapeRegExp(x),"g"),B=w,D=0;B!=C&&D++<z;)C=B,B=B.replace(A,y);return B},len:function(w){return g(w).length},search:function(w,x){return w&&x?g(w).toLowerCase().indexOf((x+"").toLowerCase()):-1},starts:function(w,x){return 0===m.search(g(w),g(x))},ends:function(w,x){var y=[g(w),g(x)];w=y[0],x=y[1];var z=m.search(w,x);return-1<z&&z===w.length-x.length},join:function(w,x){return Mavo.toArray(w).join(g(x))},idify:function(w){return g(w).replace(/\s+/g,"-").replace(/[^\w-]/g,"").toLowerCase()},readable:function(w){return g(w).replace(/([a-z])([A-Z])(?=[a-z])/g,function(x,y,z){return y+" "+z.toLowerCase()}).replace(/([a-z0-9])[_\/-](?=[a-z0-9])/g,"$1 ").replace(/^[a-z]/,function(x){return x.toUpperCase()})},uppercase:function(w){return g(w).toUpperCase()},lowercase:function(w){return g(w).toLowerCase()},from:function(w,x){return m.between(w,x)},fromlast:function(w,x){return m.between(w,x,"",!0)},to:function(w,x){return m.between(w,"",x)},tofirst:function(w,x){return m.between(w,"",x,!0)},between:function(w,x,y,z){var A=[g(w),g(x),g(y)];w=A[0],x=A[1],y=A[2];var B=x?w[z?"lastIndexOf":"indexOf"](x):-1,C=w[z?"indexOf":"lastIndexOf"](y);return w.slice(B+1,-1!==C&&y?C:w.length)},filename:function(w){return Mavo.match(new URL(g(w),Mavo.base).pathname,/[^/]+?$/)},json:function(w){return Mavo.safeToJSON(w)},get $now(){return new Date},get $today(){return m.date(new Date)},year:l("year"),month:l("month"),day:l("day"),weekday:l("weekday"),hour:l("hour"),minute:l("minute"),second:l("second"),date:function(w){return m.year(w)+"-"+m.month(w).twodigit+"-"+m.day(w).twodigit},time:function(w){return m.hour(w).twodigit+":"+m.minute(w).twodigit+":"+m.second(w).twodigit},minutes:function(w){return Math.floor(Math.abs(w)/60)},hours:function(w){return Math.floor(Math.abs(w)/3600)},days:function(w){return Math.floor(Math.abs(w)/86400)},weeks:function(w){return Math.floor(Math.abs(w)/604800)},months:function(w){return Math.floor(Math.abs(w)/(86400*30.4368))},years:function(w){return Math.floor(Math.abs(w)/(12*(86400*30.4368)))},localTimezone:-new Date().getTimezoneOffset(),log:function(){var w;return(w=console).log.apply(w,arguments),0>=arguments.length?void 0:arguments[0]},util:{numbers:function(w,x){return w=Array.isArray(w)?w:x?$$(x):[w],w.filter(function(y){return!isNaN(y)&&""!==f(y)}).map(function(y){return+y})},date:function(w){if(w=f(w),!w)return null;if("string"===b.type(w)){w=w.trim(),/^\d{4}-\d{2}-\d{2}/.test(w)||(w=m.$today+" "+w),-1===w.indexOf(":")?w+="T00:00:00":w=w.replace(/\-(\d{2})\s+(?=\d{2}:)/,"-$1T"),w=w.replace(/\s+/g,"");var x=Mavo.match(w,/[+-]\d{2}:?\d{2}|Z$/);if(x)w=new Date(w);else{var y=w.match(/\d+/g);w=new Date(y[0],(y[1]||1)-1,y[2]||1,y[3]||0,y[4]||0,y[5]||0,y[6]||0)}}else w=new Date(w);return isNaN(w)?null:w}}},o=m.util;b.lazy(m,"$url",function(){var z,u={},w=new URL(location),x=!0,y=!1;try{for(var B,C,A=w.searchParams[Symbol.iterator]();!(x=(B=A.next()).done);x=!0)C=B.value,u[C[0]]=C[1]}catch(D){y=!0,z=D}finally{try{!x&&A.return&&A.return()}finally{if(y)throw z}}return Object.defineProperties(u,{path:{value:w.pathname.split("/").filter(function(D){return!!D})},toString:{value:function(){return new URL(location)}}}),u});var p={average:"avg",iff:"iff IF",subtract:"minus",multiply:"mult product",divide:"div",lt:"lessThan smaller",gt:"moreThan greater greaterThan bigger",eq:"equal equality",th:"ordinal"},q=function(w){p[w].split(/\s+/g).forEach(function(x){return m[x]=m[w]})};for(var r in p)q(r);m._Trap=self.Proxy?new Proxy(m,{get:function(w,x){var y,z=Mavo.getCanonicalProperty(w,x)||Mavo.getCanonicalProperty(Math,x);return z&&(y=w[z]||Math[z]),y?(y.toString=function(){return x},y):x in self?self[x]:x},has:function(w,x){return"data"!=x}}):m;var s=new Intl.NumberFormat("en",{minimumIntegerDigits:"2"});s=s.format.bind(s);var t={year:function(w){return w.getFullYear()},month:function(w){return w.getMonth()+1},day:function(w){return w.getDate()},weekday:function(w){return w.getDay()||7},hour:function(w){return w.getHours()},minute:function(w){return w.getMinutes()},second:function(w){return w.getSeconds()}}})(Bliss,Mavo.value);
"use strict";var _slicedToArray=function(){function c(d,f){var g=[],h=!0,j=!1,k;try{for(var m,l=d[Symbol.iterator]();!(h=(m=l.next()).done)&&(g.push(m.value),!(f&&g.length===f));h=!0);}catch(q){j=!0,k=q}finally{try{!h&&l["return"]&&l["return"]()}finally{if(j)throw k}}return g}return function(d,f){if(Array.isArray(d))return d;if(Symbol.iterator in Object(d))return c(d,f);throw new TypeError("Invalid attempt to destructure non-iterable instance")}}();function _toConsumableArray(c){if(Array.isArray(c)){for(var d=0,f=Array(c.length);d<c.length;d++)f[d]=c[d];return f}return Array.from(c)}(function(c,d,f){var g=Mavo.Script={addUnaryOperator:function(l,m){if(m.symbol){var t,q=!0,s=!1;try{for(var v,w,u=Mavo.toArray(m.symbol)[Symbol.iterator]();!(q=(v=u.next()).done);q=!0)w=v.value,Mavo.Script.symbols[w]=l}catch(x){s=!0,t=x}finally{try{!q&&u.return&&u.return()}finally{if(s)throw t}}}return Mavo.Functions[l]=function(x){return Array.isArray(x)?x.map(d).map(m.scalar):m.scalar(d(x))}},addBinaryOperator:function(l,m){if(m.symbol){var t,q=!0,s=!1;try{for(var v,w,u=Mavo.toArray(m.symbol)[Symbol.iterator]();!(q=(v=u.next()).done);q=!0)w=v.value,Mavo.Script.symbols[w]=l}catch(x){s=!0,t=x}finally{try{!q&&u.return&&u.return()}finally{if(s)throw t}}}return m.identity=void 0===m.identity?0:m.identity,Mavo.Functions[l]=m.code||function(){for(var x=arguments.length,y=Array(x),z=0;z<x;z++)y[z]=arguments[z];1===y.length&&Array.isArray(y[0])&&(y=[].concat(_toConsumableArray(y[0]))),m.raw||(y=y.map(d));for(var B,A=m.logical?m.identity:y[0],C=function(F){var G=m.logical?y[F-1]:A,H=y[F];Array.isArray(H)?("number"==typeof m.identity&&(H=f.numbers(H)),B=Array.isArray(G)?[].concat(_toConsumableArray(H.map(function(I,J){return m.scalar(void 0===G[J]?m.identity:G[J],I)})),_toConsumableArray(G.slice(H.length))):H.map(function(I){return m.scalar(G,I)})):Array.isArray(G)?B=G.map(function(I){return m.scalar(I,H)}):B=m.scalar(G,H),A=m.reduce?m.reduce(A,B,G,H):m.logical?A&&B:B},D=1;D<y.length;D++)C(D);return A}},symbols:{},getOperatorName:function(l){return Mavo.Script.symbols[l]||l},operators:{not:{symbol:"!",scalar:function(l){return!l}},multiply:{scalar:function(l,m){return l*m},identity:1,symbol:"*"},divide:{scalar:function(l,m){return l/m},identity:1,symbol:"/"},add:{scalar:function(l,m){return+l+ +m},symbol:"+"},subtract:{scalar:function(l,m){if(isNaN(l)||isNaN(m)){var q=f.date(l),s=f.date(m);if(q&&s)return(q-s)/1e3}return l-m},symbol:"-"},mod:{scalar:function(l,m){var q=l%m;return q+=0>q?m:0,q}},lte:{logical:!0,scalar:function(l,m){var q=Mavo.Script.getNumericalOperands(l,m),s=_slicedToArray(q,2);return l=s[0],m=s[1],l<=m},identity:!0,symbol:"<="},lt:{logical:!0,scalar:function(l,m){var q=Mavo.Script.getNumericalOperands(l,m),s=_slicedToArray(q,2);return l=s[0],m=s[1],l<m},identity:!0,symbol:"<"},gte:{logical:!0,scalar:function(l,m){var q=Mavo.Script.getNumericalOperands(l,m),s=_slicedToArray(q,2);return l=s[0],m=s[1],l>=m},identity:!0,symbol:">="},gt:{logical:!0,scalar:function(l,m){var q=Mavo.Script.getNumericalOperands(l,m),s=_slicedToArray(q,2);return l=s[0],m=s[1],l>m},identity:!0,symbol:">"},eq:{logical:!0,scalar:function(l,m){return l==m},symbol:["=","=="],identity:!0},neq:{logical:!0,scalar:function(l,m){return l!=m},symbol:["!="],identity:!0},and:{logical:!0,scalar:function(l,m){return!!l&&!!m},identity:!0,symbol:"&&"},or:{logical:!0,scalar:function(l,m){return l||m},reduce:function(l,m){return l||m},identity:!1,symbol:"||"},concatenate:{symbol:"&",identity:"",scalar:function(l,m){return""+(l||"")+(m||"")}},filter:{scalar:function(l,m){return d(m)?l:null},raw:!0}},getNumericalOperands:function(l,m){if(isNaN(l)||isNaN(m)){var q=f.date(l),s=f.date(m);if(q&&s)return[q,s]}return[l,m]},serializers:{BinaryExpression:function(l){return g.serialize(l.left)+" "+l.operator+" "+g.serialize(l.right)},UnaryExpression:function(l){return""+l.operator+g.serialize(l.argument)},CallExpression:function(l){return g.serialize(l.callee)+"("+l.arguments.map(g.serialize).join(", ")+")"},ConditionalExpression:function(l){return g.serialize(l.test)+"? "+g.serialize(l.consequent)+" : "+g.serialize(l.alternate)},MemberExpression:function(l){var m=l.computed?g.serialize(l.property):"\""+l.property.name+"\"";return"get("+g.serialize(l.object)+", "+m+")"},ArrayExpression:function(l){return"["+l.elements.map(g.serialize).join(", ")+"]"},Literal:function(l){return l.raw},Identifier:function(l){return l.name},ThisExpression:function(){return"this"},Compound:function(l){return l.body.map(g.serialize).join(" ")}},transformations:{BinaryExpression:function(l){var m=Mavo.Script.getOperatorName(l.operator),q=l,s=[];do s.unshift(q.right),q=q.left;while(Mavo.Script.getOperatorName(q.operator)===m);if(s.unshift(q),1<s.length)return m+"("+s.map(g.serialize).join(", ")+")"},UnaryExpression:function(l){var m=Mavo.Script.getOperatorName(l.operator);if(m)return m+"("+g.serialize(l.argument)+")"},CallExpression:function(l){"Identifier"==l.callee.type&&("if"==l.callee.name&&(l.callee.name="iff"),l.callee.name="Mavo.Functions._Trap."+l.callee.name)}},serialize:function(l){var m=g.transformations[l.type]&&g.transformations[l.type](l);return void 0===m?g.serializers[l.type](l):m},rewrite:function(l){try{return g.serialize(g.parse(l))}catch(m){return l}},compile:function(l){return l=g.rewrite(l),new Function("data","with(Mavo.Functions._Trap)\n\t\t\t\twith (data || {}) {\n\t\t\t\t\treturn "+l+";\n\t\t\t\t}")},parse:self.jsep};for(var h in self.jsep&&(jsep.addBinaryOp("and",2),jsep.addBinaryOp("or",2),jsep.addBinaryOp("=",6),jsep.addBinaryOp("mod",10),jsep.removeBinaryOp("===")),g.serializers.LogicalExpression=g.serializers.BinaryExpression,g.transformations.LogicalExpression=g.transformations.BinaryExpression,Mavo.Script.operators){var j=Mavo.Script.operators[h];2>j.scalar.length?Mavo.Script.addUnaryOperator(h,j):Mavo.Script.addBinaryOperator(h,j)}})(Bliss,Mavo.value,Mavo.Functions.util);
