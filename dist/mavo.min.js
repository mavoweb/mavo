...
t.remove(r.ui.clear)}),this.storage||this.source?(this.storage||this.source.permissions.can("read",function(){return r.permissions.read=!0}),this.permissions.can("read",function(){return r.load()})):(this.permissions.on(["read","edit"]),t.fire(this.element,"mavo:load")),this.needsEdit||(this.permissions.off(["edit","add","delete"]),this.element.addEventListener("mavo:load",function(t){var e=n.debounce(function(){r.save()},3e3),i=function(t){t.node.saved&&e()};requestAnimationFrame(function(){r.permissions.can("save",function(){r.element.addEventListener("mavo:datachange",i)},function(){r.element.removeEventListener("mavo:datachange",i)})})})),Mavo.hooks.run("init-end",this)},get editing(){return this.root.editing},get data(){return this.getData()},getData:function(t){return this.root.getData(t)},toJSON:function(){var t=arguments.length<=0||void 0===arguments[0]?this.data:arguments[0];return n.toJSON(t)},error:function i(e){for(var r,s=function(){return t.transition(i,{opacity:0}).then(t.remove);
...
tag:"button",className:"mv-close mv-ui",textContent:"×",events:{click:function(e){t.remove(this.parentNode)}}},"Deleted "+this.name,{tag:"button",className:"mv-undo mv-ui",textContent:"Undo",events:{click:function(t){return i.deleted=!1}}}]),this.element.classList.remove("mv-highlight")):this.deleted&&(this.element.textContent="",this.element.appendChild(this.elementContents),this._deleted=!1,this.dataChanged("undelete"))}),Mavo.Node.prototype.isDeleted=function(){this.deleted;return!!this.deleted||!!this.parentGroup&&this.parentGroup.isDeleted()},Mavo.hooks.add("node-init-end",function(t){this.collection=t.options.collection,this.collection&&(this.group=this.parentGroup=this.collection.parentGroup)})}(Bliss,Bliss.$);
"use strict";function _toConsumableArray(e){if(Array.isArray(e)){for(var t=0,r=Array(e.length);t<e.length;t++)r[t]=e[t];return r}return Array.from(e)}!function(e){Mavo.attributes.push("mv-expressions");var t=Mavo.Expression=e.Class({constructor:function(e){this.expression=e},eval:function(e){this.oldValue=this.value,Mavo.hooks.run("expression-eval-beforeeval",this);try{this["function"]||(this["function"]=t.compile(this.expression)),this.value=this["function"](e)}catch(r){Mavo.hooks.run("expression-eval-error",{context:this,exception:r}),this.value=r}return this.value},toString:function(){return this.expression},live:{expression:function(e){e=e;this["function"]=null}},"static":{serializers:{BinaryExpression:function(e){return t.serialize(e.left)+" "+e.operator+" "+t.serialize(e.right)},UnaryExpression:function(e){return""+e.operator+t.serialize(e.argument)},CallExpression:function(e){return t.serialize(e.callee)+"("+e.arguments.map(t.serialize).join(", ")+")"},ConditionalExpression:function(e){
return t.serialize(e.test)+"? "+t.serialize(e.consequent)+" : "+t.serialize(e.alternate)},MemberExpression:function(e){return t.serialize(e.object)+'["'+(e.property.name||e.property.value)+'"]'},ArrayExpression:function(e){return"["+e.elements.map(t.serialize).join(", ")+"]"},Literal:function(e){return e.raw},Identifier:function(e){return e.name},ThisExpression:function(e){return"this"},Compound:function(e){return e.body.map(t.serialize).join(" ")}},transformations:{BinaryExpression:function(e){var r=Mavo.Script.getOperatorName(e.operator),n=(Mavo.Script.operators[r],e),i=[];do i.unshift(n.right),n=n.left;while(Mavo.Script.getOperatorName(n.operator)===r);if(i.unshift(n),i.length>1)return r+"("+i.map(t.serialize).join(", ")+")"},CallExpression:function(e){"Identifier"==e.callee.type&&"if"==e.callee.name&&(e.callee.name="iff")}},serialize:function(e){if(t.transformations[e.type]){var r=t.transformations[e.type](e);if(void 0!==r)return r}return t.serializers[e.type](e)},rewrite:function(e){
try{return t.serialize(t.parse(e))}catch(r){return e}},compile:function(e){return e=t.rewrite(e),new Function("data","with(Mavo.Functions._Trap)\n\t\t\t\t\twith(data) {\n\t\t\t\t\t\treturn "+e+";\n\t\t\t\t\t}")},parse:self.jsep}});self.jsep&&(jsep.addBinaryOp("and",2),jsep.addBinaryOp("or",2),jsep.addBinaryOp("=",6),jsep.removeBinaryOp("===")),t.serializers.LogicalExpression=t.serializers.BinaryExpression,t.transformations.LogicalExpression=t.transformations.BinaryExpression,t.Syntax=e.Class({constructor:function(e,t){this.start=e,this.end=t,this.regex=RegExp(Mavo.escapeRegExp(e)+"([\\S\\s]+?)"+Mavo.escapeRegExp(t),"gi")},test:function(e){return this.regex.lastIndex=0,this.regex.test(e)},tokenize:function(e){var t,r=[],n=0;for(this.regex.lastIndex=0;null!==(t=this.regex.exec(e));)t.index>n&&r.push(e.substring(n,t.index)),n=this.regex.lastIndex,r.push(new Mavo.Expression(t[1]));return n<e.length&&r.push(e.substring(n)),r},"static":{create:function(e){if(e){var r=e.getAttribute("mv-expressions");
if(r)return r=r.trim(),/\s/.test(r)?new(Function.prototype.bind.apply(t.Syntax,[null].concat(_toConsumableArray(r.split(/\s+/))))):t.Syntax.ESCAPE}},ESCAPE:-1}}),t.Syntax["default"]=new t.Syntax("[","]")}(Bliss);
"use strict";function _toConsumableArray(t){if(Array.isArray(t)){for(var e=0,i=Array(t.length);e<t.length;e++)i[e]=t[e];return i}return Array.from(t)}!function(t){var e=Mavo.Expression.Text=t.Class({constructor:function(){var i=arguments.length<=0||void 0===arguments[0]?{}:arguments[0];this.template=i.template&&i.template.template||i.template;for(var s=["group","path","syntax","fallback","attribute"],n=0;n<s.length;n++){var a=s[n];this[a]=void 0===i[a]&&this.template?this.template[a]:i[a]}if(this.node=i.node,this.node||(this.node=this.path.reduce(function(t,e){return t.childNodes[e]},this.group.element)),this.element=this.node,this.attribute=this.attribute||null,Mavo.hooks.run("expressiontext-init-start",this),!this.expression){if(3===this.node.nodeType&&(this.element=this.node.parentNode,this.node.parentNode.children.length&&!this.attribute||(this.node=this.element,this.element.normalize())),this.attribute)this.expression=this.node.getAttribute(this.attribute).trim();else{if(this.node.normalize(),
this.node.firstChild&&1===this.node.childNodes.length&&3===this.node.firstChild.nodeType){var r=this.node.firstChild.textContent.match(/^\s*|\s*$/g);r[1]&&(this.node.firstChild.splitText(this.node.firstChild.textContent.length-r[1].length),t.after(this.node.lastChild,this.node)),r[0]&&(this.node.firstChild.splitText(r[0].length),this.node.parentNode.insertBefore(this.node.firstChild,this.node))}this.expression=this.node.textContent}this.parsed=i.template?i.template.parsed:this.syntax.tokenize(this.expression)}Mavo.hooks.run("expressiontext-init-end",this),e.elements.set(this.element,[].concat(_toConsumableArray(e.elements.get(this.element)||[]),[this]))},update:function(){var t=this,e=arguments.length<=0||void 0===arguments[0]?this.data:arguments[0];this.data=e;var i={};Mavo.hooks.run("expressiontext-update-start",this),i.value=this.value=this.parsed.map(function(i){if(i instanceof Mavo.Expression){var s={context:t,expr:i};return Mavo.hooks.run("expressiontext-update-beforeeval",s),s.value=s.expr.eval(e),
Mavo.hooks.run("expressiontext-update-aftereval",s),s.value instanceof Error?void 0!==t.fallback?t.fallback:s.expr.expression:void 0===s.value||null===s.value?"":s.value}return i}),this.attribute||(i.presentational=this.value.map(function(t){return Array.isArray(t)?t.join(", "):"number"==typeof t?Mavo.Primitive.formatNumber(t):t}),i.presentational=1===i.presentational.length?i.presentational[0]:i.presentational.join("")),i.value=1===i.value.length?i.value[0]:i.value.join(""),this.primitive&&1===this.parsed.length&&("number"==typeof i.value?this.primitive.datatype="number":"boolean"==typeof i.value&&(this.primitive.datatype="boolean")),i.presentational===i.value&&(i=i.value),this.primitive?this.primitive.value=i:Mavo.Primitive.setValue(this.node,i,{attribute:this.attribute}),Mavo.hooks.run("expressiontext-update-end",this)},"static":{elements:new WeakMap,search:function(t,i){var s=e.elements.get(t)||[];return arguments.length>1?s.length?s.filter(function(t){return t.attribute===i})[0]||null:null:s;
}}});Mavo.hooks.add("primitive-init-start",function(){this.expressionText=Mavo.Expression.Text.search(this.element,this.attribute),this.expressionText&&(this.expressionText.primitive=this,this.store=this.store||"none",this.modes="read")}),Mavo.hooks.add("collection-add-end",function(t){if(t.item instanceof Mavo.Primitive&&this.itemTemplate){var e=Mavo.Expression.Text.search(this.itemTemplate.element)[0];e&&e.group.expressions.all.push(new Mavo.Expression.Text({node:t.item.element,template:e}))}})}(Bliss);
"use strict";var _typeof="function"==typeof Symbol&&"symbol"==typeof Symbol.iterator?function(t){return typeof t}:function(t){return t&&"function"==typeof Symbol&&t.constructor===Symbol?"symbol":typeof t};!function(t,e){Mavo.attributes.push("mv-value","mv-if");var i=Mavo.Expressions=t.Class({constructor:function(t){var e=this;if(t&&(this.group=t,this.group.expressions=this),this.all=[],Mavo.hooks.run("expressions-init-start",this),this.group){var i=this.group.template;if(i&&i.expressions){var n=!0,r=!1,s=void 0;try{for(var o,a=i.expressions.all[Symbol.iterator]();!(n=(o=a.next()).done);n=!0){var u=o.value;this.all.push(new Mavo.Expression.Text({template:u,group:this.group}))}}catch(v){r=!0,s=v}finally{try{!n&&a["return"]&&a["return"]()}finally{if(r)throw s}}}else{var l=Mavo.Expression.Syntax.create(this.group.element.closest("[mv-expressions]"))||Mavo.Expression.Syntax["default"];this.traverse(this.group.element,void 0,l)}}this.dependents=new Set,this.active=!0,this.group.element.addEventListener("mavo:datachange",function(t){
return e.update()})},update:function(){if(this.active&&!this.group.isDeleted()&&this.all.length+this.dependents.size!==0){var t={context:this,data:this.group.getRelativeData()};Mavo.hooks.run("expressions-update-start",t);var e=!0,i=!1,n=void 0;try{for(var r,s=this.all[Symbol.iterator]();!(e=(r=s.next()).done);e=!0){var o=r.value;o.update(t.data)}}catch(a){i=!0,n=a}finally{try{!e&&s["return"]&&s["return"]()}finally{if(i)throw n}}var u=!0,v=!1,l=void 0;try{for(var p,d=this.dependents[Symbol.iterator]();!(u=(p=d.next()).done);u=!0){var h=p.value;h.update()}}catch(a){v=!0,l=a}finally{try{!u&&d["return"]&&d["return"]()}finally{if(v)throw l}}}},extract:function(t,e,n,r){e&&"mv-expressions"==e.name||(e&&i.directives.indexOf(e.name)>-1||r.test(e?e.value:t.textContent))&&this.all.push(new Mavo.Expression.Text({node:t,syntax:r,path:n?n.slice(1).split("/").map(function(t){return+t}):[],attribute:e&&e.name,group:this.group}))},traverse:function(t){var i=this,n=arguments.length<=1||void 0===arguments[1]?"":arguments[1],r=arguments[2];
if(8!==t.nodeType)if(3===t.nodeType)this.extract(t,null,n,r);else if(t==this.group.element||!Mavo.is("group",t)){if(r=Mavo.Expression.Syntax.create(t)||r,r===Mavo.Expression.Syntax.ESCAPE)return;e(t.attributes).forEach(function(e){return i.extract(t,e,n,r)}),e(t.childNodes).forEach(function(t,e){return i.traverse(t,n+"/"+e,r)})}},"static":{directives:[]}});self.Proxy&&Mavo.hooks.add("node-getdata-end",function(t){var e=this;t.options.relative&&t.data&&"object"===_typeof(t.data)&&(t.data=new Proxy(t.data,{get:function(i,n,r){if(n in i||n in r&&n in i)return i[n];if("$index"==n)return e.index+1;if(n==e.mavo.id)return i;var s=e.walkUp(function(i){if(n in i.children)return i.expressions.dependents.add(e.expressions),i.children[n].getData(t.options)});return void 0!==s?s:void 0},has:function(i,n){if(n in i)return!0;if("$index"==n||n==e.mavo.id)return!0;var r=e.walkUp(function(t){if(n in t.children)return!0});return void 0!==r?r:(r=e.find(n),void 0!==r?(r=Array.isArray(r)?r.map(function(e){return e.getData(t.options);
}).filter(function(t){return null!==t}):r.getData(t.options),i[n]=r,!0):void 0)},set:function(t,e,i){throw Error("You can’t set data via expressions.")}}))}),Mavo.Node.prototype.getRelativeData=function(){return this.getData({relative:!0,store:"*","null":!0,unhandled:this.mavo.unhandled})},Mavo.hooks.add("group-init-start",function(){new Mavo.Expressions(this)}),Mavo.hooks.add("group-init-end",function(){this.expressions.update()}),Mavo.hooks.add("group-render-start",function(){this.expressions.active=!1}),Mavo.hooks.add("group-render-end",function(){var t=this;requestAnimationFrame(function(){t.expressions.active=!0,t.expressions.update()})})}(Bliss,Bliss.$),Mavo.Expressions.directives.push("mv-value"),Mavo.hooks.add("expressiontext-init-start",function(){"mv-value"==this.attribute&&(this.attribute=Mavo.Primitive.getValueAttribute(this.element),this.fallback=this.fallback||Mavo.Primitive.getValue(this.element,{attribute:this.attribute}),this.expression=this.element.getAttribute("mv-value"),
this.parsed=[new Mavo.Expression(this.expression)],this.expression=this.syntax.start+this.expression+this.syntax.end)});
"use strict";Mavo.Expressions.directives.push("mv-if"),Mavo.hooks.add("expressiontext-init-start",function(){"mv-if"==this.attribute&&(this.expression=this.element.getAttribute("mv-if"),this.parsed=[new Mavo.Expression(this.expression)],this.expression=this.syntax.start+this.expression+this.syntax.end,this.parentIf=this.element.parentNode&&Mavo.Expression.Text.search(this.element.parentNode.closest("[mv-if]"),"mv-if"),this.parentIf&&(this.parentIf.childIfs=(this.parentIf.childIfs||new Set).add(this)))}),Mavo.hooks.add("expressiontext-update-end",function(){var e=this;"mv-if"==this.attribute&&this.group.mavo.treeBuilt.then(function(){var t=e.value[0],i=e.oldValue&&e.oldValue[0];if(e.parentIf){var n=e.parentIf.value[0];e.value[0]=t=t&&n}if(t!==i){if(n!==!1&&(t?e.comment&&e.comment.parentNode&&e.comment.parentNode.replaceChild(e.element,e.comment):e.element.parentNode&&(e.comment||(e.comment=document.createComment("mv-if")),e.element.parentNode.replaceChild(e.comment,e.element))),e.childProperties){
var r=!0,o=!1,a=void 0;try{for(var s,l=e.childProperties[Symbol.iterator]();!(r=(s=l.next()).done);r=!0){var d=s.value;d.hidden=!t}}catch(h){o=!0,a=h}finally{try{!r&&l["return"]&&l["return"]()}finally{if(o)throw a}}}if(e.childIfs){var m=!0,v=!1,f=void 0;try{for(var p,u=e.childIfs[Symbol.iterator]();!(m=(p=u.next()).done);m=!0){var c=p.value;c.update()}}catch(h){v=!0,f=h}finally{try{!m&&u["return"]&&u["return"]()}finally{if(v)throw f}}}e.oldValue=e.value}})}),Mavo.hooks.add("unit-isdatanull",function(e){e.result=e.result||this.hidden&&"*"==e.options.store}),$.live(Mavo.Primitive.prototype,"hidden",function(e){this._hidden!==e&&(this._hidden=e,this.dataChanged())}),$.lazy(Mavo.Expression.Text.prototype,"childProperties",function(){var e=this;if("mv-if"==this.attribute){var t=$$(Mavo.selectors.property,this.element).filter(function(t){return t.closest("[mv-if]")==e.element}).map(function(e){return Mavo.Node.get(e)});return t.length&&this.element.addEventListener("mavo:datachange",function(t){
requestAnimationFrame(function(){e.element.parentNode||e.group.element.dispatchEvent(t)})}),t}});
