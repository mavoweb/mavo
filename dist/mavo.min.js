...
"use strict";function _toConsumableArray(t){if(Array.isArray(t)){for(var e=0,i=Array(t.length);e<t.length;e++)i[e]=t[e];return i}return Array.from(t)}!function(t){var e=Mavo.Expression.Text=t.Class({constructor:function(){var i=arguments.length<=0||void 0===arguments[0]?{}:arguments[0];this.template=i.template&&i.template.template||i.template;for(var n=["group","path","syntax","fallback","attribute"],s=0;s<n.length;s++){var a=n[s];this[a]=void 0===i[a]&&this.template?this.template[a]:i[a]}if(this.node=i.node,this.node||(this.node=this.path.reduce(function(t,e){return t.childNodes[e]},this.group.element)),this.element=this.node,this.attribute=this.attribute||null,Mavo.hooks.run("expressiontext-init-start",this),!this.expression){if(3===this.node.nodeType&&(this.element=this.node.parentNode,this.node.parentNode.children.length&&!this.attribute||(this.node=this.element,this.element.normalize())),this.attribute)this.expression=this.node.getAttribute(this.attribute).trim();else{if(this.node.normalize(),
this.node.firstChild&&1===this.node.childNodes.length&&3===this.node.firstChild.nodeType){var r=this.node.firstChild.textContent.match(/^\s*|\s*$/g);r[1]&&(this.node.firstChild.splitText(this.node.firstChild.textContent.length-r[1].length),t.after(this.node.lastChild,this.node)),r[0]&&(this.node.firstChild.splitText(r[0].length),this.node.parentNode.insertBefore(this.node.firstChild,this.node))}this.expression=this.node.textContent}this.parsed=i.template?i.template.parsed:this.syntax.tokenize(this.expression)}Mavo.hooks.run("expressiontext-init-end",this),e.elements.set(this.element,[].concat(_toConsumableArray(e.elements.get(this.element)||[]),[this]))},update:function(){var t=this,e=arguments.length<=0||void 0===arguments[0]?this.data:arguments[0];this.data=e;var i={};Mavo.hooks.run("expressiontext-update-start",this),i.value=this.value=this.parsed.map(function(i){if(i instanceof Mavo.Expression){var n={context:t,expr:i};return Mavo.hooks.run("expressiontext-update-beforeeval",n),n.value=n.expr.eval(e),
Mavo.hooks.run("expressiontext-update-aftereval",n),n.value instanceof Error?void 0!==t.fallback?t.fallback:n.expr.expression:void 0===n.value||null===n.value?"":n.value}return i}),this.attribute||(i.presentational=this.value.map(function(t){return Array.isArray(t)?t.join(", "):"number"==typeof t?Mavo.Primitive.formatNumber(t):t}),i.presentational=1===i.presentational.length?i.presentational[0]:i.presentational.join("")),i.value=1===i.value.length?i.value[0]:i.value.join(""),this.primitive&&1===this.parsed.length&&("number"==typeof i.value?this.primitive.datatype="number":"boolean"==typeof i.value&&(this.primitive.datatype="boolean")),i.presentational===i.value&&(i=i.value),this.primitive?this.primitive.value=i:Mavo.Primitive.setValue(this.node,i,{attribute:this.attribute}),Mavo.hooks.run("expressiontext-update-end",this)},"static":{elements:new WeakMap,search:function(t,i){var n=e.elements.get(t)||[];return arguments.length>1?n.length?n.filter(function(t){return t.attribute===i})[0]||null:null:n;
}}})}(Bliss);
"use strict";var _typeof="function"==typeof Symbol&&"symbol"==typeof Symbol.iterator?function(t){return typeof t}:function(t){return t&&"function"==typeof Symbol&&t.constructor===Symbol?"symbol":typeof t};!function(t,e){Mavo.attributes.push("mv-value","mv-if");var i=Mavo.Expressions=t.Class({constructor:function(t){var e=this;if(t&&(this.group=t,this.group.expressions=this),this.all=[],Mavo.hooks.run("expressions-init-start",this),this.group){var i=this.group.template;if(i&&i.expressions){var s=!0,n=!1,r=void 0;try{for(var o,a=i.expressions.all[Symbol.iterator]();!(s=(o=a.next()).done);s=!0){var u=o.value;this.all.push(new Mavo.Expression.Text({template:u,group:this.group}))}}catch(p){n=!0,r=p}finally{try{!s&&a["return"]&&a["return"]()}finally{if(n)throw r}}}else{var v=Mavo.Expression.Syntax.create(this.group.element.closest("[mv-expressions]"))||Mavo.Expression.Syntax["default"];this.traverse(this.group.element,void 0,v)}}this.dependents=new Set,this.active=!0,this.group.element.addEventListener("mavo:datachange",function(t){
return e.update()})},update:function(){if(this.active&&!this.group.isDeleted()&&this.all.length+this.dependents.size!==0){var t={context:this,data:this.group.getRelativeData()};Mavo.hooks.run("expressions-update-start",t);var e=!0,i=!1,s=void 0;try{for(var n,r=this.all[Symbol.iterator]();!(e=(n=r.next()).done);e=!0){var o=n.value;o.update(t.data)}}catch(a){i=!0,s=a}finally{try{!e&&r["return"]&&r["return"]()}finally{if(i)throw s}}var u=!0,p=!1,v=void 0;try{for(var h,l=this.dependents[Symbol.iterator]();!(u=(h=l.next()).done);u=!0){var d=h.value;d.update()}}catch(a){p=!0,v=a}finally{try{!u&&l["return"]&&l["return"]()}finally{if(p)throw v}}}},extract:function(t,e,s,n){e&&"mv-expressions"==e.name||(e&&i.directives.indexOf(e.name)>-1||n.test(e?e.value:t.textContent))&&this.all.push(new Mavo.Expression.Text({node:t,syntax:n,path:s?s.slice(1).split("/").map(function(t){return+t}):[],attribute:e&&e.name,group:this.group}))},traverse:function(t){var i=this,s=arguments.length<=1||void 0===arguments[1]?"":arguments[1],n=arguments[2];
if(8!==t.nodeType)if(3===t.nodeType)this.extract(t,null,s,n);else if(t==this.group.element||!Mavo.is("group",t)){if(n=Mavo.Expression.Syntax.create(t)||n,n===Mavo.Expression.Syntax.ESCAPE)return;e(t.attributes).forEach(function(e){return i.extract(t,e,s,n)}),e(t.childNodes).forEach(function(t,e){return i.traverse(t,s+"/"+e,n)})}},"static":{directives:[]}});self.Proxy&&Mavo.hooks.add("node-getdata-end",function(t){var e=this;t.options.relative&&t.data&&"object"===_typeof(t.data)&&(t.data=new Proxy(t.data,{get:function(i,s,n){if(s in i||s in n&&s in i)return i[s];if("$index"==s)return e.index+1;if(s==e.mavo.id)return i;var r=e.walkUp(function(i){if(s in i.children)return i.expressions.dependents.add(e.expressions),i.children[s].getData(t.options)});return void 0!==r?r:void 0},has:function(i,s){if(s in i)return!0;if("$index"==s||s==e.mavo.id)return!0;var n=e.walkUp(function(t){if(s in t.children)return!0});return void 0!==n?n:(n=e.find(s),void 0!==n?(n=Array.isArray(n)?n.map(function(e){return e.getData(t.options);
}).filter(function(t){return null!==t}):n.getData(t.options),i[s]=n,!0):void 0)},set:function(t,e,i){throw Error("You can’t set data via expressions.")}}))}),Mavo.Node.prototype.getRelativeData=function(){return this.getData({relative:!0,store:"*","null":!0,unhandled:this.mavo.unhandled})},Mavo.hooks.add("group-init-start",function(){new Mavo.Expressions(this)}),Mavo.hooks.add("collection-add-end",function(t){if(t.item instanceof Mavo.Primitive&&this.itemTemplate){var e=Mavo.Expression.Text.search(this.itemTemplate.element)[0];e&&e.group.expressions.all.push(new Mavo.Expression.Text({node:t.item.element,template:e}))}}),Mavo.hooks.add("group-init-end",function(){this.expressions.update()}),Mavo.hooks.add("primitive-init-start",function(){this.expressionText=Mavo.Expression.Text.search(this.element,this.attribute),this.expressionText&&(this.expressionText.primitive=this,this.store=this.store||"none",this.modes="read")}),Mavo.hooks.add("group-render-start",function(){this.expressions.active=!1;
}),Mavo.hooks.add("group-render-end",function(){var t=this;requestAnimationFrame(function(){t.expressions.active=!0,t.expressions.update()})})}(Bliss,Bliss.$),Mavo.Expressions.directives.push("mv-value"),Mavo.hooks.add("expressiontext-init-start",function(){"mv-value"==this.attribute&&(this.attribute=Mavo.Primitive.getValueAttribute(this.element),this.fallback=this.fallback||Mavo.Primitive.getValue(this.element,{attribute:this.attribute}),this.expression=this.element.getAttribute("mv-value"),this.parsed=[new Mavo.Expression(this.expression)],this.expression=this.syntax.start+this.expression+this.syntax.end)});
